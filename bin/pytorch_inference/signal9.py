...
def launch_pytorch_app(args, input_pipe, restore_file):
    ...
    command = [ #'/heaptrack/build/bin/heaptrack',
	 path_to_app(),
        '--restore=' + restore_file,
        '--input=' + input_pipe, '--inputIsPipe',
        '--output=' + args.output_file,# '--outputIsPipe',
		# '--logPipe=' + args.log_file,
        '--validElasticLicenseKeyConfirmed=true',
        ]
...
def wait_for_pipe(file_name, num_retries=5) :
	...
	while num_retries > 0:
		try:
			print("Checking " + file_name + " is fifo")
			if stat.S_ISFIFO(os.stat(file_name).st_mode):
                                print(file_name + " fifo exists")
                                break
	...
	print("returning: " + str( stat.S_ISFIFO(os.stat(file_name).st_mode)))
...
def write_random_request(request_id, destination):    
    ...
    #json.dump(build_random_inference_request(request_id=request_id), sys.stdout)            
...
def main():
    ...
    os.environ["(ASAN_OPTIONS"]="halt_on_error=false,log_path=./as.log"
    ...
    #os.mkfifo(input_pipe_name, 0o600)

    p = launch_pytorch_app(args, input_pipe=input_pipe_name, restore_file=restore_file_name)

    if not wait_for_pipe(input_pipe_name):	
        print("Error: input pipe [{}] has not been created".format(input_pipe_name))
        return
    else:
        print("Error: input pipe [{}] has been created".format(input_pipe_name))

    with open(input_pipe_name, 'w') as input_pipe:
