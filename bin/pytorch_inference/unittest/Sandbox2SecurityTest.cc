...
struct Sandbox2TestFixture {
    Sandbox2TestFixture() {
        ...
        m_hasPrivileges = (getuid() == 0 || access("/proc/sys/kernel/unprivileged_userns_clone", F_OK) == 0);
    }
...
int main() {
    ...
    sandbox2::Sandbox2 sandbox(std::move(policy), 
                               std::make_unique<sandbox2::Executor>(test_program, args));
