...
# We want Eigen version 3.2.5 for our current branch
grep '^#define EIGEN_WORLD_VERSION 3' "$VERSION_FILE" > /dev/null 2>&1 && \
grep '^#define EIGEN_MAJOR_VERSION 2' "$VERSION_FILE" > /dev/null 2>&1 && \
grep '^#define EIGEN_MINOR_VERSION 5' "$VERSION_FILE" > /dev/null 2>&1
if [ $? -ne 0 ] ; then
    rm -rf eigen
    git -c advice.detachedHead=false clone --depth=1 --branch=3.2.5 https://gitlab.com/libeigen/eigen-backup.git eigen
fi
