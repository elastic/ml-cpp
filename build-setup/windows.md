...
- `/c/PROGRA~2/MICROS~1.0/VC/bin/x86_amd64`
- `/c/PROGRA~2/MICROS~1.0/VC/bin`
- `/c/PROGRA~2/MICROS~1.0/Common7/IDE`
- `/c/PROGRA~2/MICROS~1/WINDOW~4/8.1/bin/x64`
- `/c/PROGRA~2/MICROS~1/WINDOW~4/8.1/bin/x86`
- `/c/PROGRA~2/MICROS~1.0/TEAMTO~1/PERFOR~1/x64`
- `/c/PROGRA~2/MICROS~1.0/TEAMTO~1/PERFOR~1`
...
Visual Studio 2013 Express is free. Download the web installer for update 4, `wdexpress_full.exe`, from <https://www.microsoft.com/en-gb/download/details.aspx?id=44914>, and run it.

Accept the license and the default installation location.

### Git for Windows

Download `Git-2.10.2-64-bit.exe` from <https://github.com/git-for-windows/git/releases/tag/v2.10.2.windows.1>.
...
Start a command prompt using Start Menu -&gt; Apps -&gt; Microsoft Visual Studio 2013 -&gt; Visual Studio Tools -&gt; VS2013 x64 Cross Tools Command Prompt, then in it type:
...
Download the source code from <http://zlib.net/> - the file is called `zlib1211.zip`. Unzip this file under `C:\tools`, so that you end up with a directory called `C:\tools\zlib-1.2.8`.

To build, start a command prompt using Start Menu -&gt; Apps -&gt; Microsoft Visual Studio 2013 -&gt; Visual Studio Tools -&gt; VS2013 x64 Cross Tools Command Prompt, then in it type:
...
All the build output will end up in the top level `C:\tools\zlib-1.2.8` directory. Once the build is complete, copy `zlib1.dll` and `minigzip.exe` to `C:\usr\local\bin`. Copy `zlib.lib` and `zdll.lib` to `C:\usr\local\lib`. And copy `zlib.h` and `zconf.h` to `C:\usr\local\include`.
...
Double click `C:\tools\cppunit-1.13.2\src\CppUnitLibraries2010.sln`. Visual Studio 2013 should load. Accept its offer to convert the old style project file to the latest format.

Select "Release" in the "Solution Configurations" dropdown and "x64" in the "Solution Platforms" dropdown. Then press F7 to build. The IDE will attempt to build all five projects - some of them will fail to build, but that doesn't matter as the two important ones are the static and dynamic cppunit libraries.
...
Next, double click on C:\tools\apache-log4cxx-0.10.0\projects\log4cxx.dsw to start the Visual Studio 2013 IDE. When asked if you want to convert the project file format, click OK.
...
In property manager tree (on right hand side), click on log4cxx, then click the properties button. In the resulting dialog box, go to Configuration Properties -&gt; Linker -&gt; General, select "Additional Library Directories", choose "&lt;Edit...&gt;" from the dropdown and add:
...
Press F7 to build the solution, and hopefully log4cxx will build. The other three projects will NOT build, but that doesn't matter as they were already built in the preceeding steps. Assuming log4cxx itself builds, you can quit the IDE.
