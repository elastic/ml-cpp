        ...
        if (isNonInformative) {
            ...
            for (std::size_t i = 0u; i < samples.size(); ++i) {
                ...
                double seasonalScale =
                    std::sqrt(maths_t::seasonalVarianceScale(weightStyles, weights[i]));
                ...
                double deviation =
                    std::sqrt((scaledPrecision + 1.0) / scaledPrecision * scaledRate / shape);
    ...
    bool operator()(double x, double &result) const {
        ...
        double impliedRate =
            m_Rate + 0.5 * (m_SampleSquareDeviation +
                            m_Precision * m_WeightedNumberSamples * (sampleMean - m_Mean) *
        ...
        try {
            for (std::size_t i = 0u; i < samples.size(); ++i) {
                ...
                if (seasonalScale != 1.0) {
                    sampleMoments.add(
                        predictionMean + (samples[i] - predictionMean) / seasonalScale, n * w);
            ...
            m_SampleSquareDeviation =
                (m_WeightedNumberSamples - 1.0) * CBasicStatistics::variance(sampleMoments);
...
void CNormalMeanPrecConjugate::reset(maths_t::EDataType dataType,
    ...
    m_GammaRate =
        NON_INFORMATIVE_RATE + n / 2.0 * (variance + (this->isInteger() ? 1.0 / 12.0 : 0.0));
...
bool CNormalMeanPrecConjugate::needsOffset(void) const { return false; }
...
CNormalMeanPrecConjugate::EPrior CNormalMeanPrecConjugate::type(void) const { return E_Normal; }
...
double CNormalMeanPrecConjugate::offset(void) const { return 0.0; }
...
void CNormalMeanPrecConjugate::addSamples(const TWeightStyleVec &weightStyles,
    ...
    double sampleSquareDeviation =
        (scaledNumberSamples - 1.0) * CBasicStatistics::variance(sampleMoments);
    ...
    m_GammaRate +=
        0.5 * (sampleSquareDeviation +
               m_GaussianPrecision * scaledNumberSamples * (sampleMean - m_GaussianMean) *
...
CNormalMeanPrecConjugate::marginalLikelihoodConfidenceInterval(double percentage,
    ...
    try {
        ...
        double scale =
            std::sqrt((scaledPrecision + 1.0) / scaledPrecision * scaledRate / m_GammaShape);
        ...
        if (m_GammaShape > MINIMUM_GAUSSIAN_SHAPE) {
            ...
            double x2 = percentage > 0.0 ? boost::math::quantile(normal, (1.0 + percentage) / 2.0) -
                                               (this->isInteger() ? 0.5 : 0.0)
                                         : x1;
...
void CNormalMeanPrecConjugate::sampleMarginalLikelihood(std::size_t numberSamples,
    ...
    if (m_GammaShape > MINIMUM_GAUSSIAN_SHAPE) {
        double variance =
            (m_GaussianPrecision + 1.0) / m_GaussianPrecision * m_GammaRate / m_GammaShape;
        ...
        try {
            ...
            for (std::size_t i = 1u; i < numberSamples; ++i) {
                ...
                double partialExpectation =
                    m_GaussianMean * q - variance * CTools::safePdf(normal, xq);
                ...
                if (sample >= support.first && sample <= support.second) {
                    ...
                    LOG_ERROR("Sample out of bounds: sample = "
                              << sample << ", gaussianMean = " << m_GaussianMean
        ...
        try {
            ...
            double constant =
                CTools::safePdf(students, 0.0) * scale * degreesFreedom / (degreesFreedom - 1.0);
            ...
            for (std::size_t i = 1u; i < numberSamples; ++i) {
                ...
                double partialExpectation =
                    m_GaussianMean * q -
                    constant * std::exp(-(degreesFreedom - 1.0) / 2.0 * std::log(1.0 + residual));
            ...
            LOG_ERROR("Failed to sample: " << e.what() << ", degreesFreedom = " << degreesFreedom);
        }
    }
...
void CNormalMeanPrecConjugate::print(const std::string &indent, std::string &result) const {
    ...
    result += "mean = " + core::CStringUtils::typeToStringPretty(this->marginalLikelihoodMean()) +
              " sd = " +
              core::CStringUtils::typeToStringPretty(std::sqrt(this->marginalLikelihoodVariance()));
}
...
std::size_t CNormalMeanPrecConjugate::memoryUsage(void) const { return 0; }

std::size_t CNormalMeanPrecConjugate::staticSize(void) const { return sizeof(*this); }
...
double CNormalMeanPrecConjugate::mean(void) const { return m_GaussianMean; }
...
bool CNormalMeanPrecConjugate::isBad(void) const {
    return !CMathsFuncs::isFinite(m_GaussianMean) || !CMathsFuncs::isFinite(m_GaussianPrecision) ||
