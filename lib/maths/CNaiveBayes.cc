...
CNaiveBayesFeatureDensityFromPrior::CNaiveBayesFeatureDensityFromPrior(const CPrior& prior) : m_Prior(prior.clone()) {
}
...
bool CNaiveBayesFeatureDensityFromPrior::acceptRestoreTraverser(const SDistributionRestoreParams& params,
    ...
    do {
        ...
        RESTORE(PRIOR_TAG,
                traverser.traverseSubLevel(boost::bind<bool>(CPriorStateSerialiser(), boost::cref(params), boost::ref(m_Prior), _1)));
...
double CNaiveBayesFeatureDensityFromPrior::logValue(const TDouble1Vec& x) const {
    double result;
    if (m_Prior->jointLogMarginalLikelihood(CConstantWeights::COUNT, x, CConstantWeights::SINGLE_UNIT, result) != maths_t::E_FpNoErrors) {
...
double CNaiveBayesFeatureDensityFromPrior::logMaximumValue() const {
    ...
    if (m_Prior->jointLogMarginalLikelihood(
            CConstantWeights::COUNT, {m_Prior->marginalLikelihoodMode()}, CConstantWeights::SINGLE_UNIT, result) != maths_t::E_FpNoErrors) {
...
CNaiveBayes::CNaiveBayes(const CNaiveBayesFeatureDensity& exemplar, double decayRate, TOptionalDouble minMaxLogLikelihoodToUseFeature)
...
CNaiveBayes::CNaiveBayes(const SDistributionRestoreParams& params, core::CStateRestoreTraverser& traverser)
    : m_DecayRate{params.s_DecayRate}, m_ClassConditionalDensities{2} {
    traverser.traverseSubLevel(boost::bind(&CNaiveBayes::acceptRestoreTraverser, this, boost::cref(params), _1));
}

bool CNaiveBayes::acceptRestoreTraverser(const SDistributionRestoreParams& params, core::CStateRestoreTraverser& traverser) {
    std::size_t label;
    do {
        const std::string& name{traverser.name()};
        ...
        RESTORE_SETUP_TEARDOWN(
            CLASS_MODEL_TAG,
            SClass class_,
            traverser.traverseSubLevel(boost::bind(&SClass::acceptRestoreTraverser, boost::ref(class_), boost::cref(params), _1)),
            m_ClassConditionalDensities.emplace(label, class_))
        RESTORE_SETUP_TEARDOWN(MIN_MAX_LOG_LIKELIHOOD_TO_USE_FEATURE_TAG,
                               double value,
...
void CNaiveBayes::acceptPersistInserter(core::CStatePersistInserter& inserter) const {
    ...
    for (const auto& class_ : classes) {
        ...
        inserter.insertLevel(CLASS_MODEL_TAG, boost::bind(&SClass::acceptPersistInserter, boost::ref(class_->second), _1));
    }
    if (m_MinMaxLogLikelihoodToUseFeature) {
        inserter.insertValue(
            MIN_MAX_LOG_LIKELIHOOD_TO_USE_FEATURE_TAG, *m_MinMaxLogLikelihoodToUseFeature, core::CIEEE754::E_SinglePrecision);
    }
}
...
void CNaiveBayes::addTrainingDataPoint(std::size_t label, const TDouble1VecVec& x) {
    ...
    if (class_.s_ConditionalDensities.empty()) {
        ...
        std::generate_n(
            std::back_inserter(class_.s_ConditionalDensities), x.size(), [this]() { return TFeatureDensityPtr{m_Exemplar->clone()}; });
    }
...
CNaiveBayes::TDoubleSizePrVec CNaiveBayes::highestClassProbabilities(std::size_t n, const TDouble1VecVec& x) const {
...
double CNaiveBayes::classProbability(std::size_t label, const TDouble1VecVec& x) const {
    TDoubleSizePrVec p(this->classProbabilities(x));
    auto i = std::find_if(p.begin(), p.end(), [label](const TDoubleSizePr& p_) { return p_.second == label; });
        ...
        if (x[i].size() > 0) {
            ...
            if (m_MinMaxLogLikelihoodToUseFeature) {
                weight = CTools::logisticFunction(
                    (maxLogLikelihood[0] - *m_MinMaxLogLikelihoodToUseFeature) / std::fabs(*m_MinMaxLogLikelihoodToUseFeature), 0.1);
            }
...
std::size_t CNaiveBayes::memoryUsage() const {
    return core::CMemory::dynamicSize(m_Exemplar) + core::CMemory::dynamicSize(m_ClassConditionalDensities);
}
...
bool CNaiveBayes::validate(const TDouble1VecVec& x) const {
    ...
    if (class_ != m_ClassConditionalDensities.end() && class_->second.s_ConditionalDensities.size() > 0 &&
...
bool CNaiveBayes::SClass::acceptRestoreTraverser(const SDistributionRestoreParams& params, core::CStateRestoreTraverser& traverser) {
    do {
        const std::string& name{traverser.name()};
        ...
        RESTORE_SETUP_TEARDOWN(CONDITIONAL_DENSITY_FROM_PRIOR_TAG,
                               ...
                               traverser.traverseSubLevel(boost::bind(
                                   &CNaiveBayesFeatureDensityFromPrior::acceptRestoreTraverser, boost::ref(tmp), boost::cref(params), _1)),
    ...
    for (const auto& density : s_ConditionalDensities) {
        if (dynamic_cast<const CNaiveBayesFeatureDensityFromPrior*>(density.get())) {
            inserter.insertLevel(CONDITIONAL_DENSITY_FROM_PRIOR_TAG,
                                 boost::bind(&CNaiveBayesFeatureDensity::acceptPersistInserter, density.get(), _1));
            continue;
        }
