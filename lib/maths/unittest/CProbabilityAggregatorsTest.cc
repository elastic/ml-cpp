...
void CProbabilityAggregatorsTest::testJointProbabilityOfLessLikelySamples(void) {
...

    {
            ...
            for (size_t j = 0; j < boost::size(percentiles); ++j) {
                for (size_t k = 0; k < boost::size(percentiles); ++k) {
                    ...
                    double probabilities[] = {
                        2.0 * percentiles[i], 2.0 * percentiles[j], 2.0 * percentiles[k]};
        ...
        double totalError =
            ::fabs(totalCount - totalExpectedCount) / std::max(totalCount, totalExpectedCount);
...

    {
        ...
        for (std::size_t i = 0u; i < probabilities.size(); ++i) {
            ...
            CPPUNIT_ASSERT_DOUBLES_EQUAL(
                expectedProbability, probability, 1e-5 * expectedProbability);
        }
    }
}
...
void CProbabilityAggregatorsTest::testLogJointProbabilityOfLessLikelySamples(void) {
...

    {
        ...
        LOG_DEBUG("log(pu) - log(p) = " << upperBound - logP << ", log(p) - log(pl) "
                                        << logP - lowerBound);
...
void CProbabilityAggregatorsTest::testProbabilityOfMFromNExtremeSamples(void) {
...

    {
        ...
        for (std::size_t i = 2; i < 4; ++i) {
            ...
            for (;;) {
                ...
                double expectedProbability =
                    static_cast<double>(count) / static_cast<double>(nTrials);
            ...
            LOG_DEBUG("totalError = " << totalError << ", totalRelativeError = "
                                      << (totalError / totalProbability));
    ...
    {
        double probabilities[] = {
            0.9917012, 0.9917012, 0.9917012, 0.9917012, 0.9917012, 0.9917012, 0.9917012, 0.9917012,
            0.9917012, 0.9917012, 0.9917012, 0.9917012, 0.9917012, 0.9917012, 0.9917012, 0.9917012,
            0.9917012, 0.9917012, 0.9917012, 0.9917012, 0.9917012, 0.9917012, 0.9917012, 0.9917012,
            0.9917012, 0.9917012, 0.9917012, 0.9917012, 0.9917012, 0.9917012, 0.9917012, 0.9917012,
            0.9917012, 0.9917012, 0.9917012, 0.9917012, 0.9917012, 0.9917012, 0.9917012, 0.9917012,
...

    {
        ...
        double pmin[] = {
            0.004703117, 0.05059556, 1.0 - std::numeric_limits<double>::epsilon(), 1.0, 1.0};
    ...
    {
        double probabilities[] = {0.08528782661735056,
                                  0.3246988524001009,
                                  0.5428693993904167,
                                  0.9999999999999999,
                                  0.9999999999999999};
    ...
    {
        double probabilities[] = {3.622684004911715e-76,
                                  3.622684004911715e-76,
                                  0.1534837115755979,
                                  0.1608058997234747,
                                  0.5143979767475618};
