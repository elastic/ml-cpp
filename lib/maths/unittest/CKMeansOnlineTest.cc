...
public:
    ...
    typedef
        typename maths::CKMeansOnline<POINT>::TDoubleMeanVarAccumulator TDoubleMeanVarAccumulator;
    typedef typename maths::CKMeansOnline<POINT>::TFloatMeanAccumulatorDoublePr
        TFloatMeanAccumulatorDoublePr;
    ...
    for (std::size_t t = 1u; t <= 50; ++t) {
        ...
        LOG_DEBUG(
            "expected = " << maths::CBasicStatistics::mean(expected) << ","
                          << maths::CBasicStatistics::maximumLikelihoodVariance(expected).inner(
                                 ones) /
...

    {
        ...
        for (std::size_t t = 1u; t <= 10; ++t) {
            ...
            double costOnline_ = std::numeric_limits<double>::max();
            {
                ...
                maths::CSphericalGaussianInfoCriterion<
                    maths::CKMeansOnline<TVector2>::TSphericalCluster,
                    maths::E_BIC>
                    criterion;
...
void CKMeansOnlineTest::testSample(void) {
    ...
    double covarianceError0 =
        (covariance0 - expectedCovariance0).frobenius() / expectedCovariance0.frobenius();
    ...
    double covarianceError1 =
        (covariance1 - expectedCovariance1).frobenius() / expectedCovariance1.frobenius();
