...
BOOST_AUTO_TEST_CASE(testSymmetricMatrixNxN) {
    // Construction.
    {
        ...
        for (std::size_t i = 0u; i < 3; ++i) {
            for (std::size_t j = 0u; j < 3; ++j) {
                BOOST_CHECK_EQUAL(0.0, matrix(i, j));
            }
        }
        BOOST_CHECK_EQUAL(0.0, matrix.trace());
    }
    {
        ...
        for (std::size_t i = 0u; i < 3; ++i) {
            for (std::size_t j = 0u; j < 3; ++j) {
                BOOST_CHECK_EQUAL(3.0, matrix(i, j));
            }
        }
        BOOST_CHECK_EQUAL(9.0, matrix.trace());
    }
    {
        ...
        for (std::size_t i = 0u; i < 5; ++i) {
            for (std::size_t j = 0u; j < 5; ++j) {
                BOOST_CHECK_EQUAL(m[i][j], matrix(i, j));
            }
        }
        BOOST_CHECK_EQUAL(10.8, matrix.trace());
    }
    {
        ...
        for (std::size_t i = 0u; i < 4; ++i) {
            for (std::size_t j = 0u; j < 4; ++j) {
                BOOST_CHECK_EQUAL(static_cast<double>((i + 1) * (j + 1)), matrix(i, j));
            }
        }
        BOOST_CHECK_EQUAL(30.0, matrix.trace());
    }
    {
        double v[]{1.0, 2.0, 3.0, 4.0};
        maths::CVectorNx1<double, 4> vector(v);
        ...
        for (std::size_t i = 0u; i < 4; ++i) {
            for (std::size_t j = 0u; j < 4; ++j) {
                BOOST_CHECK_EQUAL(i == j ? vector(i) : 0.0, matrix(i, j));
            }
        }
    }
    ...
    // Operators
    {
        ...
        for (std::size_t i = 0u; i < 5; ++i) {
            for (std::size_t j = 0u; j < 5; ++j) {
                BOOST_CHECK_EQUAL(2.0 * m[i][j], sum(i, j));
            }
        }
    }
    {
        ...
        for (std::size_t i = 0u; i < 3; ++i) {
            for (std::size_t j = 0u; j < 3; ++j) {
                BOOST_CHECK_EQUAL(m1[i][j] - m2[i][j], difference(i, j));
            }
        }
    }
    {
...
BOOST_AUTO_TEST_CASE(testVectorNx1) {
    // Construction.
    {
        ...
        for (std::size_t i = 0u; i < 3; ++i) {
            BOOST_CHECK_EQUAL(0.0, vector(i));
        }
    }
    {
        ...
        LOG_DEBUG(<< "vector = " << vector);
        for (std::size_t i = 0u; i < 3; ++i) {
            BOOST_CHECK_EQUAL(3.0, vector(i));
        }
    }
    {
        ...
        LOG_DEBUG(<< "vector = " << vector);
        for (std::size_t i = 0u; i < 5; ++i) {
            BOOST_CHECK_EQUAL(v[i], vector(i));
        }
    }
    ...
    // Operators
    {
        ...
        for (std::size_t i = 0u; i < 5; ++i) {
            BOOST_CHECK_EQUAL(2.0 * v[i], sum(i));
        }
    }
    {
        LOG_DEBUG(<< "Difference");
        ...
        for (std::size_t i = 0u; i < 3; ++i) {
            BOOST_CHECK_EQUAL(v1[i] - v2[i], difference(i));
        }
    }
    {
        ...
        for (std::size_t t = 0u; t < 20; ++t) {
            ...
            LOG_DEBUG(<< "Sy = " << sy);
            for (std::size_t i = 0u; i < 4; ++i) {
                BOOST_CHECK_EQUAL(expected(i), sy(i));
            }
        }
    }
    {
        ...
        LOG_DEBUG(<< "3 * v = " << vs);
        for (std::size_t i = 0u; i < 5; ++i) {
            BOOST_CHECK_EQUAL(3.0 * static_cast<double>((i + 1)), vs(i));
        }
    }
    {
        LOG_DEBUG(<< "Matrix Scalar Division");
        ...
        LOG_DEBUG(<< "v / 4.0 = " << vs);
        for (std::size_t i = 0u; i < 5; ++i) {
            BOOST_CHECK_EQUAL(static_cast<double>((i + 1)) / 4.0, vs(i));
        }
    }
}
...
BOOST_AUTO_TEST_CASE(testSymmetricMatrix) {
    // Construction.
    {
        ...
        LOG_DEBUG(<< "matrix = " << matrix);
        BOOST_CHECK_EQUAL(std::size_t(3), matrix.rows());
        BOOST_CHECK_EQUAL(std::size_t(3), matrix.columns());
        for (std::size_t i = 0u; i < 3; ++i) {
            for (std::size_t j = 0u; j < 3; ++j) {
                BOOST_CHECK_EQUAL(0.0, matrix(i, j));
            }
    ...
    }
    {
        ...
        LOG_DEBUG(<< "matrix = " << matrix);
        BOOST_CHECK_EQUAL(std::size_t(4), matrix.rows());
        BOOST_CHECK_EQUAL(std::size_t(4), matrix.columns());
        for (std::size_t i = 0u; i < 4; ++i) {
            for (std::size_t j = 0u; j < 4; ++j) {
                BOOST_CHECK_EQUAL(3.0, matrix(i, j));
            }
    ...
    }
    {
        ...
        LOG_DEBUG(<< "matrix = " << matrix);
        BOOST_CHECK_EQUAL(std::size_t(5), matrix.rows());
        BOOST_CHECK_EQUAL(std::size_t(5), matrix.columns());
        for (std::size_t i = 0u; i < 5; ++i) {
            for (std::size_t j = 0u; j < 5; ++j) {
                BOOST_CHECK_EQUAL(m[i][j], matrix(i, j));
            }
    ...
    }
    {
        ...
        for (std::size_t i = 0u, k = 0u; i < 5; ++i) {
            for (std::size_t j = 0u; j <= i; ++j, ++k) {
                BOOST_CHECK_EQUAL(m[k], matrix(i, j));
                BOOST_CHECK_EQUAL(m[k], matrix(j, i));
            }
        }
        BOOST_CHECK_EQUAL(10.8, matrix.trace());
    }
    ...
    // Operators
    {
        ...
        for (std::size_t i = 0u, k = 0u; i < 5; ++i) {
            for (std::size_t j = 0u; j <= i; ++j, ++k) {
                BOOST_CHECK_EQUAL(2.0 * m[k], sum(i, j));
                BOOST_CHECK_EQUAL(2.0 * m[k], sum(j, i));
            }
        }
    }
    {
        ...
        for (std::size_t i = 0u, k = 0u; i < 3; ++i) {
            for (std::size_t j = 0u; j <= i; ++j, ++k) {
                BOOST_CHECK_EQUAL(m1[k] - m2[k], difference(i, j));
                BOOST_CHECK_EQUAL(m1[k] - m2[k], difference(j, i));
            }
        }
    }
    {
...
BOOST_AUTO_TEST_CASE(testVector) {
    // Construction.
    {
        ...
        LOG_DEBUG(<< "vector = " << vector);
        BOOST_CHECK_EQUAL(std::size_t(3), vector.dimension());
        for (std::size_t i = 0u; i < 3; ++i) {
            BOOST_CHECK_EQUAL(0.0, vector(i));
        }
    }
    {
        ...
        LOG_DEBUG(<< "vector = " << vector);
        BOOST_CHECK_EQUAL(std::size_t(4), vector.dimension());
        for (std::size_t i = 0u; i < 4; ++i) {
            BOOST_CHECK_EQUAL(3.0, vector(i));
        }
    }
    {
        ...
        BOOST_CHECK_EQUAL(std::size_t(5), vector.dimension());
        LOG_DEBUG(<< "vector = " << vector);
        for (std::size_t i = 0u; i < 5; ++i) {
            BOOST_CHECK_EQUAL(v[i], vector(i));
        }
    ...
    }
    {
        ...
        for (std::size_t t = 0u; t < 20; ++t) {
            ...
            LOG_DEBUG(<< "Sy = " << sy);
            for (std::size_t i = 0u; i < 4; ++i) {
                BOOST_CHECK_CLOSE_ABSOLUTE(expected(i), sy(i), 1e-10);
            }
        }
    }
    {
...
BOOST_AUTO_TEST_CASE(testNorms) {
    ...
    for (std::size_t i = 0u; i < boost::size(v); ++i) {
        ...
        BOOST_CHECK_CLOSE_ABSOLUTE(expectedEuclidean[i], v_.euclidean(), 5e-6);
    }
    ...
    for (std::size_t i = 0u; i < boost::size(m); ++i) {
        ...
        BOOST_CHECK_CLOSE_ABSOLUTE(expectedFrobenius[i], m_.frobenius(), 5e-6);
    }
}
...
BOOST_AUTO_TEST_CASE(testUtils) {
    // Test component min, max, sqrt and fabs.
    {
...

        {
            ...
            for (std::size_t i = 0u; i < 5; ++i) {
                BOOST_CHECK_EQUAL(expected[i], (maths::min(v1, 4.0))(i));
            }
        }
        for (std::size_t i = 0u; i < 5; ++i) {
            BOOST_CHECK_EQUAL((maths::min(v1, 4.0))(i), (maths::min(4.0, v1))(i));
        }
        {
            double expected[]{1.0, 3.0, 2.2, 4.9, 8.0};
            LOG_DEBUG(<< "min(v1, v2) = " << maths::min(v1, v2));
            for (std::size_t i = 0u; i < 5; ++i) {
                BOOST_CHECK_EQUAL(expected[i], (maths::min(v1, v2))(i));
            }
        }

        {
            ...
            for (std::size_t i = 0u; i < 5; ++i) {
                BOOST_CHECK_EQUAL(expected[i], (maths::max(v1, 3.0))(i));
            }
        }
        for (std::size_t i = 0u; i < 5; ++i) {
            BOOST_CHECK_EQUAL((maths::max(v1, 3.0))(i), (maths::max(3.0, v1))(i));
        }
        {
            double expected[]{1.5, 3.1, 2.7, 5.2, 12.0};
            LOG_DEBUG(<< "max(v1, v2) = " << maths::max(v1, v2));
            for (std::size_t i = 0u; i < 5; ++i) {
                BOOST_CHECK_EQUAL(expected[i], (maths::max(v1, v2))(i));
            }
        }

        {
            ...
            for (std::size_t i = 0u; i < 5; ++i) {
                BOOST_CHECK_EQUAL(expected[i], (maths::sqrt(v1))(i));
            }
        }

        {
            ...
            for (std::size_t i = 0u; i < 5; ++i) {
                BOOST_CHECK_EQUAL(expected[i], (maths::fabs(v3))(i));
            }
        }
    }
    {
...

        {
            ...
            for (std::size_t i = 0u; i < 3; ++i) {
                for (std::size_t j = 0u; j < 3; ++j) {
                    BOOST_CHECK_EQUAL(expected[i][j], (maths::min(m1, 3.0))(i, j));
                }
            }
        }
        for (std::size_t i = 0u; i < 3; ++i) {
        ...
        }
        {
            ...
            for (std::size_t i = 0u; i < 3; ++i) {
                for (std::size_t j = 0u; j < 3; ++j) {
                    BOOST_CHECK_EQUAL(expected[i][j], (maths::min(m1, m2))(i, j));
                }
            }
        }

        {
            ...
            for (std::size_t i = 0u; i < 3; ++i) {
                for (std::size_t j = 0u; j < 3; ++j) {
                    BOOST_CHECK_EQUAL(expected[i][j], (maths::max(m1, 2.0))(i, j));
                }
            }
        }
        for (std::size_t i = 0u; i < 3; ++i) {
        ...
        }
        {
            ...
            for (std::size_t i = 0u; i < 3; ++i) {
                for (std::size_t j = 0u; j < 3; ++j) {
                    BOOST_CHECK_EQUAL(expected[i][j], (maths::max(m1, m2))(i, j));
                }
            }
        }

        {
            ...
            for (std::size_t i = 0u; i < 3; ++i) {
                for (std::size_t j = 0u; j < 3; ++j) {
                    BOOST_CHECK_EQUAL(expected[i][j], (maths::sqrt(m1))(i, j));
                }
            }
        }

        {
            ...
            for (std::size_t i = 0u; i < 3; ++i) {
                for (std::size_t j = 0u; j < 3; ++j) {
                    BOOST_CHECK_EQUAL(expected[i][j], (maths::fabs(m3))(i, j));
                }
            }
        }
    }
    ...
    LOG_DEBUG(<< "CVectorNx1");
    {
        ...
        for (std::size_t i = 0u; i < 4; ++i) {
            ...
            for (std::size_t j = i + 1; j < 4; ++j) {
                ...
                BOOST_CHECK_CLOSE_ABSOLUTE(expected, covariance(j, i), 1e-10);
            }
        }
    }
...
BOOST_AUTO_TEST_CASE(testSampleGaussian) {
    // Test singular matrix.
    {
        ...
        BOOST_CHECK_EQUAL(std::size_t(99), samples.size());
        ...
        BOOST_CHECK_CLOSE_ABSOLUTE(0.0, meanError.euclidean() / mean.euclidean(), 1e-10);
        BOOST_CHECK_CLOSE_ABSOLUTE(
    ...
    // Construct a matrix whose eigenvalues and vectors are known.
    {
        ...
        BOOST_CHECK_EQUAL(std::size_t(100), samples.size());
...
BOOST_AUTO_TEST_CASE(testLogDeterminant) {
    ...
    // Construct a matrix whose eigenvalues and vectors are known.
    {
        ...
        double logDeterminant;
        maths::logDeterminant(M, logDeterminant);
        BOOST_CHECK_CLOSE_ABSOLUTE(std::log(10.0 * 5.0 * 5.0 * 2.0), logDeterminant, 1e-10);
    }
}
...
BOOST_AUTO_TEST_CASE(testProjected) {
...

    {
        ...
        BOOST_CHECK_EQUAL(std::string("1.2 2.4\n2.4   1"), print(projectedMatrix));
        BOOST_CHECK_EQUAL(std::string("0.3\n3.4"), print(projectedVector));
    }
    {
        std::size_t ss[]{1, 0};
        TSizeVec subspace(std::begin(ss), std::end(ss));
        ...
        LOG_DEBUG(<< "projectedMatrix =\n" << projectedMatrix);
        LOG_DEBUG(<< "projectedVector =\n" << projectedVector);
        BOOST_CHECK_EQUAL(std::string("  1 2.4\n2.4 1.2"), print(projectedMatrix));
...
BOOST_AUTO_TEST_CASE(testShims) {
    ...
    {
        BOOST_CHECK_EQUAL(std::size_t(4), maths::las::dimension(vector1));
        BOOST_CHECK_EQUAL(std::size_t(4), maths::las::dimension(vector2));
        BOOST_CHECK_EQUAL(std::size_t(4), maths::las::dimension(vector3));
        BOOST_CHECK_EQUAL(std::size_t(4), maths::las::dimension(vector4));
    }
    LOG_DEBUG(<< "Test zero");
    {
        BOOST_TEST(TVector4(0.0) == maths::las::zero(vector1));
        BOOST_TEST(TVector(4, 0.0) == maths::las::zero(vector2));
        BOOST_TEST(TDenseVector::Zero(4) == maths::las::zero(vector3));
        BOOST_TEST(TDenseVector::Zero(4) == maths::las::zero(vector4));
    }
    LOG_DEBUG(<< "Test conformableZeroMatrix");
    {
        BOOST_TEST(TMatrix4(0.0) == maths::las::conformableZeroMatrix(vector1));
        BOOST_TEST(TMatrix(4, 0.0) == maths::las::conformableZeroMatrix(vector2));
        BOOST_TEST(TDenseMatrix::Zero(4, 4) == maths::las::conformableZeroMatrix(vector3));
        BOOST_TEST(TDenseMatrix::Zero(4, 4) == maths::las::conformableZeroMatrix(vector4));
    }
    LOG_DEBUG(<< "Test isZero");
    {
        BOOST_TEST(maths::las::isZero(vector1) == false);
        BOOST_TEST(maths::las::isZero(vector2) == false);
        BOOST_TEST(maths::las::isZero(vector3) == false);
        BOOST_TEST(maths::las::isZero(vector4) == false);
        BOOST_TEST(maths::las::isZero(maths::las::zero(vector1)));
        BOOST_TEST(maths::las::isZero(maths::las::zero(vector2)));
        BOOST_TEST(maths::las::isZero(maths::las::zero(vector3)));
        BOOST_TEST(maths::las::isZero(maths::las::zero(vector4)));
    }
    LOG_DEBUG(<< "Test ones");
    {
        BOOST_TEST(TVector4(1.0) == maths::las::ones(vector1));
        BOOST_TEST(TVector(4, 1.0) == maths::las::ones(vector2));
        BOOST_TEST(TDenseVector::Ones(4) == maths::las::ones(vector3));
        BOOST_TEST(TDenseVector::Ones(4) == maths::las::ones(vector4));
    }
    LOG_DEBUG(<< "Test constant");
    {
        BOOST_TEST(TVector4(5.1) == maths::las::constant(vector1, 5.1));
        BOOST_TEST(TVector(4, 5.1) == maths::las::constant(vector2, 5.1));
        BOOST_TEST(5.1 * TDenseVector::Ones(4) == maths::las::constant(vector3, 5.1));
        BOOST_TEST(5.1 * TDenseVector::Ones(4) == maths::las::constant(vector4, 5.1));
    }
    LOG_DEBUG(<< "Test min");
    {
    ...
    LOG_DEBUG(<< "Test distance");
    {
        ...
        LOG_DEBUG(<< " d1 = " << d3);
        LOG_DEBUG(<< " d1 = " << d4);
        BOOST_CHECK_CLOSE_ABSOLUTE(5.29528091794949, d1, 1e-10);
        BOOST_CHECK_CLOSE_ABSOLUTE(5.29528091794949, d2, 1e-10);
        BOOST_CHECK_CLOSE_ABSOLUTE(5.29528091794949, d3, 1e-10);
        BOOST_CHECK_CLOSE_ABSOLUTE(5.29528091794949, d4, 1e-10);
    }
    LOG_DEBUG(<< "Test Euclidean norm");
    {
        ...
        LOG_DEBUG(<< " n1 = " << n3);
        LOG_DEBUG(<< " n1 = " << n4);
        BOOST_CHECK_CLOSE_ABSOLUTE(5.37215040742532, n1, 1e-10);
        BOOST_CHECK_CLOSE_ABSOLUTE(5.37215040742532, n2, 1e-10);
        BOOST_CHECK_CLOSE_ABSOLUTE(5.37215040742532, n3, 1e-10);
        BOOST_CHECK_CLOSE_ABSOLUTE(5.37215040742532, n4, 1e-10);
    }
    LOG_DEBUG(<< "Test L1");
    {
        ...
        LOG_DEBUG(<< " l1 = " << l3);
        LOG_DEBUG(<< " l1 = " << l4);
        BOOST_CHECK_CLOSE_ABSOLUTE(11.8, l1, 1e-10);
        BOOST_CHECK_CLOSE_ABSOLUTE(11.8, l2, 1e-10);
        BOOST_CHECK_CLOSE_ABSOLUTE(11.8, l3, 1e-10);
        BOOST_CHECK_CLOSE_ABSOLUTE(11.8, l4, 1e-10);
    }
    LOG_DEBUG(<< "Test Frobenius");
    {
        ...
        LOG_DEBUG(<< " f1 = " << f3);
        LOG_DEBUG(<< " f1 = " << f4);
        BOOST_CHECK_CLOSE_ABSOLUTE(7.92906047397799, f1, 1e-10);
        BOOST_CHECK_CLOSE_ABSOLUTE(7.92906047397799, f2, 1e-10);
        BOOST_CHECK_CLOSE_ABSOLUTE(7.92906047397799, f3, 1e-10);
        BOOST_CHECK_CLOSE_ABSOLUTE(7.92906047397799, f4, 1e-10);
    }
    LOG_DEBUG(<< "Test inner");
    {
        ...
        LOG_DEBUG(<< "i1 = " << i3);
        LOG_DEBUG(<< "i1 = " << i4);
        BOOST_CHECK_CLOSE_ABSOLUTE(21.77, i1, 1e-10);
        BOOST_CHECK_CLOSE_ABSOLUTE(21.77, i2, 1e-10);
        BOOST_CHECK_CLOSE_ABSOLUTE(21.77, i3, 1e-10);
        BOOST_CHECK_CLOSE_ABSOLUTE(21.77, i4, 1e-10);
    }
    LOG_DEBUG(<< "Test outer");
    {
        ...
        for (std::size_t i = 0u; i < 4; ++i) {
            for (std::size_t j = 0u; j < 4; ++j) {
                BOOST_CHECK_EQUAL(vector1(i) * vector1(j), outer1(i, j));
                BOOST_CHECK_EQUAL(vector2(i) * vector2(j), outer2(i, j));
                BOOST_CHECK_EQUAL(vector3(i) * vector3(j), outer3(i, j));
                BOOST_CHECK_EQUAL(vector4(i) * vector4(j), outer4(i, j));
            }
        }
    }
}
...
BOOST_AUTO_TEST_CASE(testPersist) {
...

    {
        ...
        BOOST_TEST(restoredMatrix.fromDelimited(str));

        LOG_DEBUG(<< "delimited = " << str);

        for (std::size_t i = 0u; i < 4; ++i) {
            for (std::size_t j = 0u; j < 4; ++j) {
                BOOST_CHECK_EQUAL(matrix(i, j), restoredMatrix(i, j));
            }
        }

        BOOST_TEST(!restoredMatrix.fromDelimited(std::string()));

        std::string bad("0.1,0.3,0.5,3");
        BOOST_TEST(!restoredMatrix.fromDelimited(bad));
    ...
    }
    {
        ...
        BOOST_TEST(restoredVector.fromDelimited(str));

        LOG_DEBUG(<< "delimited = " << str);

        for (std::size_t i = 0u; i < 3; ++i) {
            BOOST_CHECK_EQUAL(vector(i), restoredVector(i));
        }

        BOOST_TEST(!restoredVector.fromDelimited(std::string()));

        std::string bad("0.1,0.3,0.5,3");
        BOOST_TEST(!restoredVector.fromDelimited(bad));
        bad = "0.1,0.3,a";
        BOOST_TEST(!restoredVector.fromDelimited(bad));
    }
    {
