...
public:
    ...
    template <typename POINT> void add(const std::vector<POINT> &points) {
...
void CXMeansTest::testImproveStructure(void) {
    ...
    for (std::size_t t = 0u; t < 10; ++t) {
        ...
        TUInt64Vec inactive;
        std::set_intersection(oldChecksums.begin(),
                              oldChecksums.end(),
                              newChecksums.begin(),
                              newChecksums.end(),
    ...
    for (std::size_t t = 0; t < 50; ++t) {
        ...
        for (std::size_t i = 0u; i < xmeans.clusters().size(); ++i) {
            ...
            for (std::size_t j = 0u; j < clusterPoints.size(); ++j) {
                ...
                std::size_t k = 0u;
                for (/**/; k < points.size(); ++k) {
                    for (TVector2VecCItr itr = std::lower_bound(
                                             points[k].begin(), points[k].end(), clusterPoints[j]),
                                         end = std::upper_bound(
                                             points[k].begin(), points[k].end(), clusterPoints[j]);
                         itr != end;
                         ++itr) {
...
void CXMeansTest::testPoorlyConditioned(void) {
    ...
    double points_[][2] = {
        {0.0, 0.0},    {1.0, 0.5},    {2.0, 1.0},    {3.0, 1.5},    {4.0, 2.0},    {5.0, 2.5},
