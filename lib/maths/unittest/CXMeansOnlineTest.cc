...
public:
    ...
    const TClusterVec &clusters(void) const { return this->maths::CXMeansOnline<T, N>::clusters(); }
};
...
void CXMeansOnlineTest::testCluster(void) {
    ...
    TXMeans2::TOptionalClusterClusterPr split = cluster.split(rng, 5.0, clusterer.indexGenerator());
...
void CXMeansOnlineTest::testClusteringWithOutliers(void) {
    ...
    for (std::size_t i = 0u; i < boost::size(outliers_); ++i) {
        outliers.push_back(TDoubleVec(boost::begin(outliers_[i]), boost::end(outliers_[i])));
    }
    ...
    for (std::size_t t = 0u; t < 10; ++t) {
        ...
        TXMeans2ForTest clusterer(
            maths_t::E_ContinuousData, maths_t::E_ClustersFractionWeight, 0.0, 0.01);
...
void CXMeansOnlineTest::testLatLongData(void) {
    ...
    TXMeans2FloatForTest clusterer(
        maths_t::E_ContinuousData, maths_t::E_ClustersFractionWeight, 0.0005);
