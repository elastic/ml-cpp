...
double sum(const TDoubleVec &x) { return std::accumulate(x.begin(), x.end(), 0.0); }
...
void CMultivariateOneOfNPriorTest::testPropagation(void) {
    ...
    for (std::size_t i = 0u; i < samples.size(); ++i) {
        filter.addSamples(
            COUNT_WEIGHT, TDouble10Vec1Vec(1, samples[i]), TDouble10Vec4Vec1Vec(1, UNIT_WEIGHT_2));
    }
...
void CMultivariateOneOfNPriorTest::testModelUpdate(void) {
    ...
    for (std::size_t i = 0u; i < boost::size(dataTypes); ++i) {
        ...
        normal.addSamples(
            COUNT_WEIGHT, samples, TDouble10Vec4Vec1Vec(samples.size(), UNIT_WEIGHT_2));
        multimodal.addSamples(
            COUNT_WEIGHT, samples, TDouble10Vec4Vec1Vec(samples.size(), UNIT_WEIGHT_2));
        oneOfN.addSamples(
            COUNT_WEIGHT, samples, TDouble10Vec4Vec1Vec(samples.size(), UNIT_WEIGHT_2));
...
void CMultivariateOneOfNPriorTest::testMarginalLikelihood(void) {
    ...
    test::CRandomNumbers rng;
    {
        ...
        for (std::size_t t = 0u; t < nt; ++t) {
            ...
            for (std::size_t i = 0u; i < samples.size(); ++i) {
                filter.addSamples(
                    COUNT_WEIGHT, TDouble10Vec1Vec(1, samples[i]), SINGLE_UNIT_WEIGHT_2);
...
void CMultivariateOneOfNPriorTest::testSampleMarginalLikelihood(void) {
    ...
    for (std::size_t i = 0u; i < samples.size(); ++i) {
        ...
        if (!filter.isNonInformative()) {
            ...
            expectedSampled.insert(
                expectedSampled.end(), multimodalSamples.begin(), multimodalSamples.end());
