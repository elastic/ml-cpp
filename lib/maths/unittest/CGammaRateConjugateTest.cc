...
void CGammaRateConjugateTest::testShapeEstimation(void) {
    ...
    for (size_t i = 0; i < boost::size(decayRates); ++i) {
        ...
        for (unsigned int test = 0u; test < 100u; ++test) {
            ...
            TGammaRateConjugateVec filters(nAggregate,
                                           makePrior(maths_t::E_ContinuousData,
                                                     0.0,
                                                     decayRates[i]));
...
void CGammaRateConjugateTest::testMarginalLikelihood(void) {
    ...
    for (std::size_t t = 0u; t < boost::size(dataTypes); ++t) {
        ...
        maths_t::ESampleWeightStyle weightStyles[] = {maths_t::E_SampleCountWeight,
    ...
    }
    {
        ...
        for (std::size_t i = 0u; i < boost::size(varianceScales); ++i) {
            ...
            for (std::size_t j = 0u; j < boost::size(percentages); ++j) {
                ...
                double q1 =
                    boost::math::quantile(scaledGamma, (50.0 - percentages[j] / 2.0) / 100.0);
                double q2 =
                    boost::math::quantile(scaledGamma, (50.0 + percentages[j] / 2.0) / 100.0);
    ...
    for (std::size_t i = 0u; i < boost::size(shapes); ++i) {
        for (std::size_t j = 0u; j < boost::size(scales); ++j) {
            ...
            for (std::size_t k = 0u; k < boost::size(varianceScales); ++k) {
                ...
                double error =
                    ::fabs(filter.marginalLikelihoodMode(weightStyle, weight) - expectedMode);
...
void CGammaRateConjugateTest::testSampleMarginalLikelihood(void) {
    ...
    for (std::size_t i = 0u; i < 3; ++i) {
        ...
        CPPUNIT_ASSERT_DOUBLES_EQUAL(maths::CBasicStatistics::mean(sampleMeanVar),
                                     maths::CBasicStatistics::mean(sampledMeanVar),
                                     eps);
        CPPUNIT_ASSERT_DOUBLES_EQUAL(maths::CBasicStatistics::variance(sampleMeanVar),
                                     maths::CBasicStatistics::variance(sampledMeanVar),
                                     eps);
    }
...
void CGammaRateConjugateTest::testCdf(void) {
    ...
    for (std::size_t i = 1u; i < 500; ++i) {
        ...
        CPPUNIT_ASSERT(
            filter.minusLogJointCdfComplement(TDouble1Vec(1, x), lowerBound, upperBound));
        ...
        for (size_t j = 0; j < boost::size(scales); ++j) {
            ...
            for (std::size_t k = 1u; k < 10; ++k) {
                ...
                double px = static_cast<double>(
                                std::lower_bound(likelihoods.begin(), likelihoods.end(), fx) -
                                likelihoods.begin()) /
                ...
                LOG_DEBUG("expected P(x) = " << px << ", actual P(x) = " << (lb + ub) / 2.0
                                             << " sample sd = " << ssd);
    ...
    for (size_t i = 0; i < boost::size(shapes); ++i) {
        for (size_t j = 0; j < boost::size(scales); ++j) {
            ...
            for (std::size_t k = 0u; k < nSamples; ++k) {
                ...
                double l1, u1;
                CPPUNIT_ASSERT(
                    filter1.probabilityOfLessLikelySamples(maths_t::E_TwoSided, sample, l1, u1));
                ...
                double l2, u2;
                CPPUNIT_ASSERT(
                    filter2.probabilityOfLessLikelySamples(maths_t::E_TwoSided, sample, l2, u2));
            ...
            LOG_DEBUG("shape = " << shapes[i] << ", rate = " << scales[j] << ", p1 = " << p1
                                 << ", p2 = " << p2);
...
void CGammaRateConjugateTest::testVarianceScale(void) {
...

        {
            ...
            for (size_t i = 0; i < boost::size(varianceScales); ++i) {
                ...
                for (size_t j = 0; j < boost::size(percentiles); ++j) {
                    ...
                    LOG_DEBUG("percentile = " << percentiles[j] << ", probability = "
    ...
    for (std::size_t s = 0u; s < boost::size(scales); ++s) {
        for (std::size_t t = 0u; t < boost::size(dataTypes); ++t) {
                    ...
                    for (std::size_t k = 0u; k < boost::size(varianceScales); ++k) {
                        ...
                        for (unsigned int test = 0u; test < 5; ++test) {
                            ...
                            double estimatedMean =
                                filter.likelihoodShape() / filter.likelihoodRate();
                            ...
                            double trialMeanError =
                                ::fabs(estimatedMean - (mean + dm)) / std::max(1.0, mean + dm);
                            double trialVarianceError =
                                ::fabs(estimatedVariance - (variance + dv)) /
                        ...
                        LOG_DEBUG("mean error = " << maths::CBasicStatistics::mean(meanError));
                        LOG_DEBUG(
                            "variance error = " << maths::CBasicStatistics::mean(varianceError));

                        CPPUNIT_ASSERT(maths::CBasicStatistics::mean(meanError) <
                                       maximumMeanError[t]);
                        CPPUNIT_ASSERT(maths::CBasicStatistics::mean(varianceError) <
            ...
            CPPUNIT_ASSERT(maths::CBasicStatistics::mean(meanVarianceError) <
...
CppUnit::Test* CGammaRateConjugateTest::suite(void) {
    ...
    suiteOfTests->addTest(new CppUnit::TestCaller<
                          CGammaRateConjugateTest>("CGammaRateConjugateTest::testMultipleUpdate",
                                                   &CGammaRateConjugateTest::testMultipleUpdate));
    suiteOfTests->addTest(new CppUnit::TestCaller<
                          CGammaRateConjugateTest>("CGammaRateConjugateTest::testPropagation",
                                                   &CGammaRateConjugateTest::testPropagation));
    suiteOfTests->addTest(new CppUnit::TestCaller<
                          CGammaRateConjugateTest>("CGammaRateConjugateTest::testShapeEstimation",
                                                   &CGammaRateConjugateTest::testShapeEstimation));
    suiteOfTests->addTest(new CppUnit::TestCaller<
                          CGammaRateConjugateTest>("CGammaRateConjugateTest::testRateEstimation",
                                                   &CGammaRateConjugateTest::testRateEstimation));
    suiteOfTests->addTest(
        new CppUnit::TestCaller<
            CGammaRateConjugateTest>("CGammaRateConjugateTest::testMarginalLikelihood",
                                     &CGammaRateConjugateTest::testMarginalLikelihood));
    suiteOfTests->addTest(
        new CppUnit::TestCaller<
            CGammaRateConjugateTest>("CGammaRateConjugateTest::testMarginalLikelihoodMean",
                                     &CGammaRateConjugateTest::testMarginalLikelihoodMean));
    suiteOfTests->addTest(
        new CppUnit::TestCaller<
            CGammaRateConjugateTest>("CGammaRateConjugateTest::testMarginalLikelihoodMode",
                                     &CGammaRateConjugateTest::testMarginalLikelihoodMode));
    suiteOfTests->addTest(
        new CppUnit::TestCaller<
            CGammaRateConjugateTest>("CGammaRateConjugateTest::testMarginalLikelihoodVariance",
                                     &CGammaRateConjugateTest::testMarginalLikelihoodVariance));
    suiteOfTests->addTest(
        new CppUnit::TestCaller<
            CGammaRateConjugateTest>("CGammaRateConjugateTest::testSampleMarginalLikelihood",
                                     &CGammaRateConjugateTest::testSampleMarginalLikelihood));
    suiteOfTests->addTest(
        new CppUnit::TestCaller<CGammaRateConjugateTest>("CGammaRateConjugateTest::testCdf",
                                                         &CGammaRateConjugateTest::testCdf));
    suiteOfTests->addTest(
        new CppUnit::TestCaller<
            CGammaRateConjugateTest>("CGammaRateConjugateTest::testProbabilityOfLessLikelySamples",
                                     &CGammaRateConjugateTest::testProbabilityOfLessLikelySamples));
    suiteOfTests->addTest(new CppUnit::TestCaller<
                          CGammaRateConjugateTest>("CGammaRateConjugateTest::testAnomalyScore",
                                                   &CGammaRateConjugateTest::testAnomalyScore));
    suiteOfTests->addTest(
        new CppUnit::TestCaller<CGammaRateConjugateTest>("CGammaRateConjugateTest::testOffset",
                                                         &CGammaRateConjugateTest::testOffset));
    suiteOfTests->addTest(new CppUnit::TestCaller<
                          CGammaRateConjugateTest>("CGammaRateConjugateTest::testIntegerData",
                                                   &CGammaRateConjugateTest::testIntegerData));
    suiteOfTests->addTest(new CppUnit::TestCaller<
                          CGammaRateConjugateTest>("CGammaRateConjugateTest::testLowVariationData",
                                                   &CGammaRateConjugateTest::testLowVariationData));
    suiteOfTests->addTest(
        new CppUnit::TestCaller<CGammaRateConjugateTest>("CGammaRateConjugateTest::testPersist",
                                                         &CGammaRateConjugateTest::testPersist));
    suiteOfTests->addTest(new CppUnit::TestCaller<
                          CGammaRateConjugateTest>("CGammaRateConjugateTest::testVarianceScale",
                                                   &CGammaRateConjugateTest::testVarianceScale));
    suiteOfTests->addTest(new CppUnit::TestCaller<
                          CGammaRateConjugateTest>("CGammaRateConjugateTest::testNegativeSample",
