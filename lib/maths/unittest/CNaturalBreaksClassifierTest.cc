    ...
    for (;;) {
        ...
        for (std::size_t i = 0u, j = 0u; i < split.size(); ++i) {
            ...
            if (!computeDeviation(category, p, categoryDeviation) ||
                ...
                deviation = INF;
                LOG_TRACE("Pruning solutions variation = "
                          << deviation << ", deviationMin = " << deviationMin
                ...
                for (std::size_t j = 3u; j < 7; ++j) {
                    ...
                    do {
                        ...
                        if (expectSplit && haveSplit) {
                            LOG_DEBUG(
                                "expected = " << core::CContainerPrinter::print(expectedSplit));
                            LOG_DEBUG("actual =   " << core::CContainerPrinter::print(split));
    ...
    }
    {
...

        {
            ...
            }
            {
                ...
                CPPUNIT_ASSERT(
                    ::fabs(CBasicStatistics::count(twoSplit[0]) - static_cast<double>(n1)) /
                ...
                CPPUNIT_ASSERT(
                    ::fabs(CBasicStatistics::count(twoSplit[1]) - static_cast<double>(n2)) /
