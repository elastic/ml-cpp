...
void CMultimodalPriorTest::testSampleMarginalLikelihood(void) {
    ...
    typedef maths::CBasicStatistics::SSampleMeanVarSkew<double>::TAccumulator
...
void CMultimodalPriorTest::testProbabilityOfLessLikelySamples(void) {
    ...
    }
    {
        ...
        for (std::size_t i = 0u; i < boost::size(weights); ++i) {
            ...
            rng.generateLogNormalSamples(locations[i],
                                         squareScales[i],
        ...
        TLogNormalVec modes;
        modes.push_back(
            boost::math::lognormal_distribution<>(locations[0], ::sqrt(squareScales[0])));
        modes.push_back(
            boost::math::lognormal_distribution<>(locations[1], ::sqrt(squareScales[1])));
        modes.push_back(
            boost::math::lognormal_distribution<>(locations[2], ::sqrt(squareScales[2])));
        maths::CMixtureDistribution<boost::math::lognormal_distribution<>> mixture(mixtureWeights,
                                                                                   modes);
    ...
    }
    {
        ...
        maths::CMixtureDistribution<boost::math::gamma_distribution<>> mixture(mixtureWeights,
                                                                               modes);
