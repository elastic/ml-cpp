...
void CMultivariateNormalConjugateTest::testMultipleUpdate(void) {
    ...
    for (size_t i = 0; i < boost::size(dataTypes); ++i) {
        ...
        for (std::size_t j = 0u; j < count; ++j) {
            filter1.addSamples(
                COUNT_WEIGHT, TDouble10Vec1Vec(1, TDouble10Vec(2, x)), SINGLE_UNIT_WEIGHT_2);
        }
...
void CMultivariateNormalConjugateTest::testMeanVectorEstimation(void) {
    ...
    for (std::size_t i = 0; i < boost::size(decayRates); ++i) {
        ...
        for (unsigned int t = 0; t < nt; ++t) {
            ...
            for (std::size_t j = 0u; j < samples.size(); ++j) {
                filter.addSamples(
                    COUNT_WEIGHT, TDouble10Vec1Vec(1, samples[j]), SINGLE_UNIT_WEIGHT_2);
...
void CMultivariateNormalConjugateTest::testMarginalLikelihoodMode(void) {
    ...
    for (std::size_t i = 0u; i < epsilons.size(); i += 2) {
        ...
        filter.jointLogMarginalLikelihood(
            COUNT_WEIGHT, TDouble10Vec1Vec(1, mode), SINGLE_UNIT_WEIGHT_2, ll);
    ...
    for (std::size_t i = 0u; i < boost::size(means); ++i) {
        ...
        for (std::size_t j = 0u; j < boost::size(covariances); ++j) {
            ...
            for (std::size_t k = 0u; k < samples.size(); ++k) {
                filter.addSamples(
                    COUNT_WEIGHT, TDouble10Vec1Vec(1, samples[k]), SINGLE_UNIT_WEIGHT_2);
            }
...
void CMultivariateNormalConjugateTest::testLowVariationData(void) {
...

    {
        ...
        for (std::size_t i = 0u; i < 100; ++i) {
            filter.addSamples(
                COUNT_WEIGHT, TDouble10Vec1Vec(1, TDouble10Vec(2, 430.0)), SINGLE_UNIT_WEIGHT_2);
        }
