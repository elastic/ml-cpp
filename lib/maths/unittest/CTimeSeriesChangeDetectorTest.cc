...
TPriorPtr makeResidualModel()
{
    maths::CGammaRateConjugate gamma{
            maths::CGammaRateConjugate::nonInformativePrior(maths_t::E_ContinuousData, 0.1, DECAY_RATE)};
...
void CTimeSeriesChangeDetectorTest::testNoChange()
{
    ...
    for (std::size_t t = 0u; t < 100; ++t)
    {
        ...
        switch (t % 3)
        {
        case 0: rng.generateNormalSamples(10.0, variances[(t/3) % variances.size()], 1000, samples); break;
        case 1: rng.generateLogNormalSamples(1.0, scales[(t/3) % scales.size()], 1000, samples);     break;
...
void CTimeSeriesChangeDetectorTest::testPersist()
{
    ...
    for (std::size_t i = 990u; i < samples.size(); ++i)
    {
        ...
        LOG_DEBUG("expected " << origDetector.checksum()
                  << " got " << restoredDetector.checksum());
    ...
    for (std::size_t t = 0u; t < 100; ++t)
    {
        ...
        for (std::size_t i = 950u; i < samples.size(); ++i)
        {
            ...
            if (change)
            {
                ...
                CPPUNIT_ASSERT_DOUBLES_EQUAL(expectedChange,
                                             change->s_Value[0],
