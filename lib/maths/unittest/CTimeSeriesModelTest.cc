...
void CTimeSeriesModelTest::testAddBucketValue(void)
{
    ...
    prior.adjustOffset(maths::CConstantWeights::COUNT,
                       {-1.0}, maths::CConstantWeights::SINGLE_UNIT);
...
void CTimeSeriesModelTest::testAddSamples(void)
{
    ...
    LOG_DEBUG("Propagation interval multivariate");
    {
        ...
        double interval[]{1.0, 1.1, 0.4};
        TDouble2Vec samples[]{{13.5, 13.4, 13.3},
                              {13.9, 13.8, 13.7},
                              {20.1, 20.0, 10.9}};
    ...
    LOG_DEBUG("Decay rate control univariate");
    {
        ...
        {
            double sample{20.0 + 4.0 * std::sin(  boost::math::double_constants::two_pi
                                                * static_cast<double>(time) / 86400.0)
...
void CTimeSeriesModelTest::testPredict(void)
{
    ...
    LOG_DEBUG("Multivariate Seasonal");
    {
        ...
        rng.generateMultivariateNormalSamples(mean,
                                              {{3.0, 2.9, 0.5},
                                               {2.9, 2.6, 0.1},
                                               {0.5, 0.1, 2.0}},
                                              1000, samples);
        ...
        for (core_t::TTime time_ = time; time_ < time + 86400; time_ += 3600)
        {
            ...
            {
                double trend_{mean[i] + 10.0 + 5.0 * std::sin(  boost::math::double_constants::two_pi
                                                              * static_cast<double>(time_) / 86400.0)};
...
void CTimeSeriesModelTest::testProbability(void)
{
    ...
    LOG_DEBUG("Multivariate");
    {
        ...
        TDoubleVecVec samples;
        rng.generateMultivariateNormalSamples({10.0, 15.0, 11.0},
                                              {{3.0, 2.9, 0.5},
                                               {2.9, 2.6, 0.1},
                                               {0.5, 0.1, 2.0}},
                                               1000, samples);
    ...
    LOG_DEBUG("Anomalies");
    {
        ...
        for (auto sample : samples)
        {
            ...
            bool conditional;
            TSize1Vec mostAnomalousCorrelate;
            model.probability(computeProbabilityParams(weight), {{time}}, {{sample}},
...
void CTimeSeriesModelTest::testMemoryUsage(void)
{
    ...
    LOG_DEBUG("Univariate");
    {
        ...
        boost::scoped_ptr<maths::CModel> model{
            new maths::CUnivariateTimeSeriesModel{modelParams(bucketLength), 0,
                                                  trend, univariateNormal(),
                                                  &controllers}};
...
void CTimeSeriesModelTest::testPersist(void)
{
    ...
    LOG_DEBUG("Univariate");
    {
        maths::CTimeSeriesDecomposition trend{24.0 * DECAY_RATE, bucketLength};
        auto controllers = decayRateControllers(1);
        maths::CUnivariateTimeSeriesModel origModel{params, 1,
                                                    trend, univariateNormal(),
                                                    &controllers};
    ...
    {
        TDoubleVecVec samples;
        rng.generateMultivariateNormalSamples({11.0, 10.0, 12.0},
                                              {{4.0, 2.9, 0.5},
                                               {2.9, 2.6, 0.1},
                                               {0.5, 0.1, 2.0}},
                                              1000, samples);
    ...
    LOG_DEBUG("Univariate");
    {
             ...
             time += halfHour, ++i)
        {
            ...
            std::string interval_{core::CContainerPrinter::print(
                    restoredModel.confidenceInterval(time, 90.0,
                                                     maths::CConstantWeights::COUNT,
...
void CTimeSeriesModelTest::testAnomalyModel(void)
{
    ...
    LOG_DEBUG("Multivariate")
    {
        ...
        core_t::TTime bucketLength{600};
        TDoubleVecVec samples;
        rng.generateMultivariateNormalSamples({10.0, 10.0, 10.0},
                                              {{4.0, 0.9, 0.5},
                                               {0.9, 2.6, 0.1},
                                               {0.5, 0.1, 3.0}},
                                              length, samples);
...
void CTimeSeriesModelTest::testStepChangeDiscontinuities(void)
{
    ...
    auto updateModel = [&](core_t::TTime time, double value, maths::CUnivariateTimeSeriesModel &model)
        {
            weight[0][0] = model.winsorisationWeight(1.0, time, {value});
    ...
    //actual << "r = [";
    //modelBounds << "x = [";
    //auto updateTestDebug = [&](core_t::TTime time, double value, const maths::CUnivariateTimeSeriesModel &model)
    //    {
    ...
    LOG_DEBUG("Univariate: Piecwise Constant");
    {
        ...
        auto pushErrorBar = [&](const maths::SErrorBar &errorBar)
            {
                forecast.push_back({errorBar.s_LowerBound,
    ...
    LOG_DEBUG("Univariate: Saw Tooth");
    {
        ...
        double percentageOutOfBounds{100.0 * outOfBounds / static_cast<double>(forecast.size())};
        LOG_DEBUG("% out-of-bounds = " << percentageOutOfBounds);
        CPPUNIT_ASSERT(percentageOutOfBounds < 6.0);
    }
}

void CTimeSeriesModelTest::daylightSaving(void)
{
    LOG_DEBUG("+----------------------------------------+");
    LOG_DEBUG("|  CTimeSeriesModelTest::daylightSaving  |");
    LOG_DEBUG("+----------------------------------------+");
