...
BOOST_AUTO_TEST_CASE(testIsFinite) {
    ...
    test1.push_back(-1e6);
    BOOST_TEST_REQUIRE(std::equal(test1.begin(), test1.end(), maths::CMathsFuncs::beginFinite(test1)));
    ...
    test2.push_back(-1e6);
    test2.push_back(zero() / zero());
    BOOST_TEST_REQUIRE(std::equal(test1.begin(), test1.end(), maths::CMathsFuncs::beginFinite(test2)));
...
BOOST_AUTO_TEST_CASE(testFpStatus) {
    ...
    BOOST_REQUIRE_EQUAL(maths_t::E_FpOverflowed, maths::CMathsFuncs::fpStatus(1.0 / zero()));
    BOOST_REQUIRE_EQUAL(maths_t::E_FpFailed, maths::CMathsFuncs::fpStatus(zero() / zero()));
}
