...
void CMathsFuncsTest::testIsFinite(void) {
    ...
    test1.push_back(-1e6);
    CPPUNIT_ASSERT(std::equal(test1.begin(), test1.end(), maths::CMathsFuncs::beginFinite(test1)));
    ...
    test2.push_back(-1e6);
    test2.push_back(zero() / zero());
    CPPUNIT_ASSERT(std::equal(test1.begin(), test1.end(), maths::CMathsFuncs::beginFinite(test2)));

    TDoubleVec test3;
    CPPUNIT_ASSERT(maths::CMathsFuncs::beginFinite(test3) == maths::CMathsFuncs::endFinite(test3));

    TDoubleVec test4;
    test4.push_back(zero() / zero());
    test4.push_back(1.0 / zero());
    test4.push_back(zero() / zero());
    CPPUNIT_ASSERT(maths::CMathsFuncs::beginFinite(test4) == maths::CMathsFuncs::endFinite(test4));
}

void CMathsFuncsTest::testFpStatus(void) {
    CPPUNIT_ASSERT_EQUAL(maths_t::E_FpNoErrors, maths::CMathsFuncs::fpStatus(3.8));
    CPPUNIT_ASSERT_EQUAL(maths_t::E_FpOverflowed, maths::CMathsFuncs::fpStatus(1.0 / zero()));
...
CppUnit::Test *CMathsFuncsTest::suite(void) {
    ...
    suiteOfTests->addTest(new CppUnit::TestCaller<CMathsFuncsTest>("CMathsFuncsTest::testIsInf",
                                                                   &CMathsFuncsTest::testIsInf));
    suiteOfTests->addTest(new CppUnit::TestCaller<CMathsFuncsTest>("CMathsFuncsTest::testIsFinite",
                                                                   &CMathsFuncsTest::testIsFinite));
    suiteOfTests->addTest(new CppUnit::TestCaller<CMathsFuncsTest>("CMathsFuncsTest::testFpStatus",
