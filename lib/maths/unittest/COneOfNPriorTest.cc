...
void COneOfNPriorTest::testFilter(void) {
    ...
    TPriorPtrVec models;
    models.push_back(
        TPriorPtr(maths::CGammaRateConjugate::nonInformativePrior(E_ContinuousData).clone()));
    models.push_back(TPriorPtr(
        maths::CLogNormalMeanPrecConjugate::nonInformativePrior(E_ContinuousData).clone()));
    models.push_back(
        TPriorPtr(maths::CNormalMeanPrecConjugate::nonInformativePrior(E_ContinuousData).clone()));
    ...
    filter.removeModels(maths::CPrior::CModelFilter()
                            .remove(maths::CPrior::E_Poisson)
...
void COneOfNPriorTest::testWeights(void) {
...

    {
        ...
        models.push_back(
            TPriorPtr(CNormalMeanPrecConjugate::nonInformativePrior(E_ContinuousData).clone()));
...

    {
        ...
        for (std::size_t decayRate = 0; decayRate < boost::size(decayRates); ++decayRate) {
            ...
            models.push_back(TPriorPtr(CPoissonMeanConjugate::nonInformativePrior().clone()));
            models.push_back(
                TPriorPtr(CNormalMeanPrecConjugate::nonInformativePrior(E_IntegerData).clone()));
            ...
            LOG_DEBUG("log weights ratio = " << (logWeights[1] - logWeights[0]) /
                                                    previousLogWeightRatio);
...
void COneOfNPriorTest::testModelSelection(void) {
...

    {
        ...
        double expectedLogWeightRatio =
            (normalExpectedLogWeight - poissonExpectedLogWeight) * static_cast<double>(nSamples);
    ...
    }
    {
        ...
        models.push_back(TPriorPtr(
            new maths::CMultimodalPrior(maths_t::E_ContinuousData,
                                        clusterer,
...
void COneOfNPriorTest::testMarginalLikelihood(void) {
    ...
    for (std::size_t t = 0u; t < boost::size(dataTypes); ++t) {
        ...
        models.push_back(TPriorPtr(CPoissonMeanConjugate::nonInformativePrior().clone()));
        models.push_back(
            TPriorPtr(CNormalMeanPrecConjugate::nonInformativePrior(dataTypes[t]).clone()));
        models.push_back(
            TPriorPtr(CLogNormalMeanPrecConjugate::nonInformativePrior(dataTypes[t]).clone()));
    ...
    TPriorPtrVec models;
    models.push_back(
        TPriorPtr(CLogNormalMeanPrecConjugate::nonInformativePrior(E_ContinuousData).clone()));
    models.push_back(
        TPriorPtr(CNormalMeanPrecConjugate::nonInformativePrior(E_ContinuousData).clone()));
...

    {
        ...
        for (std::size_t i = 0u; i < boost::size(locations); ++i) {
            for (std::size_t j = 0u; j < boost::size(squareScales); ++j) {
                ...
                double a = 0.01;
                double b =
                    boost::math::mode(logNormal) + 1.0 * boost::math::standard_deviation(logNormal);
                maths::CSolvers::maximize(a,
                                          b,
                                          likelihood(a),
                                          likelihood(b),
        ...
        for (std::size_t i = 0u; i < boost::size(means); ++i) {
            for (std::size_t j = 0u; j < boost::size(variances); ++j) {
                ...
                models.push_back(TPriorPtr(
                    CNormalMeanPrecConjugate::nonInformativePrior(E_ContinuousData).clone()));
                models.push_back(
                    TPriorPtr(CGammaRateConjugate::nonInformativePrior(E_ContinuousData).clone()));
                ...
                for (std::size_t k = 0u; k < samples.size(); ++k) {
                    ...
                    relativeError.add(
                        ::fabs(expectedVariance - filter.marginalLikelihoodVariance()) /
                        expectedVariance);
                }
...
CppUnit::Test* COneOfNPriorTest::suite(void) {
    ...
    suiteOfTests->addTest(new CppUnit::TestCaller<COneOfNPriorTest>("COneOfNPriorTest::testFilter",
                                                                    &COneOfNPriorTest::testFilter));
    suiteOfTests->addTest(
        new CppUnit::TestCaller<COneOfNPriorTest>("COneOfNPriorTest::testMultipleUpdate",
                                                  &COneOfNPriorTest::testMultipleUpdate));
    suiteOfTests->addTest(
        new CppUnit::TestCaller<COneOfNPriorTest>("COneOfNPriorTest::testWeights",
                                                  &COneOfNPriorTest::testWeights));
    suiteOfTests->addTest(new CppUnit::TestCaller<COneOfNPriorTest>("COneOfNPriorTest::testModels",
                                                                    &COneOfNPriorTest::testModels));
    suiteOfTests->addTest(
        new CppUnit::TestCaller<COneOfNPriorTest>("COneOfNPriorTest::testModelSelection",
                                                  &COneOfNPriorTest::testModelSelection));
    suiteOfTests->addTest(
        new CppUnit::TestCaller<COneOfNPriorTest>("COneOfNPriorTest::testMarginalLikelihood",
                                                  &COneOfNPriorTest::testMarginalLikelihood));
    suiteOfTests->addTest(
        new CppUnit::TestCaller<COneOfNPriorTest>("COneOfNPriorTest::testSampleMarginalLikelihood",
                                                  &COneOfNPriorTest::testSampleMarginalLikelihood));
    suiteOfTests->addTest(
        new CppUnit::TestCaller<COneOfNPriorTest>("COneOfNPriorTest::testMarginalLikelihoodMean",
                                                  &COneOfNPriorTest::testMarginalLikelihoodMean));
    suiteOfTests->addTest(
        new CppUnit::TestCaller<COneOfNPriorTest>("COneOfNPriorTest::testMarginalLikelihoodMode",
                                                  &COneOfNPriorTest::testMarginalLikelihoodMode));
    suiteOfTests->addTest(new CppUnit::TestCaller<
                          COneOfNPriorTest>("COneOfNPriorTest::testMarginalLikelihoodVariance",
                                            &COneOfNPriorTest::testMarginalLikelihoodVariance));
    suiteOfTests->addTest(new CppUnit::TestCaller<COneOfNPriorTest>("COneOfNPriorTest::testCdf",
                                                                    &COneOfNPriorTest::testCdf));
    suiteOfTests->addTest(new CppUnit::TestCaller<
                          COneOfNPriorTest>("COneOfNPriorTest::testProbabilityOfLessLikelySamples",
                                            &COneOfNPriorTest::testProbabilityOfLessLikelySamples));
    suiteOfTests->addTest(
        new CppUnit::TestCaller<COneOfNPriorTest>("COneOfNPriorTest::testPersist",
