...
void gaussianWithUniformNoise(test::CRandomNumbers &rng, TVectorVec &points)
{
    ...
    TDoubleVecVec covariance{{ 1.0,  0.1, -0.1, 0.3, 0.2, -0.1},
                             { 0.1,  1.3, -0.3, 0.1, 0.1,  0.1},
                             {-0.1, -0.3,  2.1, 0.1, 0.2,  0.1},
                             { 0.3,  0.1,  0.1, 0.8, 0.2, -0.2},
                             { 0.2,  0.1,  0.2, 0.2, 2.2, -0.1},
                             {-0.1,  0.1,  0.1,-0.2,-0.1,  3.1}};
...
void CLocalOutlierFactorsTest::testDlof(void)
{
    ...
    for (const auto &point : points)
    {
        ...
        ldof.push_back(  maths::CBasicStatistics::mean(d)
                       / maths::CBasicStatistics::mean(D));
    }
...
void CLocalOutlierFactorsTest::testTotalDistancekNN(void)
{
    ...
    for (const auto &point : points)
    {
        TVectorVec neighbours;
        nearestNeightbours(3, points, point, neighbours);
        distances.push_back(std::accumulate(neighbours.begin(), neighbours.end(), 0.0,
...
CppUnit::Test *CLocalOutlierFactorsTest::suite(void)
{
    ...
    suiteOfTests->addTest( new CppUnit::TestCaller<CLocalOutlierFactorsTest>(
                                   "CLocalOutlierFactorsTest::testLof",
                                   &CLocalOutlierFactorsTest::testLof) );
    suiteOfTests->addTest( new CppUnit::TestCaller<CLocalOutlierFactorsTest>(
                                   "CLocalOutlierFactorsTest::testDlof",
                                   &CLocalOutlierFactorsTest::testDlof) );
    suiteOfTests->addTest( new CppUnit::TestCaller<CLocalOutlierFactorsTest>(
                                   "CLocalOutlierFactorsTest::testDistancekNN",
                                   &CLocalOutlierFactorsTest::testDistancekNN) );
    suiteOfTests->addTest( new CppUnit::TestCaller<CLocalOutlierFactorsTest>(
                                   "CLocalOutlierFactorsTest::testTotalDistancekNN",
                                   &CLocalOutlierFactorsTest::testTotalDistancekNN) );
    suiteOfTests->addTest( new CppUnit::TestCaller<CLocalOutlierFactorsTest>(
                                   "CLocalOutlierFactorsTest::testEnsemble",
                                   &CLocalOutlierFactorsTest::testEnsemble) );
