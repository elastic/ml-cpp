...
core_t::TTime CExpandingWindow::beginValuesTime() const {
    ...
    core_t::TTime firstSampleTime{common::CIntegerTools::ceil(m_StartTime, m_SampleInterval)};
    core_t::TTime lastSampleTime{common::CIntegerTools::floor(endTime, m_SampleInterval)};
...
CExpandingWindow::valuesMinusPrediction(TFloatMeanAccumulatorVec bucketValues,
    ...
    for (std::size_t i = 0; i < bucketValues.size(); ++i) {
        if (common::CBasicStatistics::count(bucketValues[i]) > 0.0) {
            common::CBasicStatistics::moment<0>(bucketValues[i]) -=
...
double CExpandingWindow::withinBucketVariance() const {
    return common::CBasicStatistics::mean(m_AverageWithinBucketVariance);
}

void CExpandingWindow::initialize(core_t::TTime time) {
    m_StartTime = common::CIntegerTools::floor(time, m_BucketLengths[0]);
}
    ...
    if (time >= m_StartTime) {
        ...
        if (index != m_BucketIndex) {
            m_AverageWithinBucketVariance.add(
                common::CBasicStatistics::variance(m_WithinBucketVariance),
                common::CBasicStatistics::count(m_WithinBucketVariance));
            m_WithinBucketVariance = TMeanVarAccumulator{};
            m_BucketIndex = index;
        }
...
uint64_t CExpandingWindow::checksum(uint64_t seed) const {
    ...
    seed = common::CChecksum::calculate(seed, m_BucketValues);
    return common::CChecksum::calculate(seed, m_MeanOffset);
}
