...
public:
    ...
    void addForecast(core_t::TTime time, const maths::common::SErrorBar& forecast) {
        if (ENABLED) {
...
void mockSink(maths::common::SErrorBar errorBar, TErrorBarVec& prediction) {
...
public:
    ...
    void run(TTrend trend) const {
        ...
        maths::time_series::CUnivariateTimeSeriesModel::TDecayRateController2Ary controllers{
            decayRateControllers()};
        maths::time_series::CUnivariateTimeSeriesModel model(
            params(m_BucketLength), TAG,
            maths::time_series::CTimeSeriesDecomposition(0.012, m_BucketLength),
        ...
        BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(error) < m_MaximumError);
    }
...
BOOST_AUTO_TEST_CASE(testDailyVaryingLongTermTrend) {
    ...
    maths::common::CSpline<> trend_(maths::common::CSplineTypes::E_Cubic);
    trend_.interpolate(times, values, maths::common::CSplineTypes::E_Natural);
...
BOOST_AUTO_TEST_CASE(testNonNegative) {
    ...
    maths::time_series::CTimeSeriesDecomposition trend(0.012, bucketLength);
    ...
    TModelPtr forecastModel(model.cloneForForecast());
    forecastModel->forecast(
        0, start, start, end, maths::common::CModel::DEFAULT_BOUNDS_PERCENTILE,
