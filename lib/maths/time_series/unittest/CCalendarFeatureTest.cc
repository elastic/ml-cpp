...
using TCalendarFeatureVec = std::vector<maths::time_series::CCalendarFeature>;
...
BOOST_AUTO_TEST_CASE(testInitialize) {
    ...
    for (std::size_t i = 0; i < times.size(); ++i) {
        ...
        maths::time_series::CCalendarFeature::TCalendarFeature4Ary expected;
        expected[0] = maths::time_series::CCalendarFeature(
            maths::time_series::CCalendarFeature::DAYS_SINCE_START_OF_MONTH, time);
        expected[1] = maths::time_series::CCalendarFeature(
            maths::time_series::CCalendarFeature::DAYS_BEFORE_END_OF_MONTH, time);
        expected[2] = maths::time_series::CCalendarFeature(
            maths::time_series::CCalendarFeature::DAY_OF_WEEK_AND_WEEKS_SINCE_START_OF_MONTH,
            time);
        expected[3] = maths::time_series::CCalendarFeature(
            maths::time_series::CCalendarFeature::DAY_OF_WEEK_AND_WEEKS_BEFORE_END_OF_MONTH, time);

        maths::time_series::CCalendarFeature::TCalendarFeature4Ary actual =
            maths::time_series::CCalendarFeature::features(time);
...
BOOST_AUTO_TEST_CASE(testComparison) {
    ...
    for (std::size_t i = 0; i < times.size(); ++i) {
        core_t::TTime time{static_cast<core_t::TTime>(times[i])};
        maths::time_series::CCalendarFeature::TCalendarFeature4Ary fi =
            maths::time_series::CCalendarFeature::features(time);
...
BOOST_AUTO_TEST_CASE(testOffset) {
    ...
    for (const auto& time_ : times) {
        ...
        maths::time_series::CCalendarFeature::TCalendarFeature4Ary features =
            maths::time_series::CCalendarFeature::features(time);
    ...
    LOG_DEBUG(<< "Test days since start of month");
    {
        maths::time_series::CCalendarFeature feature(
            maths::time_series::CCalendarFeature::DAYS_SINCE_START_OF_MONTH, feb1st);
    ...
    {
        maths::time_series::CCalendarFeature feature(
            maths::time_series::CCalendarFeature::DAYS_SINCE_START_OF_MONTH,
            feb1st + 12 * DAY);
        for (core_t::TTime time = march1st; time < april1st; time += DAY) {
    ...
    {
        maths::time_series::CCalendarFeature feature(
            maths::time_series::CCalendarFeature::DAYS_BEFORE_END_OF_MONTH, feb1st);
        for (core_t::TTime time = march1st; time < april1st; time += DAY) {
    ...
    LOG_DEBUG(<< "Test day of week and week until end of month");
    {
        ...
        maths::time_series::CCalendarFeature feature(
            maths::time_series::CCalendarFeature::DAY_OF_WEEK_AND_WEEKS_BEFORE_END_OF_MONTH,
            feb1st);
        for (core_t::TTime time = april1st; time < may1st; time += DAY) {
            BOOST_REQUIRE_EQUAL(time - april1st - 4 * DAY, feature.offset(time));
            BOOST_REQUIRE_EQUAL(time - april1st - 4 * DAY + 7200,
...
BOOST_AUTO_TEST_CASE(testPersist) {
    maths::time_series::CCalendarFeature::TCalendarFeature4Ary features =
        maths::time_series::CCalendarFeature::features(core::CTimeUtils::now());

    for (std::size_t i = 0; i < 4; ++i) {
        std::string state = features[i].toDelimited();
        LOG_DEBUG(<< "state = " << state);

        maths::time_series::CCalendarFeature restored;
