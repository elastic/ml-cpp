...
BOOST_AUTO_TEST_CASE(testSpread) {
    ...
    for (std::size_t i = 0; i < 100; ++i) {
        ...
        maths::time_series::CAdaptiveBucketing::spread(0.0, period, 150.0, samples);
        double eps{1e-2};
        ...
        for (std::size_t j = 0; j < samples.size(); ++j) {
            dcdx += maths::common::CTools::pow2(samples[j] + eps - origSamples[j]) -
                    maths::common::CTools::pow2(samples[j] - eps - origSamples[j]);
        }
        dcdx /= 2.0 * eps;
