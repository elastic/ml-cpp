...
using TSegmentation = maths::time_series::CTimeSeriesSegmentation;
...
BOOST_AUTO_TEST_CASE(testPiecewiseLinear) {
    ...
    for (auto outlierFraction : {0.0, 0.1}) {
        ...
        for (core_t::TTime time = 0; time < range; time += halfHour) {
            ...
            debug.addValue(maths::common::CBasicStatistics::mean(values[time / halfHour]));
        }
        ...
        for (const auto& residual : residuals) {
            ...
            debug.addResidual(maths::common::CBasicStatistics::mean(residual));
        }
        ...
        // Not biased.
        BOOST_TEST_REQUIRE(
            std::fabs(maths::common::CBasicStatistics::mean(residualMoments) -
                      maths::common::CBasicStatistics::mean(noiseMoments)) <
            2.0 * std::sqrt(maths::common::CBasicStatistics::variance(noiseMoments) /
                            maths::common::CBasicStatistics::count(noiseMoments)));

        // We've explained nearly all the variance.
        BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::variance(residualMoments) <
                           1.4 * maths::common::CBasicStatistics::variance(noiseMoments));
    }
    ...
    TMeanVarAccumulator residualMoments;
    for (auto i : inliers) {
        residualMoments.add(maths::common::CBasicStatistics::mean(residuals[i]));
        debug.addResidual(maths::common::CBasicStatistics::mean(residuals[i]));
    }
    LOG_DEBUG(<< "noise moments    = " << noiseMoments);
    LOG_DEBUG(<< "residual moments = " << residualMoments);
    ...
    BOOST_TEST_REQUIRE(
        std::fabs(maths::common::CBasicStatistics::mean(residualMoments) -
                  maths::common::CBasicStatistics::mean(noiseMoments)) <
        3.5 * std::sqrt(maths::common::CBasicStatistics::variance(noiseMoments) /
                        maths::common::CBasicStatistics::count(noiseMoments)));
...
BOOST_AUTO_TEST_CASE(testPiecewiseLinearScaledSeasonal) {
    ...
    maths::time_series::CSignal::TSeasonalComponentVec periods[]{
        {maths::time_series::CSignal::seasonalComponentSummary(48)},
        {maths::time_series::CSignal::SSeasonalComponentSummary{48, 0, 336, {0, 240}},
         maths::time_series::CSignal::SSeasonalComponentSummary{336, 0, 336, {0, 240}},
         maths::time_series::CSignal::SSeasonalComponentSummary{48, 0, 336, {240, 336}},
         maths::time_series::CSignal::SSeasonalComponentSummary{336, 0, 336, {240, 336}}},
        {maths::time_series::CSignal::seasonalComponentSummary(48)}};
    ...
    for (auto seasonal : {smoothDaily, weekends}) {
        ...
        maths::time_series::CSignal::TMeanAccumulatorVecVec components;
        maths::time_series::CSignal::fitSeasonalComponentsRobust(
            periods[i], 0.05, values, components);
        TSizeVec segmentation(TSegmentation::piecewiseLinearScaledSeasonal(
            values,
            [&](std::size_t j) {
                double result{0.0};
                for (std::size_t k = 0; k < periods[i].size(); ++k) {
                    if (periods[i][k].contains(j)) {
                        result += maths::common::CBasicStatistics::mean(
...
BOOST_AUTO_TEST_CASE(testRemovePiecewiseLinearScaledSeasonal) {
    ...
    for (auto seasonal : {smoothDaily, weekends, spikeyDaily}) {
        ...
        BOOST_TEST_REQUIRE(
            std::fabs(maths::common::CBasicStatistics::mean(residualMoments) -
                      maths::common::CBasicStatistics::mean(noiseMoments)) <
            3.0 * std::sqrt(maths::common::CBasicStatistics::variance(noiseMoments) /
                            maths::common::CBasicStatistics::count(noiseMoments)));

        // We've explained nearly all the variance.
        BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::variance(residualMoments) <
                           1.1 * maths::common::CBasicStatistics::variance(noiseMoments));
    }
}
...
BOOST_AUTO_TEST_CASE(testRemovePiecewiseLinearDiscontinuities) {
    ...
    for (const auto& value : values) {
        BOOST_REQUIRE_EQUAL(
            3.0, static_cast<double>(maths::common::CBasicStatistics::mean(value)));
    }
    ...
    for (std::size_t i = 1; i < values.size(); ++i) {
        BOOST_TEST_REQUIRE(
            std::fabs(maths::common::CBasicStatistics::mean(values[i]) -
                      maths::common::CBasicStatistics::mean(values[i - 1])) < 0.25);
    }
    for (std::size_t i = 1; i < length; ++i) {
        if (i < 50) {
            BOOST_REQUIRE_CLOSE_ABSOLUTE(
                0.1,
                static_cast<double>(maths::common::CBasicStatistics::mean(values[i]) -
                                    maths::common::CBasicStatistics::mean(values[i - 1])),
                1e-4);
        } else if (i > 50 && i < 200) {
            BOOST_REQUIRE_CLOSE_ABSOLUTE(
                -0.1,
            ...
            BOOST_REQUIRE_CLOSE_ABSOLUTE(
                ...
                static_cast<double>(maths::common::CBasicStatistics::mean(values[i])), 1e-4);
        }
    }
}
...
BOOST_AUTO_TEST_CASE(testMeanScalePiecewiseLinearScaledSeasonal) {
    ...
    auto predictor = [](const maths::time_series::CSignal::TSeasonalComponentVec& periods,
                        const maths::time_series::CSignal::TMeanAccumulatorVecVec& components,
                        std::size_t j) {
        double result{0.0};
        for (std::size_t i = 0; i < components.size(); ++i) {
            if (periods[i].contains(j)) {
                result += maths::common::CBasicStatistics::mean(
                    components[i][periods[i].offset(j)]);
            }
        }
        return result;
    };
    ...
    maths::time_series::CSignal::TSeasonalComponentVec periods[]{
        {maths::time_series::CSignal::seasonalComponentSummary(24)},
        {maths::time_series::CSignal::SSeasonalComponentSummary{24, 0, 168, {0, 120}},
         maths::time_series::CSignal::SSeasonalComponentSummary{168, 0, 168, {0, 120}},
         maths::time_series::CSignal::SSeasonalComponentSummary{24, 0, 168, {120, 168}},
         maths::time_series::CSignal::SSeasonalComponentSummary{168, 0, 168, {120, 168}}}};
    ...
    for (const auto& seasonal : {smoothDaily, weekends}) {
        ...
        for (std::size_t test = 0; test < 10; ++test) {
            ...
            maths::time_series::CSignal::fitSeasonalComponents(periods[i], values, components);
            ...
            BOOST_REQUIRE_CLOSE_ABSOLUTE(
                ...
                maths::common::CBasicStatistics::variance(errorMoments), 0.3);

            overallErrorMoments += errorMoments;
        }

        BOOST_REQUIRE_CLOSE_ABSOLUTE(
            0.0, maths::common::CBasicStatistics::mean(overallErrorMoments),
            std::sqrt(static_cast<double>(values.size()) /
                      static_cast<double>(repeats[i]) / 10.0));
        BOOST_REQUIRE_CLOSE(
            1.0 / (static_cast<double>(values.size()) / static_cast<double>(repeats[i])),
            maths::common::CBasicStatistics::variance(overallErrorMoments),
            40.0); // 40%
        ++i;
    }
}
...
BOOST_AUTO_TEST_CASE(testPiecewiseTimeShifted) {
    ...
    BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanError) < 2.7);
}
