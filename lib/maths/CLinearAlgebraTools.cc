...
public:
    ...
    static maths_t::EFloatingPointErrorStatus compute(std::size_t d,
        ...
        case 1:
            ...
            result = -0.5 * (residual(0) * residual(0) / covariance_(0, 0) + core::constants::LOG_TWO_PI +
                             ::log(covariance_(0, 0)));
            return maths_t::E_FpNoErrors;
        ...
        default: {
            ...
            result =
                -0.5 * (residual.inner(y) + static_cast<double>(rank) * core::constants::LOG_TWO_PI + logDeterminant);
            return maths_t::E_FpNoErrors;
        }
        }
    }
...
public:
    ...
    static void
    generate(std::size_t n, const VECTOR& mean_, const MATRIX& covariance_, std::vector<VECTOR_PRECISE>& result) {
        result.clear();
        if (n == 0) {
            return;
        }
...
#define INVERSE_QUADRATIC_PRODUCT(T, N)                                                                                \
    maths_t::EFloatingPointErrorStatus inverseQuadraticProduct(std::size_t d,                                          \
        ...
        return CInverseQuadraticProduct<SDenseMatrix<CSymmetricMatrixNxN<T, N>>::Type,                                 \
                                        SDenseVector<CVectorNx1<T, N>>::Type>::compute(d,                              \
                                                                                       covariance,                     \
                                                                                       residual,                       \
                                                                                       result,                         \
...
maths_t::EFloatingPointErrorStatus inverseQuadraticProduct(std::size_t d,
    ...
    return CInverseQuadraticProduct<SDenseMatrix<CSymmetricMatrix<double>>::Type,
                                    SDenseVector<CVector<double>>::Type>::compute(d,
                                                                                  covariance,
                                                                                  residual,
                                                                                  result,
                                                                                  ignoreSingularSubspace);
