    ...
    do {
        ...
        if (name == ROWS_TAG) {
            ...
            if (core::CPersistUtils::fromString(
                    traverser.value(), counts, DELIMITER, PAIR_DELIMITER) == false) {
            ...
            if (traverser.traverseSubLevel(boost::bind(
                    &SSketch::acceptRestoreTraverser, &sketch, _1, m_Rows, m_Columns)) == false) {
                return false;
            }
        }
...
std::size_t CCountMinSketch::rows(void) const { return m_Rows; }

std::size_t CCountMinSketch::columns(void) const { return m_Columns; }
...
double CCountMinSketch::totalCount(void) const { return m_TotalCount; }
...
bool CCountMinSketch::sketched(void) const { return boost::get<SSketch>(&m_Sketch) != 0; }
...
bool CCountMinSketch::SSketch::acceptRestoreTraverser(core::CStateRestoreTraverser &traverser,
    ...
    do {
        ...
        if (name == HASHES_TAG) {
            ...
            if (core::CPersistUtils::fromString(
                    traverser.value(), hashFromString, s_Hashes, DELIMITER) == false ||
                s_Hashes.size() != rows) {
    ...
    if (s_Counts.size() != rows) {
        LOG_ERROR("Unexpected number of counts " << s_Counts.size() << ", number of rows " << rows);
        return false;
    }
    return true;
}
