                                                          ...
                                                          maths_t::ETail &tail) const
{
    ...
    else
    {
        ...
        for (;;)
        {
            ...
            // bracket point.
            double lowerBound = b2 + std::log(  std::max(fx, MIN_DOUBLE)
                                                          ...
                                                          maths_t::ETail &tail) const
{
    ...
    if (x < sp.first)
    {
        ...
        if (y[i % 2] > support.second)
        {
            return truncate(sp.second ?
                            safeCdf(beta_, x) :
                            safeCdfComplement(beta_, x), 0.0, 1.0);
        }
    ...
    else
    {
        ...
        {
            return truncate(sp.second ?
                            safeCdfComplement(beta_, x) :
                            safeCdf(beta_, x), 0.0, 1.0);
        }
    ...
    try
    {
        ...
        if (::fabs(error) <= PDF_TOLERANCE * std::max(fx, fy))
        {
            ...
            return truncate(sp.second ?
                            ...
                            safeCdf(beta_, y[i % 2]) - safeCdf(beta_, x), 0.0, 1.0);
        }
        else if (error > 0.0)
        {
            if (x < sp.first)
...
void CTools::CMixtureProbabilityOfLessLikelySample::intervals(TDoubleDoublePrVec &intervals)
{
    ...
    m_Endpoints.erase(std::unique(m_Endpoints.begin(),
...
double CTools::SIntervalExpectation::operator()(const normal &normal_,
                                                double a,
                                                double b) const
{
    if (a > b)
...
double CTools::SIntervalExpectation::operator()(const lognormal &logNormal,
                                                double a,
                                                double b) const
{
    if (a > b)
...
double CTools::SIntervalExpectation::operator()(const gamma &gamma_,
                                                double a,
                                                double b) const
{
    if (a > b)
