    ...
    for (std::size_t i = 0; i < 2; ++i) {
        ...
        for (std::size_t l = 2; l < rootb; ++l) {
            ...
            for (std::size_t p = 0; p < mi.size(); ++p) {
                mic = std::max(
                    mic, mi[p] / common::CTools::fastLog(static_cast<double>(p + 2)));
            }
        }
...
std::size_t CMic::maximumGridSize() const {
    ...
    return static_cast<std::size_t>(
        std::pow(static_cast<double>(m_Samples.size()),
                 common::CTools::linearlyInterpolate(*leftKnot, *rightKnot, exponentAtLeftKnot,
...
double CMic::maximumXAxisPartitionSizeToSearch() const {
    ...
    return common::CTools::linearlyInterpolate(*leftKnot, *rightKnot, sizeAtLeftKnot,
...
double CMic::entropy(const TVectorXd& dist) {
    ...
    for (auto p : dist) {
        if (p > 0.0) {
            result -= p * common::CTools::fastLog(p);
        }
    }
    return result;
}
