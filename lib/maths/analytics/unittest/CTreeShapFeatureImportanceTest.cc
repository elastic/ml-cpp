...
using TTree = std::vector<maths::analytics::CBoostedTreeNode>;
using TTreeVec = std::vector<TTree>;
using TDataFrameUPtr = std::unique_ptr<core::CDataFrame>;
using TTreeShapFeatureImportanceUPtr =
    std::unique_ptr<maths::analytics::CTreeShapFeatureImportance>;
using TEncoderUPtr = std::unique_ptr<maths::analytics::CDataFrameCategoryEncoder>;
...
class CStubMakeDataFrameCategoryEncoder final
    : public maths::analytics::CMakeDataFrameCategoryEncoder {
public:
    ...
    CMakeDataFrameCategoryEncoder::TEncodingUPtrVec makeEncodings() override {
        ...
        for (std::size_t i = 0; i < m_NumberColumns; ++i) {
            result.push_back(std::make_unique<maths::analytics::CDataFrameCategoryEncoder::CIdentityEncoding>(
                i, 1.0));
        }
        return result;
    }
...
struct SFixtureSingleTree {
    SFixtureSingleTree() : s_Trees(1) {
        ...
        s_TopTreeFeatureImportance =
            std::make_unique<maths::analytics::CTreeShapFeatureImportance>(
...
struct SFixtureRandomTrees {
    SFixtureRandomTrees() {
        ...
        s_TopTwoTreeFeatureImportance =
            std::make_unique<maths::analytics::CTreeShapFeatureImportance>(
                1, *s_Frame, *s_Encoder, s_SingleTree, 2);
        s_ForestFeatureImportance = std::make_unique<maths::analytics::CTreeShapFeatureImportance>(
            1, *s_Frame, *s_Encoder, s_MultipleTrees, s_NumberFeatures);
        s_ThreadedForestFeatureImportance =
            std::make_unique<maths::analytics::CTreeShapFeatureImportance>(
...
public:
    ...
    TDoubleVecVec shap(const core::CDataFrame& frame,
                       const maths::analytics::CDataFrameCategoryEncoder& encoder,
    ...
    TVector conditionalExpectation(const maths::analytics::CEncodedDataFrameRowRef& x,
                                   const TSizeSet& S) {
...
BOOST_FIXTURE_TEST_CASE(testSingleTreeExpectedNodeValues, SFixtureSingleTree) {
    std::size_t depth{maths::analytics::CTreeShapFeatureImportance::depth(s_Trees)};
    BOOST_TEST_REQUIRE(depth == 2);
    maths::analytics::CTreeShapFeatureImportance::computeInternalNodeValues(s_Trees);
...
BOOST_FIXTURE_TEST_CASE(testSingleRandomTreeShap, SFixtureRandomTrees) {
    ...
    for (std::size_t i = 0; i < expectedPhi.size(); ++i) {
        auto largest =
            maths::common::CBasicStatistics::orderStatisticsAccumulator<TDoubleSizePr, 2>(
