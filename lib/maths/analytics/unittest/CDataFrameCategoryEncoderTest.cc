...
BOOST_AUTO_TEST_CASE(testOneHotEncoding) {
    ...
    for (std::size_t threads : {1, 2}) {
        ...
        maths::analytics::CDataFrameCategoryEncoder encoder{{threads, *frame, 3}};

        for (std::size_t i = 0; i < encoder.numberEncodedColumns(); ++i) {
            switch (i) {
            case 0:
                BOOST_TEST_REQUIRE(maths::analytics::E_OneHot ==
                                   encoder.encoding(i).type());
...
BOOST_AUTO_TEST_CASE(testMeanValueEncoding) {
    ...
    for (std::size_t threads : {1, 2}) {
        ...
        maths::analytics::CMakeDataFrameCategoryEncoder factory{threads, *frame, 3};
        maths::analytics::CDataFrameCategoryEncoder encoder{factory};
        ...
        for (std::size_t i = 0; i < encoder.numberEncodedColumns(); ++i) {
            if (encoder.encoding(i).type() == maths::analytics::E_TargetMean) {
                for (std::size_t j = 0; j < expectedTargetMeanValues.size(); ++j) {
                    BOOST_REQUIRE_CLOSE_ABSOLUTE(
                        maths::common::CBasicStatistics::mean(expectedTargetMeanValues[j]),
                        encoder.encoding(i).encode(static_cast<double>(j)),
                        static_cast<double>(std::numeric_limits<float>::epsilon()) *
                            std::fabs(maths::common::CBasicStatistics::mean(
...
BOOST_AUTO_TEST_CASE(testRareCategories) {
    ...
    auto factory = maths::analytics::CMakeDataFrameCategoryEncoder{1, *frame, 3}.minimumFrequencyToOneHotEncode(
        0.1);
    factory.makeEncodings();
...
BOOST_AUTO_TEST_CASE(testCorrelatedFeatures) {
    ...
    // Two correlated + 4 independent metrics.
    {
        ...
        maths::analytics::CDataFrameCategoryEncoder encoder{{1, *frame, 6}};
    ...
    // Two correlated + two independent categorical fields.
    {
        ...
        maths::analytics::CDataFrameCategoryEncoder encoder{{1, *frame, 4}};
...
BOOST_AUTO_TEST_CASE(testWithRowMask) {
    ...
    maths::analytics::CDataFrameCategoryEncoder encoder{
        maths::analytics::CMakeDataFrameCategoryEncoder{1, *frame, 3}.rowMask(rowMask)};
    maths::analytics::CDataFrameCategoryEncoder maskedEncoder{{1, *maskedFrame, 3}};
...
BOOST_AUTO_TEST_CASE(testEncodingOfCategoricalTarget) {
    ...
    for (std::size_t i = 0; i < encoder.numberEncodedColumns(); ++i) {
        BOOST_REQUIRE_EQUAL(maths::analytics::E_IdentityEncoding,
                            encoder.encoding(i).type());
    }
}
    ...
    for (auto feature : categorical) {
        ...
        for (auto category : expectedOneHot[feature]) {
            expectedFrequencies[feature][category] =
                maths::common::CBasicStatistics::mean(meanFrequency);
        }
    }

    maths::analytics::CMakeDataFrameCategoryEncoder factory{1, *frame, 4};
    maths::analytics::CDataFrameCategoryEncoder encoder{factory};
    factory.makeEncodings();
...
BOOST_AUTO_TEST_CASE(testUnseenCategoryEncoding) {
    ...
    for (std::size_t i = 0; i < encodedRow.numberColumns() - 1; ++i) {
        if (encoder.isBinary(i)) {
            BOOST_REQUIRE_EQUAL(maths::common::CFloatStorage{0.0}, encodedRow[i]);
        } else {
            BOOST_TEST_REQUIRE(encodedRow[i] > 0.0);
        }
        rep << " " << encodedRow[i];
    }
    BOOST_REQUIRE_EQUAL(maths::common::CFloatStorage{1.5},
...
BOOST_AUTO_TEST_CASE(testDiscardNuisanceFeatures) {
    ...
    maths::analytics::CDataFrameCategoryEncoder encoder{
        maths::analytics::CMakeDataFrameCategoryEncoder{1, *frame, 6}.minimumRelativeMicToSelectFeature(
            0.02)};
...
BOOST_AUTO_TEST_CASE(testEstimateMemoryUsage) {
    ...
    }
    {
        ...
        BOOST_TEST_REQUIRE(20000000 < maths::analytics::CMakeDataFrameCategoryEncoder::estimateMemoryUsage(
                                          1000000, 2001, 1000));
        BOOST_TEST_REQUIRE(35000000 > maths::analytics::CMakeDataFrameCategoryEncoder::estimateMemoryUsage(
                                          1000000, 2001, 1000));
    }
}
...
BOOST_AUTO_TEST_CASE(testPersistRestore) {
    ...
    try {
        ...
        maths::analytics::CDataFrameCategoryEncoder restoredEncoder{traverser};
