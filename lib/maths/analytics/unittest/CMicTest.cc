        ...
        for (const auto& pi_ : allSubsetsOfSizeLessThan(t, pi)) {
            ...
            for (std::size_t i = 0; i < p.size(); ++i) {
                for (std::size_t j = 0; j < p[i].size(); ++j) {
                    mi += p[i][j] *
                          maths::common::CTools::fastLog(p[i][j] / px[i] / py[j]);
                }
            }
...
BOOST_AUTO_TEST_CASE(testBinaryVariables) {
    ...
    for (std::size_t t = 0; t < 10; ++t) {
        ...
        double mi{p[0] * maths::common::CTools::fastLog(p[0] / px[0] / py[0]) +
                  p[1] * maths::common::CTools::fastLog(p[1] / px[0] / py[1]) +
                  p[2] * maths::common::CTools::fastLog(p[2] / px[1] / py[0]) +
                  p[3] * maths::common::CTools::fastLog(p[3] / px[1] / py[1])};
        BOOST_REQUIRE_CLOSE_ABSOLUTE(maths::common::CTools::fastLog(2.0), mi / mic_, 1e-3);
    }
}
