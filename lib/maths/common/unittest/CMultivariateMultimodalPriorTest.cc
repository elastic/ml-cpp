...
using TCovariances2 = maths::common::CBasicStatistics::SSampleCovariances<TVector2>;

namespace {
template<std::size_t N>
class CMultivariateMultimodalPriorForTest
    : public maths::common::CMultivariateMultimodalPrior<N> {
public:
    using TClusterer = typename maths::common::CMultivariateMultimodalPrior<N>::TClusterer;
    using TModeVec = typename maths::common::CMultivariateMultimodalPrior<N>::TModeVec;

public:
    CMultivariateMultimodalPriorForTest(const maths::common::CMultivariateMultimodalPrior<N>& prior)
        : maths::common::CMultivariateMultimodalPrior<N>(prior) {}

    const TModeVec& modes() const {
        return this->maths::common::CMultivariateMultimodalPrior<N>::modes();
    }
};

template<std::size_t N>
maths::common::CMultivariateMultimodalPrior<N>
makePrior(maths_t::EDataType dataType, double decayRate = 0.0) {
    maths::common::CXMeansOnline<maths::common::CFloatStorage, N> clusterer(
        dataType, maths_t::E_ClustersFractionWeight, decayRate);
    return maths::common::CMultivariateMultimodalPrior<N>(
        dataType, clusterer,
        maths::common::CMultivariateNormalConjugate<N>::nonInformativePrior(dataType, decayRate),
        decayRate);
}
...
double logLikelihood(const double w[N],
    ...
    for (std::size_t i = 0; i < N; ++i) {
        ...
        TMatrix2 covariance(covariances[i], covariances[i] + 3);
        double ll;
        maths::common::gaussianLogLikelihood(covariance, TVector2(x) - mean, ll);
        lx += w[i] * std::exp(ll);
    }
    return std::log(lx);
}
...
double logLikelihood(const TDoubleVec& w,
    ...
    for (std::size_t i = 0; i < w.size(); ++i) {
        double ll;
        maths::common::gaussianLogLikelihood(TMatrix2(covariances[i]),
...
BOOST_AUTO_TEST_CASE(testMultipleUpdate) {
    ...
    for (size_t i = 0; i < std::size(dataTypes); ++i) {
        ...
        maths::common::CMultivariateMultimodalPrior<2> filter1(makePrior<2>(dataTypes[i]));
        maths::common::CMultivariateMultimodalPrior<2> filter2(filter1);
...
BOOST_AUTO_TEST_CASE(testPropagation) {
    ...
    maths::common::CMultivariateMultimodalPrior<2> filter(
...
BOOST_AUTO_TEST_CASE(testSingleMode) {
    ...
    maths::common::CMultivariateMultimodalPrior<2> filter(makePrior<2>(maths_t::E_ContinuousData));

    for (std::size_t i = 0; i < samples.size(); ++i) {
        filter.addSamples({samples[i]},
                          maths_t::CUnitWeights::singleUnit<TDouble10Vec>(2));
        BOOST_REQUIRE_EQUAL(1, filter.numberModes());
    }
}
...
BOOST_AUTO_TEST_CASE(testMultipleModes) {
    ...
    LOG_DEBUG(<< "Mixture Normals");
    {
        ...
        double differentialEntropy = maths::common::CBasicStatistics::mean(differentialEntropy_);

        for (std::size_t i = 0; i < 10; ++i) {
            rng.random_shuffle(samples.begin(), samples.end());

            maths::common::CMultivariateMultimodalPrior<2> filter1(
                makePrior<2>(maths_t::E_ContinuousData));
            maths::common::CMultivariateNormalConjugate<2> filter2 =
            ...
            LOG_DEBUG(<< "loss1G = " << maths::common::CBasicStatistics::mean(loss1G)
                      << ", loss12 = " << maths::common::CBasicStatistics::mean(loss12)
                      << ", differential entropy " << differentialEntropy);

            BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(loss12) < 0.0);
            BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(loss1G) / differentialEntropy <
                               0.0);
            loss += maths::common::CBasicStatistics::mean(loss1G);
        }
...
BOOST_AUTO_TEST_CASE(testSplitAndMerge) {
    ...
    TDoubleVecVec means(std::size(means_));
    TDoubleVecVecVec covariances(std::size(means_));
    ...
    LOG_DEBUG(<< "Clusters Split and Merge");
    {
        ...
        maths::common::CBasicStatistics::SSampleMean<double>::TAccumulator meanMeanError;
        maths::common::CBasicStatistics::SSampleMean<double>::TAccumulator meanCovError;

        //std::ofstream f;
        //f.open("results.m");
        //std::size_t subplot = 0;
        ...
        for (std::size_t i = 0; i < std::size(n); ++i) {
            ...
            for (std::size_t j = 0; j < modes.size(); ++j) {
                maths::common::CBasicStatistics::COrderStatisticsStack<double, 1> meanError;
                maths::common::CBasicStatistics::COrderStatisticsStack<double, 1> covError;

                if (modes.size() == 1) {
                    meanError.add((TVector2(modes[j].s_Prior->marginalLikelihoodMean()) -
                                   maths::common::CBasicStatistics::mean(totalCovariances))
                                      .euclidean());
                    TMatrix2 mlc(modes[j].s_Prior->marginalLikelihoodCovariance());
                    TMatrix2 tcm = maths::common::CBasicStatistics::covariances(totalCovariances);
                    ...
                    for (std::size_t k = 0; k < std::size(modeCovariances); ++k) {
                        ...
                        covError.add(
                            (TMatrix2(modes[j].s_Prior->marginalLikelihoodCovariance()) -
                             maths::common::CBasicStatistics::covariances(modeCovariances[k]))
                                .frobenius() /
                            maths::common::CBasicStatistics::covariances(modeCovariances[k])
                                .frobenius());
                    }
        ...
        LOG_DEBUG(<< "mean meanError = "
                  << maths::common::CBasicStatistics::mean(meanMeanError));
        LOG_DEBUG(<< "mean covError  = "
                  << maths::common::CBasicStatistics::mean(meanCovError));
        BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanMeanError) < 0.013);
        BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanCovError) < 0.044);
    }
}
...
BOOST_AUTO_TEST_CASE(testMarginalLikelihood) {
    ...
    for (std::size_t t = 0; t < 10; /**/) {
        ...
        for (std::size_t i = 0; i < means.size(); ++i) {
            ...
            for (std::size_t j = 0; j < std::size(intervals); ++j) {
                ...
                double zj;
                maths::common::CIntegration::sparseGaussLegendre<
                    maths::common::CIntegration::OrderSix, maths::common::CIntegration::TwoDimensions>(
    ...
    LOG_DEBUG(<< "Mean Z = " << maths::common::CBasicStatistics::mean(meanZ));
    LOG_DEBUG(<< "Mean mean error = " << maths::common::CBasicStatistics::mean(meanMeanError));
    LOG_DEBUG(<< "Mean covariance error = "
              << maths::common::CBasicStatistics::mean(meanCovarianceError));
    BOOST_REQUIRE_CLOSE_ABSOLUTE(1.0, maths::common::CBasicStatistics::mean(meanZ), 0.1);
    BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanMeanError) < 0.1);
    BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanCovarianceError) < 0.04);
}
    ...
    for (std::size_t i = 0; i < samples.size(); ++i) {
        ...
        double error = (maths::common::CBasicStatistics::mean(expectedMean) -
                        TVector2(filter.marginalLikelihoodMean()))
                           .euclidean() /
                       maths::common::CBasicStatistics::mean(expectedMean).euclidean();
...
BOOST_AUTO_TEST_CASE(testMarginalLikelihoodMode) {
    ...
    for (std::size_t t = 0; t < 50; ++t) {
        ...
        for (std::size_t i = 0; i < filter.modes().size(); ++i) {
    ...
    LOG_DEBUG(<< "mean error = " << maths::common::CBasicStatistics::mean(meanError));
    BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanError) < 0.02);
}
...
BOOST_AUTO_TEST_CASE(testSampleMarginalLikelihood) {
    ...
    TVector2 sampledMean = maths::common::CBasicStatistics::mean(sampledCovariances);
    TMatrix2 sampledCovariance = maths::common::CBasicStatistics::covariances(sampledCovariances);
    LOG_DEBUG(<< "expected mean = " << expectedMean);
    LOG_DEBUG(<< "expected covariance = " << expectedCovariance);
    ...
    for (std::size_t i = 0; i < sampled.size(); ++i) {
        double l1, l2;
        maths::common::gaussianLogLikelihood(covariances[0],
                                             TVector2(sampled[i]) - means[0], l1);
        maths::common::gaussianLogLikelihood(covariances[1],
                                             TVector2(sampled[i]) - means[1], l2);
        modeSampledCovariances[l1 > l2 ? 0 : 1].add(TVector2(sampled[i]));
    }

    for (std::size_t i = 0; i < 2; ++i) {
        TVector2 modeSampledMean =
            maths::common::CBasicStatistics::mean(modeSampledCovariances[i]);
        TMatrix2 modeSampledCovariance =
            maths::common::CBasicStatistics::covariances(modeSampledCovariances[i]);
    ...
    BOOST_REQUIRE_CLOSE_ABSOLUTE(
        ...
        maths::common::CBasicStatistics::count(modeSampledCovariances[0]) /
            maths::common::CBasicStatistics::count(modeSampledCovariances[1]),
        0.05);
}
...
BOOST_AUTO_TEST_CASE(testProbabilityOfLessLikelySamples) {
    ...
    for (std::size_t i = 0; i < std::size(w_); ++i) {
        std::size_t n = (std::size(w_[i]));

        TDoubleVec w(n);
        TDoubleVecVec means(n);
        TDoubleVecVecVec covariances(n);
        for (std::size_t j = 0; j < std::size(w_[i]); ++j) {
            w[j] = w_[i][j];
        ...
        for (std::size_t j = 0; j < means.size(); ++j) {
            ...
            for (std::size_t k = 0; k < std::size(offsets); ++k) {
                ...
                double lb, ub;
                maths::common::CMultivariatePrior::TTail10Vec tail;
            ...
            LOG_DEBUG(<< "mean absolute error = "
                      << maths::common::CBasicStatistics::mean(meanAbsError));
            BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanAbsError) < 0.25);

            LOG_DEBUG(<< "mean relative error = "
                      << maths::common::CBasicStatistics::mean(meanRelError));
            BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanRelError) < 0.6);
        }
    }
}
...
BOOST_AUTO_TEST_CASE(testLatLongData) {
    ...
    std::shared_ptr<maths::common::CMultivariatePrior> modePrior =
        maths::common::CMultivariateNormalConjugateFactory::nonInformative(2, dataType, 0.001);
    std::shared_ptr<maths::common::CMultivariatePrior> filter =
        maths::common::CMultivariateMultimodalPriorFactory::nonInformative(
            2, // dimension
...
BOOST_AUTO_TEST_CASE(testPersist) {
    ...
    maths::common::CMultivariateMultimodalPrior<2> origFilter(makePrior<2>(dataType));
    ...
    core::CJsonStatePersistInserter::persist(
        origJson, std::bind_front(&maths::common::CMultivariateMultimodalPrior<2>::acceptPersistInserter,
                                  &origFilter));
    ...
    maths::common::CMultivariateMultimodalPrior<2> restoredFilter(params, traverser);
    ...
    core::CJsonStatePersistInserter::persist(
        newJson, std::bind_front(&maths::common::CMultivariateMultimodalPrior<2>::acceptPersistInserter,
                                 &restoredFilter));
