...
BOOST_AUTO_TEST_CASE(testCategoricalSampleWithoutReplacement) {
    ...
    using TMatrix = maths::common::CSymmetricMatrixNxN<double, 6>;
    ...
    maths::common::CSampling::categoricalSampleWithoutReplacement(probabilities, 6, samples);
...
BOOST_AUTO_TEST_CASE(testMultinomialSample) {
    ...
    for (std::size_t i = 0; i < n; ++i) {
        maths::common::CSampling::multinomialSampleFast(probabilities, 20, sample);
...
BOOST_AUTO_TEST_CASE(testMultivariateNormalSample) {
...

    {
        ...
        maths::common::CSampling::multivariateNormalSample(m, C, 1000, samples);
...
BOOST_AUTO_TEST_CASE(testReservoirSampling) {
    ...
    using TSampler = maths::common::CSampling::CReservoirSampler<double>;
    using TMeanAccumulator = maths::common::CBasicStatistics::SSampleMean<double>::TAccumulator;
    ...
    for (std::size_t t = 0; t < 100; ++t) {
        ...
        maths::common::CStatisticalTests::CCramerVonMises cvm{20};
    ...
    LOG_DEBUG(<< "mean p-value = " << maths::common::CBasicStatistics::mean(pValue));
    BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(pValue) > 0.3);
}
...
BOOST_AUTO_TEST_CASE(testVectorDissimilaritySampler) {
    ...
    for (std::size_t t = 0; t < 50; ++t) {
        ...
        LOG_TRACE(<< "random mean separation = "
                  << maths::common::CBasicStatistics::mean(randomSeparation) << ", dissimilar mean separation = "
                  << maths::common::CBasicStatistics::mean(dissimilaritySeparation));
        percentageSeparationIncrease.add(
            100.0 *
            (maths::common::CBasicStatistics::mean(dissimilaritySeparation) -
             maths::common::CBasicStatistics::mean(randomSeparation)) /
            maths::common::CBasicStatistics::mean(randomSeparation));
    }
    LOG_DEBUG(<< "% separation increase = "
              << maths::common::CBasicStatistics::mean(percentageSeparationIncrease));
    BOOST_TEST_REQUIRE(
        maths::common::CBasicStatistics::mean(percentageSeparationIncrease) > 50.0);
}
...
BOOST_AUTO_TEST_CASE(testSobolSequenceSampling) {
    {
        ...
        maths::common::CSampling::sobolSequenceSample(dim, n, actual);
        for (std::size_t i = 0; i < n; ++i) {
