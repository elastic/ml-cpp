...
void nearestNeightbours(std::size_t k,
    ...
    using TMinDoublePointPrAccumulator =
        maths::common::CBasicStatistics::COrderStatisticsHeap<TDoublePointPr>;
...
BOOST_AUTO_TEST_CASE(testBuild) {
    ...
    for (std::size_t i = 0; i < numberTests; ++i) {
        ...
        maths::common::CKdTree<TVector2> kdTree;
        kdTree.build(points);
    ...
    for (std::size_t i = 0; i < numberTests; ++i) {
        ...
        maths::common::CKdTree<TVector5> kdTree;
        kdTree.build(points);
        BOOST_TEST_REQUIRE(kdTree.checkInvariants());
    }
}
...
BOOST_AUTO_TEST_CASE(testBuildWithMove) {
    ...
    {
        maths::common::CKdTree<CVectorCountMoves> kdTree;
        kdTree.build(points);
