...
using TPriorPtr = maths::common::CMultivariateOneOfNPrior::TPriorPtr;
using TPriorPtrVec = maths::common::CMultivariateOneOfNPrior::TPriorPtrVec;
using TMeanAccumulator = maths::common::CBasicStatistics::SSampleMean<double>::TAccumulator;

class CMinusLogLikelihood : public maths::common::CGradientDescent::CFunction {
public:
...
private:
    const maths::common::CMultivariateOneOfNPrior* m_Prior;
};
...
template<std::size_t N>
maths::common::CMultivariateOneOfNPrior
makeOneOfN(maths_t::EDataType dataType, double decayRate = 0.0) {
    TPriorPtrVec priors;
    priors.emplace_back(maths::common::CMultivariateNormalConjugate<N>::nonInformativePrior(
                            dataType, decayRate)
                            .clone());
    priors.emplace_back(makeMultimodal<N>(dataType, decayRate).clone());
    return maths::common::CMultivariateOneOfNPrior(N, priors, dataType, decayRate);
}
...
BOOST_AUTO_TEST_CASE(testMultipleUpdate) {
    ...
    for (std::size_t i = 0; i < std::size(dataTypes); ++i) {
        ...
        maths::common::CMultivariateOneOfNPrior filter2(filter1);
...
BOOST_AUTO_TEST_CASE(testPropagation) {
    ...
    maths::common::CMultivariateOneOfNPrior filter(
        makeOneOfN<2>(maths_t::E_ContinuousData, decayRate));
...
BOOST_AUTO_TEST_CASE(testModelUpdate) {
    ...
    for (std::size_t i = 0; i < std::size(dataTypes); ++i) {
        ...
        maths::common::CMultivariateOneOfNPrior oneOfN(makeOneOfN<2>(dataTypes[i]));
...
BOOST_AUTO_TEST_CASE(testMarginalLikelihood) {
    ...
    test::CRandomNumbers rng;
    {
        ...
        for (std::size_t t = 0; t < nt; ++t) {
            ...
            maths::common::CSampling::multivariateNormalSample(mean, covariances, 20, samples);

            maths::common::CMultivariateOneOfNPrior filter(
                makeOneOfN<2>(maths_t::E_ContinuousData));
            ...
            BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanMeanError) < 0.16);
            BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanCovarianceError) < 0.09);
        }
    }
    {
        ...
        LOG_DEBUG(<< "Mean covariance error = "
                  << maths::common::CBasicStatistics::mean(meanCovarianceError));
        BOOST_REQUIRE_CLOSE_ABSOLUTE(1.0, maths::common::CBasicStatistics::mean(meanZ), 0.3);
        BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanMeanError) < 0.1);
        BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanCovarianceError) < 0.16);
    }
}
...
BOOST_AUTO_TEST_CASE(testMarginalLikelihoodMean) {
    ...
    for (std::size_t i = 0; i < sizes.size(); ++i) {
        ...
        maths::common::CMultivariateOneOfNPrior filter(makeOneOfN<2>(maths_t::E_ContinuousData));
...
BOOST_AUTO_TEST_CASE(testMarginalLikelihoodMode) {
...

    {
        ...
        for (std::size_t i = 0; i < std::size(means); ++i) {
            for (std::size_t j = 0; j < std::size(covariances); ++j) {
                ...
                maths::common::CGradientDescent::CEmpiricalCentralGradient gradientOfLikelihood(
                    likelihood, 1e-3);
                maths::common::CGradientDescent gd(learnRates[j], 0.75);
                maths::common::CVector<double> expectedMode;
                TDoubleVec likelihoods;
                gd.run(20, // iterations
                       maths::common::CVector<double>(mean[0], mean[0] + 2),
                       likelihood, gradientOfLikelihood, expectedMode, likelihoods);
...

    {
        ...
        maths::common::CGradientDescent::CEmpiricalCentralGradient gradientOfLikelihood(
            likelihood, 1e-3);
        maths::common::CGradientDescent gd(0.2, 0.75);
        maths::common::CVector<double> expectedMode;
        TDoubleVec likelihoods;
        gd.run(20, // iterations
               maths::common::CVector<double>(means[0], means[0] + 2),
               likelihood, gradientOfLikelihood, expectedMode, likelihoods);
...
BOOST_AUTO_TEST_CASE(testProbabilityOfLessLikelySamples) {
    ...
    for (std::size_t i = 0; i < samples.size(); ++i) {
        ...
        TDoubleVec weights(filter.weights());
        maths::common::CMultivariateOneOfNPrior::TPriorCPtr3Vec models(filter.models());
    ...
    BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(error) < 0.01);
}
...
BOOST_AUTO_TEST_CASE(testPersist) {
    ...
    maths::common::CMultivariateOneOfNPrior origFilter(makeOneOfN<2>(dataType));
    for (std::size_t i = 0; i < samples.size(); ++i) {
    ...
    maths::common::CMultivariateOneOfNPrior restoredFilter(dimension, params, traverser);
