...
using TDoubleDoublePrVec = maths::common::CLowess<2>::TDoubleDoublePrVec;
...
BOOST_AUTO_TEST_CASE(testEdgeCases) {
    ...
    // Check we handle the case that all values are colocated.
    {
        ...
        LOG_DEBUG(<< "minimum = " << lowess.minimum());
...
BOOST_AUTO_TEST_CASE(testSmooth) {
    ...
    LOG_DEBUG(<< "mean error = " << maths::common::CBasicStatistics::mean(errorMoments));

    BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(errorMoments) < 0.1);
}
...
BOOST_AUTO_TEST_CASE(testSmoothPlusNoise) {
    ...
    BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(errorMoments) < 0.8);
...
BOOST_AUTO_TEST_CASE(testTrainingLossCurves) {
    ...
    BOOST_REQUIRE_CLOSE_ABSOLUTE(
        0.0, maths::common::CBasicStatistics::mean(meanRelativeError), 0.25);
}
