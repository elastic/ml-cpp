...
void empiricalProbabilityOfLessLikelySamples(const TDoubleVec& mean,
    ...
    for (std::size_t i = 0; i < samples.size(); ++i) {
        maths::common::gaussianLogLikelihood(
...
void calibrationExperiment() {
    using TVector10 = maths::common::CVectorNx1<double, 10>;
    using TMatrix10 = maths::common::CSymmetricMatrixNxN<double, 10>;
    ...
    maths::common::CMultivariateNormalConjugate<2> filters[] = {
...
BOOST_AUTO_TEST_CASE(testMultipleUpdate) {
    ...
    for (std::size_t i = 0; i < std::size(dataTypes); ++i) {
        ...
        maths::common::CMultivariateNormalConjugate<2> filter2(filter1);
        ...
        BOOST_TEST_REQUIRE(filter1.equalTolerance(
            filter2, maths::common::CToleranceTypes::E_AbsoluteTolerance, 1e-5));
    }
    ...
    for (size_t i = 0; i < std::size(dataTypes); ++i) {
        ...
        BOOST_TEST_REQUIRE(filter1.equalTolerance(
            filter2, maths::common::CToleranceTypes::E_RelativeTolerance, 1e-5));
    }
    ...
    for (std::size_t i = 0; i < std::size(decayRates); ++i) {
        ...
        for (unsigned int t = 0; t < nt; ++t) {
            ...
            Eigen::Matrix<double, 2, 2> denseCovarianceMatrix =
                maths::common::toDenseMatrix(covarianceMatrix); // Get the Eigen matrix
...
BOOST_AUTO_TEST_CASE(testMarginalLikelihood) {
    ...
    for (std::size_t t = 0; t < nt; ++t) {
        ...
        BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanMeanError) < 0.12);
        BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanCovarianceError) < 0.07);
    }
}
...
BOOST_AUTO_TEST_CASE(testSampleMarginalLikelihood) {
    ...
    for (/**/; i < samples.size(); ++i) {
        ...
        TVector2 sampleMean = maths::common::CBasicStatistics::mean(covariances);
        TMatrix2 sampleCov = maths::common::CBasicStatistics::covariances(covariances);
    ...
    LOG_DEBUG(<< "pAbsError = " << maths::common::CBasicStatistics::mean(pAbsError));
    LOG_DEBUG(<< "pRelError = " << maths::common::CBasicStatistics::mean(pRelError));
    BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(pAbsError) < 0.15);
    BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(pRelError) < 0.3);
}
    ...
    for (std::size_t i = 0; i < std::size(means); ++i) {
        ...
        for (std::size_t j = 0; j < std::size(covariances); ++j) {
            ...
            for (std::size_t k = 0; k < std::size(offsets); ++k) {
                ...
                double ll;
                maths::common::gaussianLogLikelihood(TMatrix2(covariance),
            ...
            BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanAbsError) < 0.018);

            LOG_DEBUG(<< "mean relative error = "
                      << maths::common::CBasicStatistics::mean(meanRelError));
            BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanRelError) < 0.15);
        }
    }
}
...
BOOST_AUTO_TEST_CASE(testIntegerData) {
    ...
    for (std::size_t i = 0; i < std::size(means); ++i) {
        ...
        for (std::size_t j = 0; j < std::size(covariances); ++j) {
            ...
            LOG_DEBUG(<< "meanLogLikelihood1 = "
                      << maths::common::CBasicStatistics::mean(meanLogLikelihood1) << ", meanLogLikelihood2 = "
                      << maths::common::CBasicStatistics::mean(meanLogLikelihood2));

            BOOST_REQUIRE_CLOSE_ABSOLUTE(
                maths::common::CBasicStatistics::mean(meanLogLikelihood1),
                maths::common::CBasicStatistics::mean(meanLogLikelihood2), 0.03);
        }
    }
}
...
BOOST_AUTO_TEST_CASE(testPersist) {
    ...
    maths::common::CMultivariateNormalConjugate<2> origFilter(
        maths::common::CMultivariateNormalConjugate<2>::nonInformativePrior(dataType));
    for (std::size_t i = 0; i < samples.size(); ++i) {
    ...
    core::CJsonStatePersistInserter::persist(
        origJson, std::bind(&maths::common::CMultivariateNormalConjugate<2>::acceptPersistInserter,
    ...
    maths::common::CMultivariateNormalConjugate<2> restoredFilter(params, traverser);
    ...
    core::CJsonStatePersistInserter::persist(
        newJson, std::bind(&maths::common::CMultivariateNormalConjugate<2>::acceptPersistInserter,
