...
BOOST_AUTO_TEST_CASE(testNextPow2) {
    ...
    for (std::size_t test = 1u, shift = 1;
        ...
        // Edge cases.
        BOOST_REQUIRE_EQUAL(shift, maths::common::CIntegerTools::nextPow2(test));
        BOOST_REQUIRE_EQUAL(shift, maths::common::CIntegerTools::nextPow2((test << 1) - 1));

        TSizeVec offsets;
        rng.generateUniformSamples(0, test, 100, offsets);
        for (std::size_t i = 0; i < offsets.size(); ++i) {
            BOOST_REQUIRE_EQUAL(
                shift, maths::common::CIntegerTools::nextPow2(test + offsets[i]));
        }
    }
}
...
BOOST_AUTO_TEST_CASE(testReverseBits) {
    ...
    for (std::size_t i = 0; i < values.size(); ++i) {
        ...
        actual = printBits(maths::common::CIntegerTools::reverseBits(x));
        if (i % 500 == 0) {
...
BOOST_AUTO_TEST_CASE(testGcd) {
    ...
    for (std::size_t i = 0; i < 1000; ++i) {
        ...
        LOG_TRACE(<< "big1 = " << big1 << ", big2 = " << big2 << " - expected gcd = " << bigGcd
                  << ", gcd = " << maths::common::CIntegerTools::gcd(big1, big2));
        BOOST_REQUIRE_EQUAL(bigGcd, maths::common::CIntegerTools::gcd(big1, big2));
    }
...
BOOST_AUTO_TEST_CASE(testLcm) {
    ...
    BOOST_REQUIRE_EQUAL(0, maths::common::CIntegerTools::lcm(integers));
    ...
    BOOST_REQUIRE_EQUAL(5, maths::common::CIntegerTools::lcm(integers));
    integers.push_back(0);
    BOOST_REQUIRE_EQUAL(0, maths::common::CIntegerTools::lcm(integers));

    for (std::size_t i = 0; i < 100; ++i) {
        rng.generateUniformSamples(1, 200, 2, integers);
        std::size_t lcm{maths::common::CIntegerTools::lcm(integers[0], integers[1])};
    ...
    for (std::size_t i = 0; i < 100; ++i) {
        ...
        std::size_t lcm{maths::common::CIntegerTools::lcm(integers)};
        BOOST_TEST_REQUIRE(isMultiple(lcm, integers));
    ...
    for (std::size_t i = 0; i < boost::size(n); ++i) {
        for (unsigned int j = 0; j <= n[i]; ++j) {
            ...
            BOOST_REQUIRE_CLOSE_ABSOLUTE(
                expected, maths::common::CIntegerTools::binomial(n[i], j), 1e-10);
        }
    }
}
