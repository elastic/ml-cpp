...
using TXMeans2 = maths::common::CXMeansOnline<double, 2>;
...
class CXMeansOnlineForTest : public maths::common::CXMeansOnline<T, N> {
public:
    using TSizeDoublePr2Vec = typename maths::common::CXMeansOnline<T, N>::TSizeDoublePr2Vec;
    using TPoint = typename maths::common::CXMeansOnline<T, N>::TPointPrecise;
    using TClusterVec = typename maths::common::CXMeansOnline<T, N>::TClusterVec;
    using maths::common::CXMeansOnline<T, N>::add;
...
public:
    CXMeansOnlineForTest(maths_t::EDataType dataType,
        ...
        : maths::common::CXMeansOnline<T, N>(dataType, weightCalc, decayRate, minimumClusterFraction) {
    }

    void add(const TPoint& x, double count = 1.0) {
        TSizeDoublePr2Vec dummy;
        this->maths::common::CXMeansOnline<T, N>::add(x, dummy, count);
    }

    const TClusterVec& clusters() const {
        return this->maths::common::CXMeansOnline<T, N>::clusters();
    }
};
...
BOOST_AUTO_TEST_CASE(testCluster) {
    ...
    TPoint expectedCentre = maths::common::CBasicStatistics::mean(moments);
    double expectedSpread = std::sqrt(
        maths::common::CBasicStatistics::maximumLikelihoodCovariances(moments).trace() / 2.0);
    ...
    TPoint sampleCentre = maths::common::CBasicStatistics::mean(sampleMoments);
    double sampleSpread = std::sqrt(
        maths::common::CBasicStatistics::covariances(sampleMoments).trace() / 2.0);
    ...
    auto split = cluster.split(rng, 5.0, clusterer.indexGenerator());
...
BOOST_AUTO_TEST_CASE(testClusteringVanilla) {
    ...
    for (std::size_t t = 0; t < 10; ++t) {
        ...
        for (std::size_t i = 0; i < clusters.size(); ++i) {
            LOG_DEBUG(<< "moments = "
                      << maths::common::CBasicStatistics::print(clusters[i].covariances()));
            ...
            for (std::size_t j = 0; j < expectedMoments.size(); ++j) {
                meanError.add(
                    (maths::common::CBasicStatistics::mean(clusters[i].covariances()) -
                     maths::common::CBasicStatistics::mean(expectedMoments[j]))
                        .euclidean() /
                    maths::common::CBasicStatistics::mean(expectedMoments[j]).euclidean());
                covError.add(
                    (maths::common::CBasicStatistics::covariances(clusters[i].covariances()) -
                     maths::common::CBasicStatistics::covariances(expectedMoments[j]))
                        .frobenius() /
                    maths::common::CBasicStatistics::covariances(expectedMoments[j])
                        .frobenius());
            }
    ...
    LOG_DEBUG(<< "mean cov error = " << maths::common::CBasicStatistics::mean(meanCovError));
    BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanMeanError) < 0.005);
    BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanCovError) < 0.06);
}
...
BOOST_AUTO_TEST_CASE(testClusteringWithOutliers) {
    ...
    LOG_DEBUG(<< "mean meanError = " << maths::common::CBasicStatistics::mean(meanMeanError));
    LOG_DEBUG(<< "mean covError  = " << maths::common::CBasicStatistics::mean(meanCovError));
    BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanMeanError) < 0.03);
    BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanCovError) < 0.06);
}
...
BOOST_AUTO_TEST_CASE(testManyClusters) {
    ...
    for (std::size_t i = 0; i < samples.size(); ++i) {
        ...
        for (std::size_t j = 0; j < means.size(); ++j) {
            double lj;
            maths::common::gaussianLogLikelihood(covariances[j],
                                                 samples[i] - means[j], lj);
    ...
    for (std::size_t t = 0; t < 5; ++t) {
        ...
        for (std::size_t i = 0; i < samples.size(); ++i) {
            ...
            for (std::size_t j = 0; j < clusters.size(); ++j) {
                double n = maths::common::CBasicStatistics::count(clusters[j].covariances());
                const TPoint& mean =
                    maths::common::CBasicStatistics::mean(clusters[j].covariances());
                const TMatrix& covariance = maths::common::CBasicStatistics::maximumLikelihoodCovariances(
                    clusters[j].covariances());
                double lj;
                maths::common::gaussianLogLikelihood(covariance, samples[i] - mean, lj);
                l += n * std::exp(lj);
            }
            l /= Z;
            loss.add(std::log(lgenerating[i]) - std::log(l));
        }
        LOG_DEBUG(<< "loss = " << maths::common::CBasicStatistics::mean(loss));
        BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(loss) <
                           0.02 * maths::common::CBasicStatistics::mean(differentialEntropy));
    }
}
...
BOOST_AUTO_TEST_CASE(testAdaption) {
    ...
    for (std::size_t i = 0; i < std::size(n); ++i) {
        ...
        for (std::size_t j = 0; j < clusters.size(); ++j) {
            ...
            if (clusters.size() == 1) {
                meanError.add((maths::common::CBasicStatistics::mean(clusters[j].covariances()) -
                               maths::common::CBasicStatistics::mean(totalCovariances))
                                  .euclidean());
                covError.add(
                    (maths::common::CBasicStatistics::covariances(clusters[j].covariances()) -
                ...
                for (std::size_t k = 0; k < std::size(modeCovariances); ++k) {
                    meanError.add(
                        ...
                        maths::common::CBasicStatistics::mean(modeCovariances[k])
                            .euclidean());
                    covError.add(
                        (maths::common::CBasicStatistics::covariances(
                             clusters[j].covariances()) -
    ...
    LOG_DEBUG(<< "mean meanError = " << maths::common::CBasicStatistics::mean(meanMeanError));
    LOG_DEBUG(<< "mean covError  = " << maths::common::CBasicStatistics::mean(meanCovError));
    BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanMeanError) < 0.01);
    BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanCovError) < 0.1);
}
...
BOOST_AUTO_TEST_CASE(testLatLongData) {
    ...
    for (std::size_t i = 0; i < n; ++i) {
        ...
        {
            TPoint mean = maths::common::CBasicStatistics::mean(reference);
            TMatrix covariance = maths::common::CBasicStatistics::covariances(reference);
            double ll;
            maths::common::gaussianLogLikelihood(covariance, x - mean, ll);
            LLR.add(ll);
        }

        {
            ...
            for (std::size_t j = 0; j < clusters.size(); ++j) {
                double w = maths::common::CBasicStatistics::count(clusters[j].covariances());
                TPoint mean = maths::common::CBasicStatistics::mean(clusters[j].covariances());
                TMatrix covariance = maths::common::CBasicStatistics::covariances(
                    clusters[j].covariances());
                double llj;
                maths::common::gaussianLogLikelihood(covariance, x - mean, llj);
                ll += w * std::exp(llj);
                Z += w;
    ...
    LOG_DEBUG(<< "gaussian log(L)  = " << maths::common::CBasicStatistics::mean(LLR));
    LOG_DEBUG(<< "clustered log(L) = " << maths::common::CBasicStatistics::mean(LLC));
    BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(LLC) >
                       0.55 * maths::common::CBasicStatistics::mean(LLR));
}
