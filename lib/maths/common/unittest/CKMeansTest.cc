...
class CKMeansForTest : maths::common::CKMeans<POINT> {
public:
    using TBoundingBox = typename maths::common::CKMeans<POINT>::TBoundingBox;
    using TKdTreeNodeData = typename maths::common::CKMeans<POINT>::CKdTreeNodeData;
    using TDataPropagator = typename maths::common::CKMeans<POINT>::SDataPropagator;
    using TCentreFilter = typename maths::common::CKMeans<POINT>::CCentreFilter;
    using TCentroidComputer = typename maths::common::CKMeans<POINT>::CCentroidComputer;
    using TClosestPointsCollector = typename maths::common::CKMeans<POINT>::CClosestPointsCollector;
};
}
...
using TMean4Accumulator = maths::common::CBasicStatistics::SSampleMean<TVector4>::TAccumulator;
...
struct SKdTreeDataInvariantsChecker {
    ...
    using TMeanAccumulator =
        typename maths::common::CBasicStatistics::SSampleMean<POINT>::TAccumulator;
    using TBoundingBox = typename CKMeansForTest<POINT>::TBoundingBox;

    void operator()(const typename maths::common::CKdTree<POINT, TData>::SNode& node) const {
        ...
        BOOST_REQUIRE_EQUAL(maths::common::CBasicStatistics::print(centroid),
                            maths::common::CBasicStatistics::print(*node.centroid()));
    }
};
...
public:
    ...
    bool operator()(const typename maths::common::CKdTree<POINT, TData>::SNode& node) const {
        using TDoubleSizePr = std::pair<double, std::size_t>;

        m_CentreFilter.prune(node.boundingBox());
        const TSizeVec& filtered = m_CentreFilter.filter();
        maths::common::CBasicStatistics::COrderStatisticsStack<TDoubleSizePr, 2> closest;
...
bool kmeans(const std::vector<POINT>& points, std::size_t iterations, std::vector<POINT>& centres) {
    ...
    for (std::size_t i = 0; i < iterations; ++i) {
        ...
        for (std::size_t j = 0; j < centres.size(); ++j) {
            if (maths::common::CBasicStatistics::mean(centroids[j]) != centres[j]) {
                centres[j] = maths::common::CBasicStatistics::mean(centroids[j]);
                converged = false;
            }
        }
...
double sumSquareResiduals(const TVector2VecVec& points) {
    ...
    for (std::size_t i = 0; i < points.size(); ++i) {
        ...
        m_.add(points[i]);
        TVector2 m = maths::common::CBasicStatistics::mean(m_);
...
BOOST_AUTO_TEST_CASE(testDataPropagation) {
    ...
    for (std::size_t i = 1; i <= 100; ++i) {
        ...
        {
            maths::common::CKdTree<TVector2, CKMeansForTest<TVector2>::TKdTreeNodeData> tree;
            TVector2Vec points;
        ...
        {
            maths::common::CKdTree<TVector4, CKMeansForTest<TVector4>::TKdTreeNodeData> tree;
            TVector4Vec points;
...
BOOST_AUTO_TEST_CASE(testRun) {
    ...
    for (std::size_t t = 1; t <= 100; ++t) {
...

        {
            ...
            maths::common::CKMeans<TVector2> kmeansFast;
...
BOOST_AUTO_TEST_CASE(testRunWithSphericalClusters) {
    ...
    for (std::size_t t = 0; t < 50; ++t) {
        ...
        for (std::size_t i = 0; i < std::size(means); ++i) {
            ...
            maths::common::CSampling::multivariateNormalSample(mean, covariances,
                                                               counts[i], pointsi);
            ...
            TVector2 v = maths::common::CBasicStatistics::variance(moments);
        ...
        maths::common::CKMeans<TSphericalCluster2> kmeansClusters;
...
BOOST_AUTO_TEST_CASE(testPlusPlus) {
    ...
    LOG_DEBUG(<< "# clusters sampled = "
              << maths::common::CBasicStatistics::mean(numberClustersSampled));
    LOG_DEBUG(<< "min ratio  = " << minSSRRatio);
    LOG_DEBUG(<< "mean ratio = " << maths::common::CBasicStatistics::mean(meanSSRRatio));
    LOG_DEBUG(<< "max ratio  = " << maxSSRRatio);

    BOOST_TEST_REQUIRE(minSSRRatio < 0.14);
    BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanSSRRatio) < 0.9);
    BOOST_TEST_REQUIRE(maxSSRRatio < 9.0);
    BOOST_REQUIRE_CLOSE_ABSOLUTE(
        4.0, maths::common::CBasicStatistics::mean(numberClustersSampled), 0.3);
}
