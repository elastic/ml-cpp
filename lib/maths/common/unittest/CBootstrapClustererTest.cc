...
class CBootstrapClustererForTest : public maths::common::CBootstrapClusterer<POINT> {
public:
    using TBoolVec = typename maths::common::CBootstrapClusterer<POINT>::TBoolVec;
    using TSizeVec = typename maths::common::CBootstrapClusterer<POINT>::TSizeVec;
    using TSizeVecVecVec = typename maths::common::CBootstrapClusterer<POINT>::TSizeVecVecVec;
    using TPointVec = typename maths::common::CBootstrapClusterer<POINT>::TPointVec;
    using TGraph = typename maths::common::CBootstrapClusterer<POINT>::TGraph;

public:
    CBootstrapClustererForTest(double overlapThreshold, double chainingFactor)
        : maths::common::CBootstrapClusterer<POINT>(overlapThreshold, chainingFactor) {}
    ...
    void buildClusterGraph(TSizeVecVecVec& bootstrapClusters, TGraph& graph) const {
        ...
        this->maths::common::CBootstrapClusterer<POINT>::buildClusterGraph(
            dummy, bootstrapClusters, graph);
    }

    std::size_t thickets(std::size_t n, const TGraph& graph, TSizeVec& components) const {
        return this->maths::common::CBootstrapClusterer<POINT>::thickets(n, graph, components);
    }

    bool separate(TGraph& graph, TBoolVec& parity) const {
        return this->maths::common::CBootstrapClusterer<POINT>::separate(graph, parity);
    }
    ...
    bool cutSearch(std::size_t u,
        ...
        return this->maths::common::CBootstrapClusterer<POINT>::cutSearch(
            u, v, graph, threshold, cost, parities);
    }

    TSizeVec& offsets() {
        return this->maths::common::CBootstrapClusterer<POINT>::offsets();
    }
};
...
BOOST_AUTO_TEST_CASE(testFacade) {
    ...
    for (std::size_t t = 0; t < 10; ++t) {
        ...
        TVector2Vec points1;
        maths::common::CSampling::multivariateNormalSample(m1, v1, 50, points1);
        double m2_[] = {10.0, 5.0};
        double v2_[] = {4.0, 0.0, 1.0};
        TVector2 m2(&m2_[0], &m2_[2]);
        TMatrix2 v2(&v2_[0], &v2_[3]);
        TVector2Vec points2;
        maths::common::CSampling::multivariateNormalSample(m2, v2, 50, points2);
        TVector2Vec points;
...

        {
            ...
            maths::common::CBootstrapClustererFacade<
                maths::common::CXMeans<TVector2, maths::common::CGaussianInfoCriterion<TVector2, maths::common::E_BIC>>>
                clusterer(xmeans, improveParamsKmeansIterations,
...
BOOST_AUTO_TEST_CASE(testCutSearch) {
    ...
    LOG_DEBUG(<< "quality = " << 1.0 - maths::common::CBasicStatistics::mean(quality));
    BOOST_TEST_REQUIRE(1.0 - maths::common::CBasicStatistics::mean(quality) > 0.98);
}
...
BOOST_AUTO_TEST_CASE(testSeparate) {
    ...
    BOOST_TEST_REQUIRE(1.0 - maths::common::CBasicStatistics::mean(quality) > 0.99);
}
...
BOOST_AUTO_TEST_CASE(testThickets) {
    ...
    for (std::size_t t = 0; t < trials; ++t) {
        ...
        if (c == 3) {
            ...
            for (std::size_t i = 0; i < 3; ++i) {
                double jaccard = maths::common::CSetTools::jaccard(
    ...
    LOG_DEBUG(<< "mean Jaccard = " << maths::common::CBasicStatistics::mean(meanJaccard));
    BOOST_TEST_REQUIRE(error < 2);
    BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanJaccard) > 0.99);
}
...
BOOST_AUTO_TEST_CASE(testNonConvexClustering) {
    ...
    for (std::size_t t = 0; t < 10; ++t) {
        ...
        maths::common::bootstrapCluster(flatPoints,
                                        20, // trials
        ...
        for (std::size_t i = 0; i < perfect.size(); ++i) {
            ...
            for (std::size_t j = 0; j < bootstrap.size(); ++j) {
                jmax = std::max(jmax, maths::common::CSetTools::jaccard(
    ...
    LOG_DEBUG(<< "Jaccard bootstrap to perfect = "
              << maths::common::CBasicStatistics::mean(jaccardBootstrapToPerfect));
    LOG_DEBUG(<< "Jaccard vanilla to perfect   = "
              << maths::common::CBasicStatistics::mean(jaccardVanillaToPerfect));
    LOG_DEBUG(<< "# clusters bootstrap = "
              << maths::common::CBasicStatistics::mean(numberClustersBootstrap));
    LOG_DEBUG(<< "# clusters vanilla   = "
              << maths::common::CBasicStatistics::mean(numberClustersVanilla));

    BOOST_REQUIRE_CLOSE_ABSOLUTE(
        1.0, maths::common::CBasicStatistics::mean(jaccardBootstrapToPerfect), 0.1);
    BOOST_REQUIRE_CLOSE_ABSOLUTE(
        3.0, maths::common::CBasicStatistics::mean(numberClustersBootstrap), 0.6);
    BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(jaccardBootstrapToPerfect) >
                       maths::common::CBasicStatistics::mean(jaccardVanillaToPerfect));
}
...
BOOST_AUTO_TEST_CASE(testClusteringStability) {
    ...
    for (std::size_t t = 0; t < 10; ++t) {
        ...
        maths::common::CXMeans<TVector2, maths::common::CGaussianInfoCriterion<TVector2, maths::common::E_BIC>> xmeans(
            20);
        maths::common::bootstrapCluster(points,
                                        20, // trials
                                        xmeans,
                                        4,   // improve params
        ...
        if (bootstrapClusters.size() > 1) {
            ...
            for (std::size_t i = 0; i < bootstrap.size(); ++i) {
                ...
                for (std::size_t j = 0; j < perfect.size(); ++j) {
                    double J = maths::common::CSetTools::jaccard(
    ...
    LOG_DEBUG(<< "mean = " << maths::common::CBasicStatistics::mean(meanConsistency));
    BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanConsistency) > 0.95);
}
