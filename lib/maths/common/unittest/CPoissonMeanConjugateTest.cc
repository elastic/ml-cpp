...
BOOST_AUTO_TEST_CASE(testMarginalLikelihood) {
...

    {
        ...
        BOOST_TEST_REQUIRE(std::size(shapes) == std::size(rates));
        ...
        for (std::size_t i = 0; i < std::size(shapes); ++i) {
            CPoissonMeanConjugate filter(
                maths::common::CPoissonMeanConjugate(0.0, shapes[i], rates[i]));

            for (std::size_t j = 0; j < std::size(sampleStds); ++j) {
                double mean = filter.marginalLikelihoodMean();
                unsigned int sample =
...

    {
        ...
        double expectedDifferentialEntropy =
            maths::common::CTools::differentialEntropy(poisson);
...
BOOST_AUTO_TEST_CASE(testMarginalLikelihoodVariance) {
    ...
    for (std::size_t i = 0; i < std::size(rates); ++i) {
        ...
        BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(relativeError) < 0.16);
    }
}
    ...
    for (std::size_t i = 0; i < std::size(rates); ++i) {
        ...
        for (std::size_t j = 0; j < samples.size(); ++j) {
            ...
            LOG_DEBUG(<< "expectedMean = " << filter.marginalLikelihoodMean() << ", sampledMean = "
                      << maths::common::CBasicStatistics::mean(sampledMomemts));
            LOG_DEBUG(<< "expectedMean = " << filter.marginalLikelihoodVariance() << ", sampledVariance = "
                      << maths::common::CBasicStatistics::variance(sampledMomemts));

            BOOST_REQUIRE_CLOSE_ABSOLUTE(
                filter.marginalLikelihoodMean(),
                maths::common::CBasicStatistics::mean(sampledMomemts), 1e-8);
            BOOST_REQUIRE_CLOSE_ABSOLUTE(
                filter.marginalLikelihoodVariance(),
                maths::common::CBasicStatistics::variance(sampledMomemts),
        ...
        BOOST_TEST_REQUIRE(maths::common::CBasicStatistics::mean(meanVarError) < 0.05);
    }
}
...
BOOST_AUTO_TEST_CASE(testAnomalyScore) {
    ...
    for (std::size_t i = 0; i < std::size(processRates); ++i) {
...
BOOST_AUTO_TEST_CASE(testPersist) {
    ...
    maths::common::CPoissonMeanConjugate origFilter(
        CPoissonMeanConjugate::nonInformativePrior());
    for (std::size_t i = 0; i < samples.size(); ++i) {
    ...
    maths::common::CPoissonMeanConjugate restoredFilter(params, traverser);
...
BOOST_AUTO_TEST_CASE(testNegativeSample) {
    ...
    using TEqual = maths::common::CEqualWithTolerance<double>;
    TEqual equal(maths::common::CToleranceTypes::E_RelativeTolerance, 0.002);
    BOOST_TEST_REQUIRE(filter1.equalTolerance(filter2, equal));
}
...
BOOST_AUTO_TEST_CASE(testBadValues) {
    ...
    TUIntVec samples;
    rng.generatePoissonSamples(4.0, 100, samples);

    CPoissonMeanConjugate filter = CPoissonMeanConjugate::nonInformativePrior();

    for (auto& sample : samples) {
        filter.addSamples(static_cast<double>(sample));
    }
