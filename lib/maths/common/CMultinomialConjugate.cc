...
public:
    ...
    void dump(TDoubleVec& lowerBounds, TDoubleVec& upperBounds) const {
        ...
        for (std::size_t i = 1; i < m_Cdf.size() - 1; ++i) {
...
public:
    ...
    void dump(TDoubleVec& lowerBounds, TDoubleVec& upperBounds) const {
        ...
        for (std::size_t i = 1; i < m_CdfComplement.size() - 1; ++i) {
...
void generateBetaSamples(double a, double b, std::size_t numberSamples, TDouble7Vec& samples) {
    ...
    try {
        ...
        double f = 0.0;
        mean /= dq;
        for (std::size_t i = 1; i < numberSamples; ++i, q += dq) {
...
using TDoubleDoubleSizeTr = std::tuple<double, double, std::size_t>;
...
void CMultinomialConjugate::addSamples(const TDouble1Vec& samples,
    ...
    LOG_TRACE(<< "samples = " << samples << ", m_NumberAvailableCategories = " << m_NumberAvailableCategories
              << ", m_Categories = " << m_Categories << ", m_Concentrations = " << m_Concentrations
...
void CMultinomialConjugate::propagateForwardsByTime(double time) {
    ...
    LOG_TRACE(<< "factor = " << factor << ", m_Concentrations = " << m_Concentrations
              << ", m_TotalConcentration = " << m_TotalConcentration
...
double CMultinomialConjugate::marginalLikelihoodMode(const TDoubleWeightsAry& /*weights*/) const {
    ...
    std::size_t mode = 0;
    for (std::size_t i = 1; i < m_Concentrations.size(); ++i) {
...
CMultinomialConjugate::marginalLikelihoodConfidenceInterval(double percentage,
    ...
    LOG_TRACE(<< "quantiles = " << quantiles);
    LOG_TRACE(<< "            " << m_Categories);
...
std::string CMultinomialConjugate::printJointDensityFunction() const {
    ...
    for (std::size_t i = 0; i < m_Concentrations.size(); ++i) {
        ...
        result << "x = [";
        for (unsigned int j = 0; j < POINTS; ++j, x += xIncrement) {
            result << x << " ";
        }
...
void CMultinomialConjugate::removeCategories(TDoubleVec categoriesToRemove) {
    ...
    categoriesToRemove.push_back(std::numeric_limits<double>::max());
    ...
    LOG_TRACE(<< "concentrations = " << m_Concentrations);
...
void CMultinomialConjugate::probabilitiesOfLessLikelyCategories(maths_t::EProbabilityCalculation calculation,
    ...
    case maths_t::E_TwoSided: {
        ...
        for (std::size_t i = 0; i < pCategories.size(); ++i) {
            ...
            if (std::get<0>(pCategories[i]) != pLast) {
                TDouble7Vec samples;
                double a = m_Concentrations[j];
                double b = m_TotalConcentration - m_Concentrations[j];
...
bool CMultinomialConjugate::equalTolerance(const CMultinomialConjugate& rhs,
    ...
    LOG_DEBUG(<< m_Categories << " " << rhs.m_Categories);
    LOG_DEBUG(<< m_Concentrations << " " << rhs.m_Concentrations);
