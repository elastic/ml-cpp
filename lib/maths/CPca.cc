                                                ...
                                                TDenseVectorVec &projected)
{
    ...
    std::size_t size{std::accumulate(data.begin(), data.end(), std::size_t(0),
                                     [](std::size_t size_,
                                        const TIntDoublePrVec &vector)
                                     { return size_ + vector.size(); })};
    projectOntoPrincipleComponentsRandom(numberComponents, 3 * size,
                                         dimension, support, data, projected);
}
                                                ...
                                                TDenseVectorVec &projected)
{
    ...
    std::ptrdiff_t numberColumns{
        std::max(std::count_if(
                     columnProbabilities.begin(), columnProbabilities.end(),
                     [dimension](double p) { return p > 0.05 / static_cast<double>(dimension); }),
    ...
    else
    {
        ...
        for (std::size_t trial = 0u; trial < 50; ++trial)
        {
            ...
            CSampling::categoricalSampleWithoutReplacement(rng, probabilities, numberColumns, columns);
            std::for_each(columns.begin(), columns.end(),
                          [&columnWeights](std::size_t i) { columnWeights[i] += 1.0; });
        }
        std::sort(columns.begin(), columns.end());
        std::for_each(columnWeights.begin(), columnWeights.end(),
                      [Z](double &weight) { weight = std::sqrt((weight - 1.0) / 50.0); });
    }
    ...
    {
        projected.push_back(
                svd.matrixV().leftCols(numberComponents).transpose() * toDense(columns, vector));
    }
}
...
double CPca::centredColumnNorm2(std::size_t i,
...
CPca::TDenseVector CPca::toDense(const TSizeVec &columns,
                                 const TIntDoublePrVec &x)
{
