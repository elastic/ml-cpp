...
uint32_t CTools::category32(const std::string &value) { return category32(HASHER(value)); }
...
std::size_t CTools::category64(const std::string &value) { return HASHER(value); }

double CTools::interpolate(double a, double b, double pa, double pb, double x) {
    return maths::CTools::truncate(
        pa + (pb - pa) * (x - a) / (b - a), std::min(pa, pb), std::max(pa, pb));
}

double CTools::powInterpolate(double p, double a, double b, double pa, double pb, double x) {
    return maths::CTools::truncate(
        pa + (pb - pa) * ::pow((x - a) / (b - a), p), std::min(pa, pb), std::max(pa, pb));
}
...
double CTools::logInterpolate(double a, double b, double pa, double pb, double x) {
    ...
    return maths::CTools::truncate(
        pa + (pb - pa) * (lx - la) / (lb - la), std::min(pa, pb), std::max(pa, pb));
}
...
std::string CTools::prettyPrint(double d) {
    ...
    if (::fabs(d) <= 1e-3) {
        ...
        for (char *pos = end; pos - buf > 3 && std::isdigit(static_cast<unsigned char>(pos[-4]));
             pos -= 3, ++end) {
...
std::string CTools::prettyPrint(core_t::TTime time) {
    ...
    core_t::TTime intervals[] = {
        (time / 604800), (time / 86400) % 7, (time / 3600) % 24, (time / 60) % 60, time % 60};
    for (std::size_t i = 0u; i < boost::size(intervals); ++i) {
        if (intervals[i] != 0) {
            result += (result.empty() ? "" : " ") + core::CStringUtils::typeToString(intervals[i]) +
                      SUFFIXES[i];
        }
    }
    return result;
