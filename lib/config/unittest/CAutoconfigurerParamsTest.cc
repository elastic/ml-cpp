...
void CAutoconfigurerParamsTest::testInit(void) {
    ...
    std::string expected =
        "  TimeFieldName = time\n"
        "  TimeFieldFormat = \n"
        "  FieldsOfInterest = [performance_metric, performance_metric_name, machine, user, region, "
        "program]\n"
        "  FieldsToUseInAutoconfigureByRole[constants::ARGUMENT_INDEX] = \"null\"\n"
        "  FieldsToUseInAutoconfigureByRole[constants::BY_INDEX] = [performance_metric_name, "
        "program]\n"
