...
void CAutoconfigurerParamsTest::testDefaults() {
    ...
    std::string expected =
        ...
        "  FieldsToUseInAutoconfigureByRole[constants::ARGUMENT_INDEX] = "
        "\"null\"\n"
        "  FieldsToUseInAutoconfigureByRole[constants::BY_INDEX] = \"null\"\n"
        "  FieldsToUseInAutoconfigureByRole[constants::OVER_INDEX] = \"null\"\n"
        "  FieldsToUseInAutoconfigureByRole[constants::PARTITION_INDEX] = "
        "\"null\"\n"
        "  FunctionCategoriesToConfigure = [count, rare, distinct_count, "
        "info_content, mean, min, max, sum, varp, median]\n"
        "  FieldDataType = []\n"
        ...
        "  CandidateBucketLengths = [60, 300, 600, 1800, 3600, 7200, 14400, "
        "86400]\n"
...
void CAutoconfigurerParamsTest::testInit() {
    ...
    std::string expected =
        "  TimeFieldName = time\n"
        "  TimeFieldFormat = \n"
        "  FieldsOfInterest = [performance_metric, performance_metric_name, "
        "machine, user, region, program]\n"
        "  FieldsToUseInAutoconfigureByRole[constants::ARGUMENT_INDEX] = "
        "\"null\"\n"
        "  FieldsToUseInAutoconfigureByRole[constants::BY_INDEX] = "
        "[performance_metric_name, program]\n"
        "  FieldsToUseInAutoconfigureByRole[constants::OVER_INDEX] = [user]\n"
        "  FieldsToUseInAutoconfigureByRole[constants::PARTITION_INDEX] = "
        "[user, machine, region]\n"
        "  FunctionCategoriesToConfigure = [count, min, max, median]\n"
        "  FieldDataType = [(machine,categorical), "
