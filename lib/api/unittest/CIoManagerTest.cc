...
void testCommon(const std::string& inputFileName,
...

    {
        ml::core::CBlockingCallCancellingTimer cancellerThread{
            ml::core::CThread::currentThreadId(), std::chrono::seconds{10}};
            ...
            do {
                ...
                if (ioMgr.inputStream().gcount() > 0) {
                    processedData.append(
                        buffer, static_cast<std::size_t>(ioMgr.inputStream().gcount()));
                }
...
BOOST_AUTO_TEST_CASE(testStdinStdout) {
    ...
    ml::api::CIoManager ioMgr{cancellerThread, "", false, "", false};
...
BOOST_AUTO_TEST_CASE(testFileIoGood) {
    ...
    std::remove(GOOD_OUTPUT_FILE_NAME);
    ...
    BOOST_REQUIRE_EQUAL(0, std::remove(GOOD_INPUT_FILE_NAME));
    BOOST_REQUIRE_EQUAL(0, std::remove(GOOD_OUTPUT_FILE_NAME));
}
