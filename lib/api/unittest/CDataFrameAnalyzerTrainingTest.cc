...
void testOneRunOfBoostedTreeTrainingWithStateRecovery(
    ...
    const json::object& expectedHyperparameters = expectedResults.at("hyperparameters").as_object();
    ...
    for (const auto& key : maths::analytics::CBoostedTreeHyperparameters::names()) {
        if (expectedHyperparameters.contains(key)) {
            double expected{std::stod(expectedHyperparameters.at(key).as_object().at("value").as_string().c_str())};
            double actual{std::stod(actualHyperparameters.at(key).as_object().at("value").as_string().c_str())};
...
void readIncrementalTrainingState(const std::string& resultsJson,
    ...
    for (const auto& result_ : results.as_array()) {
        ...
        if (result.contains("compressed_inference_model")) {
            ...
            json::error_code ec;
            for (const auto& item : result_.at_pointer("/model_metadata/hyperparameters").as_array()) {
                if (std::strcmp(item.at("name").as_string().c_str(), "alpha") == 0) {
                    alpha = item.at("value").to_number<double>(ec);
                    BOOST_REQUIRE(ec.failed() == false);
                } else if (std::strcmp(item.at("name").as_string().c_str(), "lambda") == 0) {
                    lambda = item.at("value").to_number<double>(ec);
                    BOOST_REQUIRE(ec.failed() == false);
                } else if (std::strcmp(item.at("name").as_string().c_str(), "gamma") == 0) {
                    gamma = item.at("value").to_number<double>(ec);
                    BOOST_REQUIRE(ec.failed() == false);
                } else if (std::strcmp(item.at("name").as_string().c_str(), "soft_tree_depth_limit") == 0) {
                    softTreeDepthLimit = item.at("value").to_number<double>(ec);
                    BOOST_REQUIRE(ec.failed() == false);
                } else if (std::strcmp(item.at("name").as_string().c_str(),
                                       "soft_tree_depth_tolerance") == 0) {
                    softTreeDepthTolerance = item.at("value").to_number<double>(ec);
                    BOOST_REQUIRE(ec.failed() == false);
                } else if (std::strcmp(item.at("name").as_string().c_str(), "eta") == 0) {
                    ...
                    BOOST_REQUIRE(ec.failed() == false);
                } else if (std::strcmp(item.at("name").as_string().c_str(), "downsample_factor") == 0) {
                    downsampleFactor = item.at("value").to_number<double>(ec);
                    BOOST_REQUIRE(ec.failed() == false);
                } else if (std::strcmp(item.at("name").as_string().c_str(), "feature_bag_fraction") == 0) {
                    featureBagFraction = item.at("value").to_number<double>(ec);
                    BOOST_REQUIRE(ec.failed() == false);
                }
            }
            if (result.at("model_metadata").as_object().contains("train_properties") &&
                result_.at_pointer("/model_metadata/train_properties").as_object().contains("loss_gap")) {
                lossGap =
                    result_.at_pointer("/model_metadata/train_properties/loss_gap").to_number<double>(ec);
                BOOST_REQUIRE(ec.failed() == false);
            }
        }
    }
    ...
    for (const auto& result_ : results.as_array()) {
        ...
        if (result.contains("row_results")) {
            predictions.emplace_back(
                result_.at_pointer("/row_results/results/ml/" + targetPredictionName).as_double());
        }
    }
}
}
...
BOOST_AUTO_TEST_CASE(testMemoryLimitHandling) {
    ...
    for (const auto& result_ : results.as_array()) {
        ...
        if (result.contains("analytics_memory_usage")) {
            std::string status{result_.at_pointer("/analytics_memory_usage/status").as_string()};
            if (status == "ok") {
                memoryStatusOk = true;
                ...
                if (result_.at_pointer("/analytics_memory_usage").as_object().contains("memory_reestimate_bytes") &&
                    result_.at_pointer("/analytics_memory_usage/memory_reestimate_bytes").as_int64() > 0) {
...
BOOST_AUTO_TEST_CASE(testRegressionTrainingWithRowsMissingTargetValue) {
    ...
    for (const auto& result_ : results.as_array()) {
        ...
        if (result.contains("row_results")) {
            json::error_code ec;
            std::size_t index(result_.at_pointer("/row_results/checksum").to_number<std::size_t>(ec));
            BOOST_REQUIRE(ec.failed() == false);
...
//BOOST_AUTO_TEST_CASE(testRegressionTrainingWithStateRecovery, * utf::disabled()) { // TODO
//
