...
CAnomalyJob::CAnomalyJob(const std::string &jobId,
    ...
    : m_JobId(jobId),
      ...
      m_ModelPlotQueue(m_ModelConfig.bucketResultsDelay(),
                       this->effectiveBucketLength(),
                       0)
{
...
bool CAnomalyJob::handleRecord(const TStrStrUMap &dataRowFields)
{
    ...
    if (time < m_LastFinalisedBucketEndTime)
    {
        ...
        ss << "Records must be in ascending time order. "
           << "Record '" << this->debugPrintRecord(dataRowFields)
...
void CAnomalyJob::writeOutResults(bool interim, model::CHierarchicalResults &results,
                                       ...
                                       uint64_t sumPastProcessingTime)
{
    if (!results.empty())
    {
        ...
        if (m_JsonOutputWriter.endOutputBatch(interim,
                    sumPastProcessingTime + processingTime) == false)
        {
                                       ...
                                       core::CStateRestoreTraverser &traverser)
{
    ...
    LOG_DEBUG("Restoring state for detector with key '"
              << key.debug() << '/' << partitionFieldValue << '\'');
...
bool CAnomalyJob::backgroundPersistState(CBackgroundPersister &backgroundPersister)
{
    ...
    if (backgroundPersister.addPersistFunc(boost::bind(&CAnomalyJob::runBackgroundPersist,
                                                       this,
                                                       args,
                                                       _1)) == false)
    {
                               ...
                               core::CDataAdder &persister)
{
    ...
    try
    {
        ...
        core::CDataAdder::TOStreamP strm = compressor.addStreamed(ML_STATE_INDEX,
                                                                  m_JobId + '_' + STATE_TYPE + '_' + snapShotId);
        if (strm != 0)
        {
...
void CAnomalyJob::outputResultsWithinRange(bool isInterim,
                                           core_t::TTime start,
                                           core_t::TTime end)
{
    if (m_LastFinalisedBucketEndTime <= 0)
