                      ...
                      ENUM &property)
{
    if (propertyName == enumPropertyName)
    {
        ...
        if (index != cases.end())
        {
            property = static_cast<typename SStripOptional<ENUM>::Type>(index - cases.begin());
        }
        else
        {
...
const TStrVec ALGORITHMS{"automatic", "kmeans", "xmeans",
                         "random_projection_kmeans",
                         "random_projection_xmeans", "agglomerative"};
const TStrVec IMPUTATION_METHODS{"random", "nn_plain",
                                 "nn_bagged_samples",
...
CClusterer::CClusterer(const CParams &params,
                       CClustererOutputWriter &resultWriter) :
        m_Params(params),
...
bool CClusterer::handleRecord(const TStrStrUMap &fields)
{
    ...
    for (const auto &field : fields)
    {
        ...
        {
            cluster.reset(m_ToCluster.emplace(m_Dictionary.word(value),
                                              m_ToClusterNames.size()).first->second);
        ...
        {
            feature.reset(m_Dimensions.emplace(m_Dictionary.word(value),
                                               m_DimensionNames.size()).first->second);
        ...
        else
        {
            ...
            if (!value.empty())
            {
                ...
                {
                    LOG_ERROR("Ignoring bad value for '" << name << "'" << ": '" << value << "'");
                    continue;
                }
                                                ...
                                                TDenseVectorVec &vectors) const
{
    ...
    {
        LOG_DEBUG("Projecting data onto "
                  << *m_Params.principleComponents() << " principle components");
        maths::CPca::projectOntoPrincipleComponents(*m_Params.principleComponents(), inliers, vectors);
    }
}
                                ...
                                TSizeVec &clusterIds) const
{
    ...
    {
        vectors_[vector.annotation()] =
                std::move(static_cast<TDenseVector&>(vector));
    }
                               ...
                               TSizeVec &clusterIds) const
{
    ...
    using TKMeansInitializer = maths::CKMeansPlusPlusInitialization<TAnnotatedDenseVector,
...
std::size_t CClusterer::xmeans(std::size_t max,
                               ...
                               TSizeVec &clusterIds) const
{
    using TXMeans = maths::CXMeans<TAnnotatedDenseVector,
                                   maths::CGaussianInfoCriterion<TAnnotatedDenseVector, maths::E_BIC>>;
    ...
    for (std::size_t i = 0u, s = 0u; i < dimension; ++i)
    {
        for (std::size_t j = 0u; j < i; ++j, ++s)
        {
            for (std::size_t k = 0u; k < clusters.size(); ++k)
            {
                ...
                maths::CClusterEvaluation::silhouetteApprox(projections, clusterSilhouettes, errors);
        ...
        for (std::size_t u = 0u; u < dimension; ++u)
        {
            if (std::find(C.begin(), C.end(), u) == C.end())
            {
                ...
                for (auto v : C)
                {
                   ...
                   {
                       trial[cluster] =  maths::CBasicStatistics::mean(trial[cluster])
                                       < maths::CBasicStatistics::mean(silhouettes[k][cluster]) ?
                                       silhouettes[k][cluster] : trial[cluster];
                   }
                }
                if (less(max, trial))
                {
                              ...
                              TDoubleVec &silhouettes) const
{
    ...
    for (std::size_t i = 0u; i < m_ToClusterNames.size(); ++i)
    {
        ...
        m_ResultWriter.addMember(OUTLIER, std::binary_search(outliers.begin(), outliers.end(), i));
...
bool CClusterer::CParams::init(const std::string &configFile)
{
    ...
    for (const auto &property : properties)
    {
        ...
        if (readProperty("treat_missing_as_zero",
                         propertyName, propertyValue, m_TreatMissingAsZero))
        {
            continue;
        }
        if (readProperty("compute_outlier_factors",
                         propertyName, propertyValue, m_ComputeOutlierFactors))
        {
            continue;
        }
        ...
        if (readProperty("number_of_clusters",
                         propertyName, propertyValue, m_NumberOfClusters))
        {
            continue;
        }
        if (readProperty("maximum_number_of_clusters",
                         propertyName, propertyValue, m_MaximumNumberOfClusters))
        {
            continue;
        }
        if (readProperty("principle_components",
                         propertyName, propertyValue, m_PrincipleComponents))
        {
            continue;
        }
    }
