...
public:
    CKillAndReapGuard(sandbox2::Sandbox2* sandbox,
                       CSandboxedProcessSpawner::TAwaitResultFn awaitResultFn)
        : m_Sandbox{sandbox}, m_AwaitResultFn{std::move(awaitResultFn)} {}

    ~CKillAndReapGuard() {
        if (m_Armed && m_Sandbox != nullptr) {
            try {
                m_Sandbox->Kill();
                if (m_AwaitResultFn) {
...
bool CSandboxedProcessSpawner::spawn(const std::string& processPath,
    ...
    // E_IdentityCaptured (LI1): arm the kill-and-reap guard on the raw
    // pointer *before* the shared_ptr conversion below, which is itself a
    // potentially-throwing allocation (MG3's control-block allocation
    // failure) as well as before registry insertion, monitor-thread
    // construction, and detach(). If the shared_ptr constructor throws, the
    // standard guarantees it has no effect on the moved-from unique_ptr, so
    // sandboxPtr (and therefore the raw pointer the guard holds) still owns
    // a live object the guard can Kill()/await.
    CKillAndReapGuard killAndReapGuard{sandboxPtr.get(), m_AwaitResultFn};

    std::shared_ptr<sandbox2::Sandbox2> sandbox;
    try {
        sandbox = std::shared_ptr<sandbox2::Sandbox2>(std::move(sandboxPtr));
    } catch (const std::exception& e) {
        LOG_ERROR(<< "Failed to take shared ownership of a launched sandboxee for " << processPath
                  << ": " << e.what());
        childPid = 0;
    ...
    const bool monitorStarted{m_MonitorLaunchFn ? m_MonitorLaunchFn(std::move(monitorBody))
    ...
    if (monitorStarted == false) {
        ...
        // both. Drop the entry this call inserted (matching by generation,
        // in case a racing call already replaced it), then let
        // killAndReapGuard's destructor Kill()/await the sandboxee.
        {
