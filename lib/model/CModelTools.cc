...
std::size_t CModelTools::CFuzzyDeduplicate::duplicate(core_t::TTime time, TDouble2Vec value)
{
    return !m_Quantize ?
            m_Count++ :
            m_QuantizedValues.emplace(boost::unordered::piecewise_construct,
                                     std::forward_as_tuple(this->quantize(time),
                                                           this->quantize(value)),
                                     std::forward_as_tuple(m_QuantizedValues.size())).first->second;
}
...
CModelTools::TDouble2Vec CModelTools::CFuzzyDeduplicate::quantize(TDouble2Vec value) const
{
    for (std::size_t i = 0u; i < value.size(); ++i)
    {
        value[i] = m_ValueEps[i] > 0.0 ?
                   m_ValueEps[i] * std::floor(value[i] / m_ValueEps[i]) : value[i];
    }
    return value;
}
...
bool CModelTools::CProbabilityAggregator::calculate(double &result) const
{
    ...
    case E_Sum:
    {
        ...
        for (const auto &aggregator : m_Aggregators)
        {
            if (!boost::apply_visitor(boost::bind<bool>(
    ...
    case E_Min:
    {
        ...
        {
            if (!boost::apply_visitor(boost::bind<bool>(
                    SReadProbability(), boost::ref(p_), _1), aggregator.first))
            {
                return false;
            }
        }
...
void CModelTools::CProbabilityCache::addProbability(model_t::EFeature feature, std::size_t id,
                                                    const TDouble2Vec1Vec &value,
                                                    double probability, const TTail2Vec &tail,
                                                    bool conditional, const TSize1Vec &mostAnomalousCorrelate)
{
