...
BOOST_FIXTURE_TEST_CASE(testRestoreWithConsistentTokenIdSucceeds, CTestFixture) {
    // Sanity check that the token-ID range validation added for issue #2875 does
    // not reject a valid, consistent state document. Same shape as the test
    // above, but the category references the only valid token ID (0).
