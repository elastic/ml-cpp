...
void CAnomalyDetectorModelConfigTest::testNormal(void) {
    {
        ...
        CPPUNIT_ASSERT_EQUAL(2.0,
                             config.factory(1, INDIVIDUAL_COUNT)
                                 ->modelParams()
                                 .s_InitialDecayRateMultiplier);
        CPPUNIT_ASSERT_EQUAL(2.0,
                             config.factory(1, INDIVIDUAL_METRIC)
                                 ->modelParams()
                                 .s_InitialDecayRateMultiplier);
        CPPUNIT_ASSERT_EQUAL(2.0,
                             config.factory(1, POPULATION_COUNT)
                                 ->modelParams()
                                 .s_InitialDecayRateMultiplier);
        CPPUNIT_ASSERT_EQUAL(2.0,
                             config.factory(1, POPULATION_METRIC)
                                 ->modelParams()
                                 .s_InitialDecayRateMultiplier);
        CPPUNIT_ASSERT_EQUAL(0.0,
                             config.factory(1, INDIVIDUAL_COUNT)
                                 ->modelParams()
                                 .s_MaximumUpdatesPerBucket);
        CPPUNIT_ASSERT_EQUAL(0.0,
                             config.factory(1, INDIVIDUAL_METRIC)
                                 ->modelParams()
                                 .s_MaximumUpdatesPerBucket);
        ...
        CPPUNIT_ASSERT_EQUAL(std::size_t(10),
        ...
        CPPUNIT_ASSERT_EQUAL(std::size_t(20),
                             config.factory(1, INDIVIDUAL_COUNT)
                                 ->modelParams()
                                 .s_SampleCountFactor);
        CPPUNIT_ASSERT_EQUAL(std::size_t(20),
        ...
        for (std::size_t i = 0u; i < model_t::NUMBER_AGGREGATION_STYLES; ++i) {
            for (std::size_t j = 0u; j < model_t::NUMBER_AGGREGATION_PARAMS; ++j) {
                params.push_back(
                    config.aggregationStyleParam(static_cast<model_t::EAggregationStyle>(i),
        ...
        CPPUNIT_ASSERT_EQUAL(4.0,
                             config.factory(1, INDIVIDUAL_COUNT)
                                 ->modelParams()
                                 .s_PruneWindowScaleMaximum);
        CPPUNIT_ASSERT_EQUAL(0.5,
                             config.factory(1, INDIVIDUAL_COUNT)
                                 ->modelParams()
                                 .s_PruneWindowScaleMinimum);
        CPPUNIT_ASSERT_EQUAL(4.0,
                             config.factory(1, INDIVIDUAL_METRIC)
                                 ->modelParams()
                                 .s_PruneWindowScaleMaximum);
        ...
        CPPUNIT_ASSERT_EQUAL(0.5,
                             config.factory(1, POPULATION_METRIC)
                                 ->modelParams()
                                 .s_PruneWindowScaleMinimum);
        CPPUNIT_ASSERT_EQUAL(std::string("[(0, 0), (70, 1.5), (85, 1.6), (90, 1.7), (95, 2), (97, "
                                         "10), (98, 20), (99.5, 50), (100, 100)]"),
    ...
    }
    {
        ...
        CPPUNIT_ASSERT(dynamic_cast<const CEventRateModelFactory*>(
        ...
        CPPUNIT_ASSERT(dynamic_cast<const CMetricModelFactory*>(
...
void CAnomalyDetectorModelConfigTest::testErrors(void) {
    {
        ...
        CPPUNIT_ASSERT_EQUAL(config2.factory(1, INDIVIDUAL_COUNT)
                                 ->modelParams()
                                 .s_InitialDecayRateMultiplier,
                             config1.factory(1, INDIVIDUAL_COUNT)
                                 ->modelParams()
                                 .s_InitialDecayRateMultiplier);
        CPPUNIT_ASSERT_EQUAL(config2.factory(1, INDIVIDUAL_METRIC)
                                 ->modelParams()
                                 .s_InitialDecayRateMultiplier,
                             config1.factory(1, INDIVIDUAL_METRIC)
                                 ->modelParams()
                                 .s_InitialDecayRateMultiplier);
        CPPUNIT_ASSERT_EQUAL(config2.factory(1, POPULATION_COUNT)
                                 ->modelParams()
                                 .s_InitialDecayRateMultiplier,
                             config1.factory(1, POPULATION_COUNT)
                                 ->modelParams()
                                 .s_InitialDecayRateMultiplier);
        CPPUNIT_ASSERT_EQUAL(config2.factory(1, POPULATION_METRIC)
                                 ->modelParams()
                                 .s_InitialDecayRateMultiplier,
                             config1.factory(1, POPULATION_METRIC)
                                 ->modelParams()
                                 .s_InitialDecayRateMultiplier);
        CPPUNIT_ASSERT_EQUAL(config2.factory(1, INDIVIDUAL_COUNT)
                                 ->modelParams()
        ...
        for (std::size_t i = 0u; i < model_t::NUMBER_AGGREGATION_STYLES; ++i) {
            for (std::size_t j = 0u; j < model_t::NUMBER_AGGREGATION_PARAMS; ++j) {
                CPPUNIT_ASSERT_EQUAL(
                    config2.aggregationStyleParam(static_cast<model_t::EAggregationStyle>(i),
                                                  static_cast<model_t::EAggregationParam>(j)),
                    config1.aggregationStyleParam(static_cast<model_t::EAggregationStyle>(i),
                                                  static_cast<model_t::EAggregationParam>(j)));
            }
        }
        CPPUNIT_ASSERT_EQUAL(config2.maximumAnomalousProbability(),
...
CppUnit::Test* CAnomalyDetectorModelConfigTest::suite(void) {
    ...
    suiteOfTests->addTest(
        new CppUnit::TestCaller<
            CAnomalyDetectorModelConfigTest>("CAnomalyDetectorModelConfigTest::testNormal",
                                             &CAnomalyDetectorModelConfigTest::testNormal));
    suiteOfTests->addTest(
        new CppUnit::TestCaller<
            CAnomalyDetectorModelConfigTest>("CAnomalyDetectorModelConfigTest::testErrors",
