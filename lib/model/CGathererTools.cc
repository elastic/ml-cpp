...
struct SSumSerializer {
    ...
    void operator()(const TSampleVec& sample, core::CStatePersistInserter& inserter) const {
        inserter.insertValue(SUM_SAMPLE_TAG, core::CPersistUtils::toString(
                                                 sample, CSample::SToString()));
    }
...
struct SInfluencerSumSerializer {
    ...
    void operator()(const TOptionalStrDoubleUMap& map,
                    core::CStatePersistInserter& inserter) const {
        TStrCRefDoublePrVec ordered;
        ordered.reserve(map.size());
        for (TOptionalStrDoubleUMapCItr i = map.begin(); i != map.end(); ++i) {
    ...
    bool operator()(TOptionalStrDoubleUMap& map, core::CStateRestoreTraverser& traverser) const {
        std::string key;
        do {
...
CGathererTools::CArrivalTimeGatherer::CArrivalTimeGatherer()
    : m_LastTime(FIRST_TIME) {
}
...
CGathererTools::CSumGatherer::CSumGatherer(const SModelParams& params,
    ...
    : m_Classifier(), m_BucketSums(params.s_LatencyBuckets, bucketLength, startTime),
      m_InfluencerBucketSums(std::distance(beginInfluencers, endInfluencers),
                             TOptionalStrDoubleUMapQueue(params.s_LatencyBuckets + 3,
                                                         bucketLength,
                                                         startTime,
                                                         TOptionalStrDoubleUMap(1))) {
}
...
SMetricFeatureData
CGathererTools::CSumGatherer::featureData(core_t::TTime time,
                                          core_t::TTime /*bucketLength*/,
    ...
    for (std::size_t i = 0; i < m_InfluencerBucketSums.size(); ++i) {
        const TOptionalStrDoubleUMap& influencerStats =
        ...
        for (const auto& stat : influencerStats) {
            influenceValues[i].emplace_back(
                TStrCRef(*stat.first), TDouble1VecDoublePr(TDouble1Vec{stat.second}, 1.0));
        }
    }
...
void CGathererTools::CSumGatherer::startNewBucket(core_t::TTime time) {
    ...
    for (std::size_t i = 0; i < m_InfluencerBucketSums.size(); ++i) {
        m_InfluencerBucketSums[i].push(TOptionalStrDoubleUMap(1), time);
    }
}
...
void CGathererTools::CSumGatherer::acceptPersistInserter(core::CStatePersistInserter& inserter) const {
    ...
    for (std::size_t i = 0; i < m_InfluencerBucketSums.size(); ++i) {
        inserter.insertLevel(
            ...
            std::bind<void>(
                TOptionalStrDoubleUMapQueue::CSerializer<SInfluencerSumSerializer>(),
                std::cref(m_InfluencerBucketSums[i]), std::placeholders::_1));
    }
}
    ...
    do {
        ...
        if (name == CLASSIFIER_TAG) {
            if (traverser.traverseSubLevel(
                    std::bind(&CDataClassifier::acceptRestoreTraverser,
                              &m_Classifier, std::placeholders::_1)) == false) {
            ...
            if (traverser.traverseSubLevel(std::bind<bool>(
                    TSampleVecQueue::CSerializer<SSumSerializer>(),
                    std::ref(m_BucketSums), std::placeholders::_1)) == false) {
            ...
            if (i < m_InfluencerBucketSums.size() &&
                traverser.traverseSubLevel(std::bind<bool>(
                    TOptionalStrDoubleUMapQueue::CSerializer<SInfluencerSumSerializer>(
                        TOptionalStrDoubleUMap(1)),
                    std::ref(m_InfluencerBucketSums[i++]), std::placeholders::_1)) == false) {
                LOG_ERROR(<< "Invalid bucket queue in " << traverser.value());
                return false;
            }
        }
...
std::string CGathererTools::CSumGatherer::print() const {
    ...
    result << m_Classifier.isInteger() << ' ' << m_BucketSums.print() << ' '
