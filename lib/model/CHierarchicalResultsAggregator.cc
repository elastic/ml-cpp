...
bool influenceProbability(const TStoredStringPtrStoredStringPtrPrDoublePrVec &influences,
    ...
    if (k < influences.size() && equal(influences[k].first, influence)) {
        result = influences[k].second == 1.0 ?
                 p : std::exp(influences[k].second * maths::CTools::fastLog(p));
        return true;
    }
...
bool CHierarchicalResultsAggregator::partitionChildProbabilities(const TNode &node, bool pivot,
                                                                 std::size_t &numberDetectors,
                                                                 TIntSizePrDouble1VecUMap (&partition)[N]) {
...
void CHierarchicalResultsAggregator::detectorProbabilities(const TNode &node, bool pivot,
                                                           std::size_t numberDetectors,
                                                           const TIntSizePrDouble1VecUMap (&partition)[N],
                                                           int &detector, int &aggregation,
        ...
        for (const auto &subset : partition[i]) {
            ...
            if (subset.second.size() == 1) {
                ...
                CAnomalyScore::compute(params[model_t::E_JointProbabilityWeight],
                                       ...
                                       static_cast<std::size_t>(params[model_t::E_MaxExtremeSamples]),
                                       m_MaximumAnomalousProbability,
                                       subset.second, rawAnomalyScore, probability);
            }
            if (!maths::CMathsFuncs::isFinite(probability)) {
                probability = 1.0;
            }
            ...
            switch (detector) {
                case -3: detector = detector_; /*first value we've seen*/ break;
                case -2: /*we have a mix of detectors*/ break;
                default: detector = (detector != detector_ ? -2 : detector_); break;
            }
            switch (aggregation) {
                case -1: aggregation = i; /*first value we've seen*/ break;
                default: aggregation = (aggregation != i ? fallback : i);     break;
            }
        }
    }
...
double CHierarchicalResultsAggregator::correctProbability(const TNode &node, bool pivot,
