...
bool CLimits::init(const std::string& configFile) {
    ...
    if (this->processSetting(propTree, "autoconfig.events", DEFAULT_AUTOCONFIG_EVENTS, m_AutoConfigEvents) == false ||
        this->processSetting(
            propTree, "anomaly.maxtimebuckets", DEFAULT_ANOMALY_MAX_TIME_BUCKETS, m_AnomalyMaxTimeBuckets) == false ||
        ...
        this->processSetting(
            propTree, "memory.modelmemorylimit", CResourceMonitor::DEFAULT_MEMORY_LIMIT_MB, m_MemoryLimitMB) == false) {
