...
CMemoryUsageEstimator::estimate(const TSizeArray& predictors) {
    using TDoubleArray = std::array<double, E_NumberPredictors>;
    ...
    TDoubleArray x0;
    for (std::size_t i = 0; i < m_Values[last].first.size(); ++i) {
    ...
    for (std::size_t i = 0; i < m_Values.size(); i++) {
        for (std::size_t j = 0; j < E_NumberPredictors; ++j) {
    ...
    double predicted = c0;
    for (std::size_t i = 0; i < E_NumberPredictors; ++i) {
