...
struct SStrDataBucketSerializer {
    void operator()(const TSizeSizePrStrDataUMap &strings,
        ...
        for (std::size_t i = 0u; i != ordered.size(); ++i) {
            ...
            inserter.insertLevel(STRING_ITEM_TAG, boost::bind(
                                     &CUniqueStringFeatureData::acceptPersistInserter,
        ...
        try {
            switch (category) {
                case model_t::E_DiurnalTimes:
                    inserter.insertLevel(TIMES_OF_DAY_TAG, boost::bind<void>(
                                             TSizeSizePrMeanAccumulatorUMapQueue::CSerializer<STimesBucketSerializer>(),
                                             boost::cref(boost::any_cast<const TSizeSizePrMeanAccumulatorUMapQueue&>(data)), _1));
                    break;
                case model_t::E_MeanArrivalTimes:
                    // TODO
                    break;
                case model_t::E_AttributePeople:
                    inserter.insertLevel(ATTRIBUTE_PEOPLE_TAG, boost::bind(
                                             &persistAttributePeopleData,
                                             boost::cref(boost::any_cast<const TSizeUSetVec&>(data)), _1));
                    break;
                case model_t::E_UniqueValues:
                    inserter.insertLevel(UNIQUE_VALUES_TAG, boost::bind<void>(
                                             TSizeSizePrStrDataUMapQueue::CSerializer<SStrDataBucketSerializer>(),
                                             boost::cref(boost::any_cast<const TSizeSizePrStrDataUMapQueue&>(data)), _1));
                    break;
            }
...
bool restoreFeatureData(core::CStateRestoreTraverser &traverser,
    ...
    if (name == ATTRIBUTE_PEOPLE_TAG) {
        TSizeUSetVec *data{boost::unsafe_any_cast<TSizeUSetVec>(
                               &featureData.emplace(model_t::E_AttributePeople,
                                                    TSizeUSetVec()).first->second)};
        if (traverser.traverseSubLevel(boost::bind(&restoreAttributePeopleData,
                                                   _1, boost::ref(*data))) == false) {
        ...
        TSizeSizePrStrDataUMapQueue *data{boost::unsafe_any_cast<TSizeSizePrStrDataUMapQueue>(
                                              &featureData.emplace(model_t::E_UniqueValues,
                                                                   TSizeSizePrStrDataUMapQueue(
                                                                       latencyBuckets,
                                                                       bucketLength,
                                                                       currentBucketStartTime,
                                                                       TSizeSizePrStrDataUMap(1))).first->second)};
        ...
        TSizeSizePrMeanAccumulatorUMapQueue *data{boost::unsafe_any_cast<TSizeSizePrMeanAccumulatorUMapQueue>(
                                                      &featureData.emplace(model_t::E_DiurnalTimes,
                                                                           TSizeSizePrMeanAccumulatorUMapQueue(
                                                                               latencyBuckets,
                                                                               bucketLength,
                                                                               currentBucketStartTime)).first->second)};
        if (traverser.traverseSubLevel(boost::bind<bool>(
                                           TSizeSizePrMeanAccumulatorUMapQueue::CSerializer<STimesBucketSerializer>(),
                                           boost::ref(*data), _1)) == false) {
...
bool CEventRateBucketGatherer::acceptRestoreTraverser(core::CStateRestoreTraverser &traverser) {
    ...
    do {
        ...
        if (restoreFeatureData(traverser, m_FeatureData,
...
uint64_t CEventRateBucketGatherer::checksum(void) const {
    ...
    return core::CHashing::hashCombine(
               seed, hasher(core::CContainerPrinter::print(hashes)));
}
...
void CEventRateBucketGatherer::personCounts(model_t::EFeature feature,
    ...
    for (const auto &count_ : this->bucketCounts(time)) {
        uint64_t &count = std::lower_bound(result.begin(), result.end(),
                                           ...
                                           maths::COrderings::SFirstLess())->second.s_Count;
    ...
    for (std::size_t i = 0u; i < influencers.size(); ++i) {
        for (const auto &influence : influencers[i]) {
            ...
            if (k == result.end() || k->first != influence.first.first) {
                ...
                LOG_ERROR("Missing feature data for person " << m_DataGatherer.personName(pid)
                          << " and attribute " << m_DataGatherer.attributeName(cid));
                continue;
            }
            k->second.s_InfluenceValues[i].emplace_back(
                TStrCRef(*CDataGatherer::extractData(influence.first)),
...
void CUniqueStringFeatureData::populateInfoContentFeatureData(SEventRateFeatureData &featureData) const {
    ...
    try {
        ...
        [&compressor](const std::string &string) { compressor.addString(string); });
        ...
        for (std::size_t i = 0u; i < m_InfluencerUniqueStrings.size(); ++i) {
            ...
            for (const auto &influence : m_InfluencerUniqueStrings[i]) {
                ...
                data.emplace_back(TStrCRef(*influence.first),
                                  TDouble1VecDoublePr(
                                      TDouble1Vec{static_cast<double>(length)}, 1.0));
            }
        }
    } catch (const std::exception &e) {
...
bool CUniqueStringFeatureData::acceptRestoreTraverser(core::CStateRestoreTraverser &traverser) {
    do {
        ...
        RESTORE(UNIQUE_STRINGS_TAG,
                traverser.traverseSubLevel(boost::bind(&restoreUniqueStrings,
                                                       _1, boost::ref(m_UniqueStrings))))
        RESTORE_SETUP_TEARDOWN(INFLUENCER_UNIQUE_STRINGS_TAG,
                               m_InfluencerUniqueStrings.push_back(TStoredStringPtrWordSetUMap()),
                               traverser.traverseSubLevel(boost::bind(&restoreInfluencerUniqueStrings,
                                                                      _1, boost::ref(m_InfluencerUniqueStrings.back()))),
                               /**/)
    } while (traverser.next());
