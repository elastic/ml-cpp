...
void CMetricPopulationModel::initialize(
    const TFeatureMathsModelPtrPrVec &newFeatureModels,
    const TFeatureMultivariatePriorPtrPrVec &newFeatureCorrelateModelPriors,
    const TFeatureCorrelationsPtrPrVec &featureCorrelatesModels) {
...
bool CMetricPopulationModel::acceptRestoreTraverser(core::CStateRestoreTraverser &traverser) {
    ...
    do {
        ...
        RESTORE(POPULATION_STATE_TAG,
                traverser.traverseSubLevel(
                    boost::bind(&CMetricPopulationModel::doAcceptRestoreTraverser, this, _1)))
...
model_t::EModelType CMetricPopulationModel::category(void) const { return model_t::E_MetricOnline; }

bool CMetricPopulationModel::isEventRate(void) const { return false; }

bool CMetricPopulationModel::isMetric(void) const { return true; }
    ...
    for (core_t::TTime time = startTime; time < endTime; time += bucketLength) {
        ...
        for (auto &featureData_ : featureData) {
            ...
            for (const auto &data_ : data) {
                ...
                core_t::TTime cutoff =
                    attributeLastBucketTimes[cid] - this->params().s_SamplingAgeCutoff;
                ...
                std::size_t n =
                    std::count_if(samples.begin(), samples.end(), [cutoff](const CSample &sample) {
                ...
                double countWeight =
                    ...
                    (updatesPerBucket > 0.0 && n > 0 ? updatesPerBucket / static_cast<double>(n)
                                                     : 1.0);
                ...
                for (const auto &sample : samples) {
                    ...
                    if (duplicate < attribute.s_Values.size()) {
                        std::for_each(
                            attribute.s_TrendWeights[duplicate][0].begin(),
...
bool CMetricPopulationModel::computeProbability(std::size_t pid,
    ...
    for (std::size_t i = 0u; i < gatherer.numberFeatures(); ++i) {
        ...
        for (std::size_t j = range.first; j < range.second; ++j) {
            ...
            if (!bucket) {
                LOG_ERROR("Expected a value for feature = "
                          << model_t::print(feature) << ", person = " << gatherer.personName(pid)
...
bool CMetricPopulationModel::computeTotalProbability(
...
std::size_t CMetricPopulationModel::staticSize(void) const { return sizeof(*this); }
...
const TSizeSizePrFeatureDataPrVec &CMetricPopulationModel::featureData(model_t::EFeature feature,
                                                                       core_t::TTime time) const {
