...
bool unset(core::CStoredStringPtr value) { return value.get() == UNSET_STRING.get(); }

//! True if the node is a leaf.
bool isLeaf(const SNode &node) { return node.s_Children.empty(); }

//! True if the node is aggregate.
bool isAggregate(const SNode &node) { return node.s_Children.size() > 0; }
...
SNode *address(SNode *ptr) { return ptr; }

//! Get the address of a node value.
SNode *address(SNode &value) { return &value; }
...
double SNode::probability(void) const { return s_AnnotatedProbability.s_Probability; }
    ...
    do {
        ...
        RESTORE(ANNOTATED_PROBABILITY_TAG,
                core::CPersistUtils::restore(
                    ANNOTATED_PROBABILITY_TAG, s_AnnotatedProbability, traverser))
...
void swap(SNode &node1, SNode &node2) { node1.swap(node2); }
...
void CHierarchicalResults::buildHierarchy(void) {
    ...
    {
        aggregateLayer<SPersonValueLess>(
            m_Nodes.begin(), m_Nodes.end(), *this, &CHierarchicalResults::newNode, layer);
    ...
    LOG_TRACE("Distinct person field names");
    {
        ...
        aggregateLayer<SPersonNameLess>(
    ...
    {
        newLayer.reserve(layer.size());
        aggregateLayer<SPartitionNameLess>(
            layer.begin(), layer.end(), *this, &CHierarchicalResults::newNode, newLayer);
        newLayer.swap(layer);
        LOG_TRACE("layer = " << core::CContainerPrinter::print(layer));
    }
...
bool CHierarchicalResults::empty(void) const { return m_Nodes.empty(); }
...
void CHierarchicalResults::setInterim(void) { m_ResultType.set(model_t::CResultType::E_Interim); }

model_t::CResultType CHierarchicalResults::resultType(void) const { return m_ResultType; }
...
void CHierarchicalResults::acceptPersistInserter(core::CStatePersistInserter &inserter) const {
    ...
    for (const auto &node : m_Nodes) {
        inserter.insertLevel(
            NODES_1_TAG,
            boost::bind(
                &SNode::acceptPersistInserter1, boost::cref(node), _1, boost::ref(nodePointers)));
    }
    ...
    std::sort(pivotRootIterators.begin(),
              pivotRootIterators.end(),
              core::CFunctional::SDereference<maths::COrderings::SFirstLess>());
    ...
    for (const auto &node : m_Nodes) {
        inserter.insertLevel(
            NODES_2_TAG,
            boost::bind(
                &SNode::acceptPersistInserter2, boost::cref(node), _1, boost::cref(nodePointers)));
    }
...
bool CHierarchicalResultsVisitor::isRoot(const TNode &node) { return !node.s_Parent; }

bool CHierarchicalResultsVisitor::isLeaf(const TNode &node) { return node.s_Children.empty(); }
