...
bool apply(ITR begin, ITR end, const F &f)
{
    for (ITR i = begin; i != end; ++i)
    {
        ...
        try
        {
            switch (category)
            {
            case model_t::E_Mean:             apply<model_t::E_Mean>(i, f);             break;
            case model_t::E_Median:           apply<model_t::E_Median>(i, f);           break;
            case model_t::E_Min:              apply<model_t::E_Min>(i, f);              break;
            case model_t::E_Max:              apply<model_t::E_Max>(i, f);              break;
            case model_t::E_Variance:         apply<model_t::E_Variance>(i, f);         break;
            case model_t::E_Sum:              apply<model_t::E_Sum>(i, f);              break;
            case model_t::E_MultivariateMean: apply<model_t::E_MultivariateMean>(i, f); break;
...
void initializeFeatureDataInstance(std::size_t dimension,
                                   TCategorySizePrAnyMap &featureData)
{
                                   ...
                                   TSizeTUMap<T> &result) const
                {
                    ...
                    do
                    {
                        ...
                        else if (name == DATA_TAG)
                        {
                            ...
                            if (traverser.traverseSubLevel(boost::bind<bool>(&T::acceptRestoreTraverser,
                                                                             &initial,
                                                                             _1)) == false)
                            {
                                LOG_ERROR("Invalid data in " << traverser.value());
...
struct SExtractFeatureData
{
    ...
    private:
        ...
        {
            result.emplace_back(pid, this->featureData(data, time, bucketLength,
...
struct SAddValue
{
    ...
    {
        auto &entry = data[cid].emplace(boost::unordered::piecewise_construct,
                                          ...
                                          CResourceMonitor &resourceMonitor)
{
    ...
    if (addedPerson)
    {
        resourceMonitor.addExtraMemory(m_DataGatherer.isPopulation() ?
                CDataGatherer::ESTIMATED_MEM_USAGE_PER_OVER_FIELD : CDataGatherer::ESTIMATED_MEM_USAGE_PER_BY_FIELD);
...
void CMetricBucketGatherer::recyclePeople(const TSizeVec &peopleToRemove)
{
    ...
    apply(m_FeatureData, boost::bind<void>(SRemovePeople(), _1, _2,
...
uint64_t CMetricBucketGatherer::checksum(void) const
{
    ...
    apply(m_FeatureData, boost::bind<void>(SHash(), _1, _2,
                                           boost::cref(*this),
                                           boost::ref(hashes)));
    LOG_TRACE("seed = " << seed);
...
void CMetricBucketGatherer::sample(core_t::TTime time)
{
    ...
    {
        apply(m_FeatureData, boost::bind<void>(SDoSample(), _1, _2,
                                               time, boost::cref(*this),
    ...
    for (std::size_t i = 0u, n = m_DataGatherer.numberFeatures(); i < n; ++i)
    {
        ...
        if (model_t::metricCategory(feature, category))
        {
            ...
            if (begin != m_FeatureData.end())
            {
                auto end = begin;
                ++end;
                apply(begin, end, boost::bind<void>(SExtractFeatureData(), _1, _2,
                                                    boost::cref(*this),
...
void CMetricBucketGatherer::initializeFeatureData(void)
{
    for (std::size_t i = 0u, n = m_DataGatherer.numberFeatures(); i < n; ++i)
    {
        ...
        if (model_t::metricCategory(feature, category))
        {
            ...
            switch (category)
            {
            case model_t::E_Mean:             initializeFeatureDataInstance<model_t::E_Mean>(dimension, m_FeatureData);             break;
            case model_t::E_Median:           initializeFeatureDataInstance<model_t::E_Median>(dimension, m_FeatureData);           break;
            case model_t::E_Min:              initializeFeatureDataInstance<model_t::E_Min>(dimension, m_FeatureData);              break;
            case model_t::E_Max:              initializeFeatureDataInstance<model_t::E_Max>(dimension, m_FeatureData);              break;
            case model_t::E_Variance:         initializeFeatureDataInstance<model_t::E_Variance>(dimension, m_FeatureData);         break;
            case model_t::E_Sum:              initializeFeatureDataInstance<model_t::E_Sum>(dimension, m_FeatureData);              break;
            case model_t::E_MultivariateMean: initializeFeatureDataInstance<model_t::E_MultivariateMean>(dimension, m_FeatureData); break;
