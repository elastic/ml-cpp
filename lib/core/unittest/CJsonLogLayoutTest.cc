...
#ifdef Windows
    BOOST_REQUIRE_EQUAL(std::string{"source.h"},
...
BOOST_AUTO_TEST_CASE(testExtractClassAndMethod) {
    ...
    BOOST_REQUIRE_EQUAL(std::string{"ns1::ns2::clazz"}, className);
    BOOST_REQUIRE_EQUAL(std::string{"someMethod"}, methodName);

    std::tie(className, methodName) = ml::core::CJsonLogLayout::extractClassAndMethod(
        "static CJsonLogLayout::TStrStrPr ml::core::CJsonLogLayout::extractClassAndMethod(std::string)");
    BOOST_REQUIRE_EQUAL(std::string{"ml::core::CJsonLogLayout"}, className);
    BOOST_REQUIRE_EQUAL(std::string{"extractClassAndMethod"}, methodName);

    std::tie(className, methodName) =
        ml::core::CJsonLogLayout::extractClassAndMethod(BOOST_CURRENT_FUNCTION);
    BOOST_REQUIRE_EQUAL(std::string{"CJsonLogLayoutTest::testExtractClassAndMethod"}, className);
    BOOST_REQUIRE_EQUAL(std::string{"test_method"}, methodName);
}
