...
BOOST_AUTO_TEST_CASE(testBuildGivenUnsortedInput) {
    ...
    BOOST_TEST_REQUIRE(prefixTree.build(prefixes) == false);
}
...
BOOST_AUTO_TEST_CASE(testEmptyString) {
    ...
    CFlatPrefixTree prefixTree;
    BOOST_TEST_REQUIRE(prefixTree.build(prefixes));

    BOOST_TEST_REQUIRE(prefixTree.matchesFully("") == false);
    BOOST_TEST_REQUIRE(prefixTree.matchesStart("") == false);
}
...
BOOST_AUTO_TEST_CASE(testSimple) {
    ...
    for (std::size_t i = 0; i < prefixes.size(); ++i) {
        BOOST_TEST_REQUIRE(prefixTree.matchesStart(prefixes[i]));
        BOOST_TEST_REQUIRE(prefixTree.matchesFully(prefixes[i]));
    }

    BOOST_TEST_REQUIRE(prefixTree.matchesStart("abcz"));
    BOOST_TEST_REQUIRE(prefixTree.matchesStart("acdz"));
    BOOST_TEST_REQUIRE(prefixTree.matchesStart("aqrsz"));
    BOOST_TEST_REQUIRE(prefixTree.matchesStart("aqrtz"));
    BOOST_TEST_REQUIRE(prefixTree.matchesStart("bdfz"));
    BOOST_TEST_REQUIRE(prefixTree.matchesFully("abcd") == false);
    BOOST_TEST_REQUIRE(prefixTree.matchesStart("ab") == false);
    BOOST_TEST_REQUIRE(prefixTree.matchesStart("c") == false);
    BOOST_TEST_REQUIRE(prefixTree.matchesStart("") == false);
    BOOST_TEST_REQUIRE(prefixTree.matchesFully("") == false);
}
...
BOOST_AUTO_TEST_CASE(testMatchesStartGivenStringThatMatchesMoreThanAGivenPrefix) {
    ...
    BOOST_TEST_REQUIRE(prefixTree.matchesStart("h2 vh5"));
    BOOST_TEST_REQUIRE(prefixTree.matchesStart("h2 vh55daetrqt4"));
}
...
BOOST_AUTO_TEST_CASE(testMatchesFullyGivenStringThatIsSubstringOfPrefix) {
    ...
    BOOST_TEST_REQUIRE(prefixTree.matchesFully("fo") == false);
}
...
BOOST_AUTO_TEST_CASE(testRandom) {
    ...
    // Assert startsWith lookups
    {
        ...
        for (std::size_t i = 0; i < 100000; i++) {
            ...
            BOOST_TEST_REQUIRE(prefixTree.matchesStart(key));
        }
    }
}
