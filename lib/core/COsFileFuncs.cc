...
int COsFileFuncs::open(const char *path, int oflag) { return ::open(path, oflag); }

int COsFileFuncs::open(const char *path, int oflag, TMode pmode) {
    return ::open(path, oflag, pmode);
}

int COsFileFuncs::dup(int fildes) { return ::dup(fildes); }
...
int COsFileFuncs::close(int fildes) { return ::close(fildes); }

int COsFileFuncs::fstat(int fildes, TStat *buf) { return ::fstat(fildes, buf); }

int COsFileFuncs::stat(const char *path, TStat *buf) { return ::stat(path, buf); }

int COsFileFuncs::lstat(const char *path, TStat *buf) { return ::lstat(path, buf); }

int COsFileFuncs::access(const char *path, int amode) { return ::access(path, amode); }

char *COsFileFuncs::getcwd(char *buf, size_t size) { return ::getcwd(buf, size); }

int COsFileFuncs::chdir(const char *path) { return ::chdir(path); }
