...
CRapidXmlParser::CRapidXmlParser() : m_XmlBufSize(0), m_NavigatedNode(nullptr) {
}
...
bool CRapidXmlParser::rootElementAttributes(TStrStrMap& rootAttributes) const {
    ...
    for (const TCharRapidXmlAttribute* attr = root->first_attribute();
        ...
        rootAttributes[std::string(attr->name(), attr->name_size())].assign(
...
bool CRapidXmlParser::toNodeHierarchy(const TCharRapidXmlNode& parentNode,
    ...
    for (CXmlNode::TStrStrPrVecItr iter = nodePtr->m_Attributes.begin();
         iter != nodePtr->m_Attributes.end(); ++iter) {
...
void CRapidXmlParser::convert(bool indent, const CXmlNodeWithChildren& root, std::string& result) {
    ...
    TCharRapidXmlNode* rootNode(doc.allocate_node(
        rapidxml::node_element, root.name().c_str(),
        root.value().empty() ? nullptr : root.value().c_str(), nameLen, valueLen));
    ...
    for (CXmlNode::TStrStrPrVecCItr attrIter = attrs.begin();
        ...
        TCharRapidXmlAttribute* attr(doc.allocate_attribute(
            ...
            attrIter->second.empty() ? nullptr : attrIter->second.c_str(), nameLen, valueLen));
...
void CRapidXmlParser::convertChildren(const CXmlNodeWithChildren& current,
    ...
    if (xmlNode.value_size() > 0 && !childVec.empty()) {
        TCharRapidXmlNode* dataNode(doc.allocate_node(
            rapidxml::node_data, nullptr, xmlNode.value(), 0, xmlNode.value_size()));
    ...
    for (CXmlNodeWithChildren::TChildNodePVecCItr childIter = childVec.begin();
        ...
        if (child != nullptr) {
            ...
            TCharRapidXmlNode* childNode(doc.allocate_node(
                rapidxml::node_element, child->name().c_str(),
                child->value().empty() ? nullptr : child->value().c_str(), nameLen, valueLen));
            ...
            for (CXmlNode::TStrStrPrVecCItr attrIter = attrs.begin();
                ...
                TCharRapidXmlAttribute* attr(doc.allocate_attribute(
                    attrIter->first.c_str(),
                    attrIter->second.empty() ? nullptr : attrIter->second.c_str(),
                    nameLen, valueLen));
