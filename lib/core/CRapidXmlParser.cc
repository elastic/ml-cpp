...
CRapidXmlParser::CRapidXmlParser(void) : m_XmlBufSize(0), m_NavigatedNode(0) {
}
...
bool CRapidXmlParser::parseBuffer(const char* begin, size_t length) {
    return this->parseBufferNonDestructive<rapidxml::parse_no_string_terminators>(begin, length);
}

bool CRapidXmlParser::parseBufferInSitu(char* begin, size_t length) {
    return this->parseBufferDestructive<rapidxml::parse_no_string_terminators>(begin, length);
}

bool CRapidXmlParser::parseStringIgnoreCdata(const std::string& xml) {
    return this->parseBufferNonDestructive<rapidxml::parse_no_string_terminators | rapidxml::parse_no_data_nodes>(xml.c_str(),
                                                                                                                  xml.length());
}
...
std::string CRapidXmlParser::dumpToString(void) const {
    std::string result;
    rapidxml::print(std::back_inserter(result), m_Doc);
    return result;
}
...
bool CRapidXmlParser::toNodeHierarchy(CStringCache& cache, CXmlNodeWithChildren::TXmlNodeWithChildrenP& rootNodePtr) const {
    // Because both the pool and the nodes use shared pointers, it doesn't
    // matter if the pool that originally allocates the nodes is destroyed
    // before the nodes themselves.  Hence we can get away with implementing
    // this version of the method in terms of the one that takes a pool.
...
bool CRapidXmlParser::toNodeHierarchy(CXmlNodeWithChildrenPool& pool, CXmlNodeWithChildren::TXmlNodeWithChildrenP& rootNodePtr) const {
    rootNodePtr.reset();
...
bool CRapidXmlParser::toNodeHierarchy(const TCharRapidXmlNode& parentNode,
    ...
    if (cache != 0) {
        ...
        nodePtr->m_Name = cache->stringFor(parentNode.name(), parentNode.name_size());
    } else {
    ...
    for (CXmlNode::TStrStrPrVecItr iter = nodePtr->m_Attributes.begin(); iter != nodePtr->m_Attributes.end(); ++iter) {
        ...
        if (cache != 0) {
            ...
            iter->first = cache->stringFor(attr->name(), attr->name_size());
            iter->second = cache->stringFor(attr->value(), attr->value_size());
        } else {
    ...
    while (childNode != 0) {
        if (childNode->type() == rapidxml::node_element) {
            ...
            nodePtr->m_Value.append(childNode->value(), childNode->value_size());
        }
...
void CRapidXmlParser::convert(const CXmlNodeWithChildren& root, std::string& result) {
    CRapidXmlParser::convert(true, root, result);
}

void CRapidXmlParser::convert(bool indent, const CXmlNodeWithChildren& root, std::string& result) {
    ...
    TCharRapidXmlNode* rootNode(
        doc.allocate_node(rapidxml::node_element, root.name().c_str(), root.value().empty() ? 0 : root.value().c_str(), nameLen, valueLen));
    ...
    for (CXmlNode::TStrStrPrVecCItr attrIter = attrs.begin(); attrIter != attrs.end(); ++attrIter) {
        ...
        TCharRapidXmlAttribute* attr(
            doc.allocate_attribute(attrIter->first.c_str(), attrIter->second.empty() ? 0 : attrIter->second.c_str(), nameLen, valueLen));
    ...
    if (indent) {
        ...
        rapidxml::print(std::back_inserter(result), doc, rapidxml::print_no_indenting);
    }
}
    ...
    for (CXmlNodeWithChildren::TChildNodePVecCItr childIter = childVec.begin(); childIter != childVec.end(); ++childIter) {
        ...
        if (child != 0) {
            ...
            TCharRapidXmlNode* childNode(doc.allocate_node(
                rapidxml::node_element, child->name().c_str(), child->value().empty() ? 0 : child->value().c_str(), nameLen, valueLen));
