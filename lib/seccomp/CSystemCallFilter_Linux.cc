...
#ifdef __x86_64__
// The rseq and clone3 syscalls won't be defined on a RHEL/CentOS 7 build
...
#define __NR_clone3 435
    ...
    BPF_JUMP(BPF_JMP | BPF_JGT | BPF_K, UPPER_NR_LIMIT, 52, 0),
    ...
    BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, __NR_access, 52, 0),
    BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, __NR_open, 51, 0),
    BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, __NR_dup2, 50, 0),
    BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, __NR_unlink, 49, 0),
    BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, __NR_stat, 48, 0),
    BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, __NR_lstat, 47, 0),
    BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, __NR_time, 46, 0),
    BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, __NR_readlink, 45, 0),
    BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, __NR_getdents, 44, 0), // for forecast temp storage
    BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, __NR_rmdir, 43, 0), // for forecast temp storage
    BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, __NR_mkdir, 42, 0), // for forecast temp storage
    BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, __NR_mknod, 41, 0),
...
#define __NR_clone3 435
#endif
    BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, __NR_faccessat, 41, 0),
#else
#error Unsupported hardware architecture
#endif

    // Allowed sys calls for all architectures, jump to return allow on match
    BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, __NR_getpid, 40, 0), // for pthread_kill
