...
// We use a special filter for PyTorch to disallow reading and writing files
const struct sock_filter PYTORCH_FILTER[] = {
    // Load the system call number into accumulator
    BPF_STMT(BPF_LD | BPF_W | BPF_ABS, SECCOMP_DATA_NR_OFFSET),
...
#error Unsupported hardware architecture
    ...
    // Allowed sys calls for all architectures, jump to return allow on match
    BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, __NR_fcntl, 38, 0), // for fdopendir
    BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, __NR_getrusage, 37, 0),
    BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, __NR_getpid, 36, 0), // for pthread_kill
    BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, __NR_statx, 35, 0), // for create_directories
    ...
    BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, __NR_fstat, 25, 0),
    BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, __NR_close, 24, 0),
    BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, __NR_connect, 23, 0),
    BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, __NR_clone3, 22, 0),
