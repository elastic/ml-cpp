...
struct SCovariancesCustomAdd<CAnnotatedVector<CVectorNx1<T, N>, SCountAndVariance>> {
    template<typename U>
    static inline void
    add(const CAnnotatedVector<CVectorNx1<T, N>, SCountAndVariance>& x,
        ...
        covariances += CBasicStatistics::SSampleCovariances<U, N>(
            T(x.annotation().s_Count) * n, x, m);
    }
};
...
struct SCovariancesLedoitWolf<CAnnotatedVector<CVectorNx1<T, N>, SCountAndVariance>> {
    ...
    estimate(const std::vector<CAnnotatedVector<CVectorNx1<T, N>, SCountAndVariance>>& points,
        ...
        for (std::size_t i = 0u; i < points.size(); ++i) {
            ...
            bn += ni *
                  pow2(((ci - m).outer() + CVectorNx1<U, N>(vi).diagonal() - s).frobenius()) /
                  d / z;
        }
        bn = std::min(bn, dn);
