...
template <> struct SSqrt<VectorTag> {
    template <typename VECTOR> static void calculate(std::size_t d, VECTOR &result) {
...
template <> struct SSqrt<MatrixTag> {
    template <typename MATRIX> static void calculate(std::size_t d, MATRIX &result) {
        for (std::size_t i = 0u; i < d; ++i) {
...
template <> struct SMin<VectorVectorTag> {
...
//! Component-wise minimum for a vector.
template <> struct SMin<VectorScalarTag> {
...
//! Component-wise minimum for a vector.
template <> struct SMin<ScalarVectorTag> {
...
template <> struct SMin<MatrixMatrixTag> {
...
//! Element-wise minimum for a symmetric matrix.
template <> struct SMin<MatrixScalarTag> {
...
//! Element-wise minimum for a symmetric matrix.
template <> struct SMin<ScalarMatrixTag> {
...
template <> struct SMax<VectorVectorTag> {
    template <typename VECTOR>
    static void calculate(std::size_t d, const VECTOR &lhs, VECTOR &rhs) {
        for (std::size_t i = 0u; i < d; ++i) {
...
//! Component-wise maximum for a vector.
template <> struct SMax<VectorScalarTag> {
    template <typename VECTOR, typename T>
    static void calculate(std::size_t d, VECTOR &lhs, const T &rhs) {
        for (std::size_t i = 0u; i < d; ++i) {
...
//! Component-wise maximum for a vector.
template <> struct SMax<ScalarVectorTag> {
    template <typename T, typename VECTOR>
    static void calculate(std::size_t d, const T &lhs, VECTOR &rhs) {
        for (std::size_t i = 0u; i < d; ++i) {
...
template <> struct SMax<MatrixMatrixTag> {
    template <typename MATRIX>
    static void calculate(std::size_t d, const MATRIX &lhs, MATRIX &rhs) {
        for (std::size_t i = 0u; i < d; ++i) {
            for (std::size_t j = 0u; j <= i; ++j) {
...
//! Element-wise maximum for a symmetric matrix.
template <> struct SMax<MatrixScalarTag> {
    template <typename MATRIX, typename T>
    static void calculate(std::size_t d, MATRIX &lhs, const T &rhs) {
        for (std::size_t i = 0u; i < d; ++i) {
            for (std::size_t j = 0u; j <= i; ++j) {
...
//! Element-wise maximum for a symmetric matrix.
template <> struct SMax<ScalarMatrixTag> {
    template <typename T, typename MATRIX>
    static void calculate(std::size_t d, const T &lhs, MATRIX &rhs) {
        for (std::size_t i = 0u; i < d; ++i) {
            for (std::size_t j = 0u; j <= i; ++j) {
...
template <> struct SFabs<VectorTag> {
    template <typename VECTOR> static void calculate(std::size_t d, VECTOR &result) {
        for (std::size_t i = 0u; i < d; ++i) {
...
template <> struct SFabs<MatrixTag> {
    template <typename MATRIX> static void calculate(std::size_t d, MATRIX &result) {
        for (std::size_t i = 0u; i < d; ++i) {
            for (std::size_t j = 0u; j <= i; ++j) {
...
//! Output for debug.
template <typename T> std::ostream &operator<<(std::ostream &o, const CSymmetricMatrix<T> &m) {
...
//! Output for debug.
template <typename T> std::ostream &operator<<(std::ostream &o, const CVector<T> &v) {
    if (v.dimension() == 0) {
        return o << "[]";
    }
    o << "[";
...
template <typename T, std::size_t N> CVectorNx1<T, N> sqrt(const CVectorNx1<T, N> &v) {
    CVectorNx1<T, N> result(v);
    linear_algebra_tools_detail::SSqrt<linear_algebra_tools_detail::VectorTag>::calculate(N,
                                                                                          result);
    return result;
}
...
CSymmetricMatrixNxN<T, N> sqrt(const CSymmetricMatrixNxN<T, N> &m) {
    ...
    linear_algebra_tools_detail::SSqrt<linear_algebra_tools_detail::MatrixTag>::calculate(N,
                                                                                          result);
    return result;
}
...
template <typename T, std::size_t N> CVectorNx1<T, N> fabs(const CVectorNx1<T, N> &v) {
    CVectorNx1<T, N> result(v);
    linear_algebra_tools_detail::SFabs<linear_algebra_tools_detail::VectorTag>::calculate(N,
                                                                                          result);
    return result;
}
...
CSymmetricMatrixNxN<T, N> fabs(const CSymmetricMatrixNxN<T, N> &m) {
    CSymmetricMatrixNxN<T, N> result(m);
    linear_algebra_tools_detail::SFabs<linear_algebra_tools_detail::MatrixTag>::calculate(N,
                                                                                          result);
    return result;
}

//! Overload sqrt for CVector.
template <typename T> CVector<T> sqrt(const CVector<T> &v) {
    CVector<T> result(v);
...
template <typename T> CSymmetricMatrix<T> sqrt(const CSymmetricMatrix<T> &m) {
...
template <typename T> CVector<T> min(const CVector<T> &lhs, const CVector<T> &rhs) {
...
//! Overload minimum for CVector.
template <typename T> CVector<T> min(const CVector<T> &lhs, const T &rhs) {
...
//! Overload minimum for CVector.
template <typename T> CVector<T> min(const T &lhs, const CVector<T> &rhs) {
    CVector<T> result(rhs);
    linear_algebra_tools_detail::SMin<linear_algebra_tools_detail::ScalarVectorTag>::calculate(
        result.dimension(), lhs, result);
    return result;
...
//! Overload minimum for CSymmetricMatrix.
template <typename T> CSymmetricMatrix<T> min(const CSymmetricMatrix<T> &lhs, const T &rhs) {
...
//! Overload minimum for CSymmetricMatrix.
template <typename T> CSymmetricMatrix<T> min(const T &lhs, const CSymmetricMatrix<T> &rhs) {
    CSymmetricMatrix<T> result(rhs);
    linear_algebra_tools_detail::SMin<linear_algebra_tools_detail::ScalarMatrixTag>::calculate(
        result.rows(), lhs, result);
    return result;
...
template <typename T> CVector<T> max(const CVector<T> &lhs, const CVector<T> &rhs) {
    CVector<T> result(rhs);
    linear_algebra_tools_detail::SMax<linear_algebra_tools_detail::VectorVectorTag>::calculate(
        result.dimension(), lhs, result);
    return result;
...
//! Overload maximum for CVector.
template <typename T> CVector<T> max(const CVector<T> &lhs, const T &rhs) {
    CVector<T> result(lhs);
    linear_algebra_tools_detail::SMax<linear_algebra_tools_detail::VectorScalarTag>::calculate(
        result.dimension(), result, rhs);
    return result;
...
//! Overload maximum for CVector.
template <typename T> CVector<T> max(const T &lhs, const CVector<T> &rhs) {
    CVector<T> result(rhs);
    linear_algebra_tools_detail::SMax<linear_algebra_tools_detail::ScalarVectorTag>::calculate(
        result.dimension(), lhs, result);
    return result;
...
//! Overload maximum for CSymmetricMatrix.
template <typename T> CSymmetricMatrix<T> max(const CSymmetricMatrix<T> &lhs, const T &rhs) {
    CSymmetricMatrix<T> result(lhs);
    linear_algebra_tools_detail::SMax<linear_algebra_tools_detail::MatrixScalarTag>::calculate(
        result.rows(), result, rhs);
    return result;
...
//! Overload maximum for CSymmetricMatrix.
template <typename T> CSymmetricMatrix<T> max(const T &lhs, const CSymmetricMatrix<T> &rhs) {
    CSymmetricMatrix<T> result(rhs);
    linear_algebra_tools_detail::SMax<linear_algebra_tools_detail::ScalarMatrixTag>::calculate(
        result.rows(), lhs, result);
    return result;
...
template <typename T> CVector<T> fabs(const CVector<T> &v) {
    CVector<T> result(v);
...
template <typename T> CSymmetricMatrix<T> fabs(const CSymmetricMatrix<T> &m) {
    CSymmetricMatrix<T> result(m);
...
//! Efficiently scale the \p i'th row and column by \p scale.
template <typename T> void scaleCovariances(std::size_t i, T scale, CSymmetricMatrix<T> &m) {
    scale = ::sqrt(scale);
    for (std::size_t j = 0u; j < m.columns(); ++j) {
        if (i == j) {
            m(i, j) = scale;
...
//! Efficiently scale the rows and columns by \p scale.
template <typename T> void scaleCovariances(const CVector<T> &scale, CSymmetricMatrix<T> &m) {
    for (std::size_t i = 0u; i < scale.dimension(); ++i) {
...
maths_t::EFloatingPointErrorStatus logDeterminant(const CSymmetricMatrix<T> &matrix,
                                                  double &result,
                                                  bool ignoreSingularSubspace = true) {
    return linear_algebra_tools_detail::logDeterminant(
        matrix.rows(), matrix, result, ignoreSingularSubspace);
}
