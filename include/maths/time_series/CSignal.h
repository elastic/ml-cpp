...
#ifndef INCLUDED_ml_maths_time_series_CSignal_h
    ...
    struct SSeasonalComponentSummary {
        ...
        double value(const MODEL& model, std::size_t index) const {
            return this->contains(index) == false
                       ? 0.0
                       : common::CBasicStatistics::mean(model[this->offset(index)]);
        }
...
public:
    ...
    static void restrictTo(const SSeasonalComponentSummary& period, CONTAINER& values) {
        if (period.windowed()) {
            values.resize(common::CIntegerTools::floor(values.size(), period.s_WindowRepeat));
...
#endif // INCLUDED_ml_maths_time_series_CSignal_h
