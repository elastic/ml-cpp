...
template <typename POINT> class CBootstrapClusterer {
public:
    ...
    struct SCutState {
        ...
        std::size_t nextToVisit(std::size_t i) const {
            return static_cast<std::size_t>(
                std::lower_bound(s_ToVisit.begin(), s_ToVisit.end(), i) - s_ToVisit.begin());
        }
                ...
                for (boost::tie(k, endk) = boost::out_edges(v, graph); k != endk; ++k) {
                    ...
                    if (this->fromVertex(w).first == i->second) {
                        consistent.push_back(
                            std::make_pair(weight * boost::get(boost::edge_weight, graph, *k), w));
                    }
                }
            }
        ...
        for (std::size_t i = 0u; i < seeds.size(); ++i) {
            if (cut.empty()) {
                ...
                for (std::size_t j = 0u;
                     ...
                     ++j, ++seed) {}
                cut.push_back(
                    std::make_pair(boost::source(*seed, graph), boost::target(*seed, graph)));
            }

            double cost;
            if (this->cutSearch(
                    cut.back().first, cut.back().second, graph, threshold, cost, result)) {
                return true;
            }
            ...
            if (n > 0) {
                ...
                newCut.clear();
                std::set_intersection(cut.begin(),
                                      cut.begin() + n,
                                      cut.begin() + n,
                                      cut.end(),
...
private:
    ...
    TSizeSizePr fromVertex(std::size_t v) const {
        std::size_t i =
            static_cast<std::size_t>(std::upper_bound(m_Offsets.begin(), m_Offsets.end(), v) -
                                     m_Offsets.begin()) -
            1;
...
template <typename POINT> const double CBootstrapClusterer<POINT>::SEPARATION_THRESHOLD(0.1);

//! \brief Extracts the clusters in canonical form (by index into
//! the point vector) for the facade implementations.
template <typename POINT> class CBootstrapClustererFacadeExtractClusters {
public:
    typedef std::vector<std::size_t> TSizeVec;
    typedef std::vector<TSizeVec> TSizeVecVec;
    typedef std::vector<POINT> TPointVec;
...
//! clusterer.
template <typename CLUSTERER> class CBootstrapClustererFacade {};
...
void bootstrapCluster(std::vector<POINT> &points,
    ...
    CBootstrapClustererFacade<CXMeans<POINT, COST>> clusterer(xmeans,
