...
template <std::size_t N> class CMultivariateMultimodalPrior : public CMultivariatePrior {
public:
...
public:
    ...
    CMultivariateMultimodalPrior(const SDistributionRestoreParams &params,
        ...
        traverser.traverseSubLevel(boost::bind(
            &CMultivariateMultimodalPrior::acceptRestoreTraverser, this, boost::cref(params), _1));
    }
    ...
    virtual TUnivariatePriorPtrDoublePr univariate(const TSize10Vec &marginalize,
        ...
        return {
            TUnivariatePriorPtr(new CMultimodalPrior(this->dataType(), this->decayRate(), modes)),
    ...
    virtual TDouble10VecDouble10VecPr marginalLikelihoodSupport(void) const {
        ...
        return {lower.template toVector<TDouble10Vec>(), upper.template toVector<TDouble10Vec>()};
    }
    ...
    virtual TDouble10Vec marginalLikelihoodMode(const TWeightStyleVec &weightStyles,
        ...
        for (const auto &mode_ : m_Modes) {
            ...
            if (prior->jointLogMarginalLikelihood(
                    TWeights::COUNT_VARIANCE, mode, weight_, likelihood) &
    ...
    virtual bool isNonInformative(void) const {
        return m_Modes.empty() || (m_Modes.size() == 1 && m_Modes[0].s_Prior->isNonInformative());
    }
        ...
        do {
            ...
            RESTORE_SETUP_TEARDOWN(
                ...
                traverser.traverseSubLevel(
                    boost::bind(&TMode::acceptRestoreTraverser, &mode, boost::cref(params), _1)),
...
template <std::size_t N> const std::string CMultivariateMultimodalPrior<N>::CLUSTERER_TAG("a");
template <std::size_t N> const std::string CMultivariateMultimodalPrior<N>::SEED_PRIOR_TAG("b");
template <std::size_t N> const std::string CMultivariateMultimodalPrior<N>::MODE_TAG("c");
template <std::size_t N> const std::string CMultivariateMultimodalPrior<N>::NUMBER_SAMPLES_TAG("d");
template <std::size_t N> const std::string CMultivariateMultimodalPrior<N>::MINIMUM_TAG("e");
template <std::size_t N> const std::string CMultivariateMultimodalPrior<N>::MAXIMUM_TAG("f");
template <std::size_t N> const std::string CMultivariateMultimodalPrior<N>::DECAY_RATE_TAG("g");
