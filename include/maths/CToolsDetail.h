...
template <typename T> double CTools::differentialEntropy(const CMixtureDistribution<T> &mixture) {
...
template <typename T> void CTools::spread(double a, double b, double separation, T &points) {
    if (points.empty()) {
        return;
    }
    if (b <= a) {
