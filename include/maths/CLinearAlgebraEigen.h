...
template <typename SCALAR, int FLAGS> struct SZero<CSparseMatrix<SCALAR, FLAGS>> {
...
template <typename SCALAR> using CSparseMatrixElement = Eigen::Triplet<SCALAR>;
...
template <typename SCALAR, int FLAGS> struct SZero<CSparseVector<SCALAR, FLAGS>> {
...
template <typename SCALAR> using CSparseVectorCoordinate = Eigen::Triplet<SCALAR>;
...
class CSparseVectorIndexIterator : public std::iterator<std::input_iterator_tag, std::ptrdiff_t> {
    ...
    std::ptrdiff_t operator*(void)const { return std::max(m_Base.row(), m_Base.col()); }
...
template <typename SCALAR> struct SZero<CDenseMatrix<SCALAR>> {
...
//! Rename to follow our conventions and add to ml::maths.
template <typename SCALAR> using CDenseVector = Eigen::Matrix<SCALAR, Eigen::Dynamic, 1>;

//! \brief Gets a zero dense vector with specified dimension.
template <typename SCALAR> struct SZero<CDenseVector<SCALAR>> {
...
template <typename MATRIX> struct SDenseMatrix { using Type = CDenseMatrix<double>; };
//! \brief Use stack matrix for size 2.
template <typename T> struct SDenseMatrix<CSymmetricMatrixNxN<T, 2>> {
    using Type = Eigen::Matrix<double, 2, 2>;
};
//! \brief Use stack matrix for size 3.
template <typename T> struct SDenseMatrix<CSymmetricMatrixNxN<T, 3>> {
    using Type = Eigen::Matrix<double, 3, 3>;
};
//! \brief Use stack matrix for size 4.
template <typename T> struct SDenseMatrix<CSymmetricMatrixNxN<T, 4>> {
    using Type = Eigen::Matrix<double, 4, 4>;
};

//! Get the Eigen matrix for \p matrix.
template <typename MATRIX> typename SDenseMatrix<MATRIX>::Type toDenseMatrix(const MATRIX &matrix) {
    return matrix.template toType<typename SDenseMatrix<MATRIX>::Type>();
}

//! Get the dynamic Eigen matrix for \p matrix.
template <typename MATRIX> CDenseMatrix<double> toDynamicDenseMatrix(const MATRIX &matrix) {
...
//! DESCRIPTION:\n
//! See SDenseMatrix.
template <typename VECTOR> struct SDenseVector { using Type = CDenseVector<double>; };
//! \brief Use stack vector for size 2.
template <typename T> struct SDenseVector<CVectorNx1<T, 2>> {
    using Type = Eigen::Matrix<double, 2, 1>;
};
//! \brief Use stack vector for size 3.
template <typename T> struct SDenseVector<CVectorNx1<T, 3>> {
    using Type = Eigen::Matrix<double, 3, 1>;
};
//! \brief Use stack vector for size 4.
template <typename T> struct SDenseVector<CVectorNx1<T, 4>> {
    using Type = Eigen::Matrix<double, 4, 1>;
};

//! Get the Eigen vector for \p vector.
template <typename VECTOR> typename SDenseMatrix<VECTOR>::Type toDenseVector(const VECTOR &vector) {
    return vector.template toType<typename SDenseVector<VECTOR>::Type>();
}

//! Get the dynamic Eigen vector for \p vector.
template <typename VECTOR> CDenseVector<double> toDynamicDenseVector(const VECTOR &vector) {
...
template <typename MATRIX> CDenseMatrixInitializer<MATRIX> fromDenseMatrix(const MATRIX &type) {
...
public:
    ...
    std::size_t dimension(void) const { return m_Type->size(); }
...
//! vector objects.
template <typename VECTOR> CDenseVectorInitializer<VECTOR> fromDenseVector(const VECTOR &type) {
