...
struct SCovariancesCustomAdd
{
    ...
    static inline void add(const POINT &x, const POINT &n,
...
struct SCovariancesLedoitWolf
{
    ...
    template<typename T> static T pow2(T x) { return x * x; }
};
}
...
template<typename POINT>
template<typename OTHER_POINT>
void CBasicStatistics::SSampleCovariances<POINT>::add(const OTHER_POINT &x, const OTHER_POINT &n)
{
...
CBasicStatistics::SSampleCovariances<POINT>::operator+=(const SSampleCovariances<OTHER_POINT> &rhs)
{
