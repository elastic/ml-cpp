...
public:
    ...
    template <std::size_t N> static bool isNan(const CVectorNx1<double, N> &val);
    //! Check if any of the components are NaN.
    static bool isNan(const CVector<double> &val);
    //! Check if any of the elements are NaN.
    template <std::size_t N> static bool isNan(const CSymmetricMatrixNxN<double, N> &val);
    //! Check if any of the elements are NaN.
    ...
    template <std::size_t N> static bool isInf(const CVectorNx1<double, N> &val);
    //! Check if any of the components are infinite.
    static bool isInf(const CVector<double> &val);
    //! Check if any of the elements are infinite.
    template <std::size_t N> static bool isInf(const CSymmetricMatrixNxN<double, N> &val);
    //! Check if any of the elements are infinite.
    ...
    template <std::size_t N> static bool isFinite(const CVectorNx1<double, N> &val);
    //! Check if all of the components are finite.
    static bool isFinite(const CVector<double> &val);
    //! Check if all of the components are NaN.
    template <std::size_t N> static bool isFinite(const CSymmetricMatrixNxN<double, N> &val);
    //! Check if all of the components are NaN.
    ...
    //! non-finite values.
    template <typename ITR> class CFiniteIterator {
    public:
    ...
    template <typename T> static CFiniteIterator<typename T::iterator> beginFinite(T &container) {
    ...
    template <typename T> static CFiniteIterator<typename T::iterator> endFinite(T &container) {
...
private:
    ...
    template <typename VECTOR, typename F> static bool aComponent(const F &f, const VECTOR &val);
