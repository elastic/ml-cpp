...
template <typename STORAGE> bool SSymmetricMatrix<STORAGE>::fromDelimited(const std::string &str) {
    return core::CPersistUtils::fromString(
        str, SFromString(), m_LowerTriangle, CLinearAlgebra::DELIMITER);
}

template <typename STORAGE> std::string SSymmetricMatrix<STORAGE>::toDelimited(void) const {
    return core::CPersistUtils::toString(m_LowerTriangle, SToString(), CLinearAlgebra::DELIMITER);
}

template <typename STORAGE> bool SVector<STORAGE>::fromDelimited(const std::string &str) {
    return core::CPersistUtils::fromString(str, SFromString(), m_X, CLinearAlgebra::DELIMITER);
}

template <typename STORAGE> std::string SVector<STORAGE>::toDelimited(void) const {
