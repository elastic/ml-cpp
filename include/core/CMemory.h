    ...
    public:
        ...
        bool registerCallback(void) {
            ...
            if (i == m_Callbacks.end()) {
                ...
                m_Callbacks.insert(i,
                                   {boost::cref(typeid(T)), &CAnyVisitor::dynamicSizeCallback<T>});
                return true;
            }
            return false;
        }
...
public:
    ...
    static std::size_t dynamicSize(const std::set<T, C, A>& t) {
        ...
        return mem +
               (memory_detail::EXTRA_NODES + t.size()) * (sizeof(T) + 4 * sizeof(std::size_t));
    }
    ...
    static std::size_t dynamicSize(const std::list<T, A>& t) {
        ...
        return mem +
               (memory_detail::EXTRA_NODES + t.size()) * (sizeof(T) + 2 * sizeof(std::size_t));
    }
