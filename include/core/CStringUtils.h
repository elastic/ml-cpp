...
public:
    ...
    template <typename T> static std::string typeToString(const T &type) {
    ...
    template <typename T> static std::string typeToStringPretty(const T &type) {
        return CStringUtils::_typeToString(type);
    }
    ...
    template <typename T> static bool stringToType(const std::string &str, T &ret) {
    ...
    template <typename T> static bool stringToTypeSilent(const std::string &str, T &ret) {
...
private:
    ...
    template <typename ITR> static std::size_t computeStringLength(ITR begin, ITR end) {
        std::size_t length(0);
        while (begin != end) {
            length += begin->length();
            ++begin;
