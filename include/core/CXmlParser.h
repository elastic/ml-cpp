...
public:
    ...
    //! return false.
    bool evalXPathExpression(const std::string &xpath, CXmlNode &value) const;

    //! Return value result from an XPath expression, if the number of matches != 1
    //! return false.
    bool evalXPathExpression(const std::string &xpath, std::string &value) const;

    //! Return value result from an XPath expression
    bool evalXPathExpression(const std::string &xpath, TStrVec &value) const;

    //! Return value result from an XPath expression, if there are
    //! duplicates return false.
    bool evalXPathExpression(const std::string &xpath, TStrSet &value) const;

    //! Return a value result from an XPath expression,
    //! if the number of matches != 1 or value is not of type
    //! return false.
    template <typename TYPE> bool evalXPathExpression(const std::string &xpath, TYPE &ret) const {
        CXmlNode value;
    ...
    bool evalXPathExpression(const std::string &, TXmlNodeVec &values) const;

    //! Return result from an XPath expression
    bool evalXPathExpression(const std::string &, TStrStrMap &values) const;
    ...
    static void convert(size_t indentSpaces, const CXmlNodeWithChildren &root, std::string &result);
    ...
    static void convert(const std::string &root, const TStrStrMap &values, std::string &result);
    ...
    //! parser.
    //! Note root is the name of the enclosing value
    bool convert(const std::string &root, const TStrStrMap &values);
    ...
    bool addNewChildNode(const std::string &name, const std::string &value);
    ...
    //! Restrict to simple <name>value</name>
    bool changeChildNodeValue(const std::string &name, const std::string &newValue);
...
private:
    ...
    static void convertChildren(const CXmlNodeWithChildren &current, xmlNode &xmlRep);
