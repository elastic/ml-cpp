...
public:
    ...
    using TStoredStringPtrStatUMap = boost::unordered_map<core::CStoredStringPtr, STATISTIC, core::CHashing::CMurmurHash2String>;
...
public:
    CSampleGatherer(const SModelParams& params,
        ...
        : m_Dimension(dimension),
          m_SampleStats(dimension, params.s_SampleCountFactor, params.s_LatencyBuckets, params.s_SampleQueueGrowthFactor, bucketLength),
          m_BucketStats(params.s_LatencyBuckets, bucketLength, startTime, TMetricPartialStatistic(dimension)),
          m_InfluencerBucketStats(
              std::distance(beginInfluencers, endInfluencers),
              TStoredStringPtrStatUMapBucketQueue(params.s_LatencyBuckets + 3, bucketLength, startTime, TStoredStringPtrStatUMap(1))) {}
    ...
    void acceptPersistInserter(core::CStatePersistInserter& inserter) const {
        ...
        if (m_SampleStats.size() > 0) {
            inserter.insertLevel(SAMPLE_STATS_TAG, boost::bind(&TSampleQueue::acceptPersistInserter, &m_SampleStats, _1));
        }
    ...
    bool acceptRestoreTraverser(core::CStateRestoreTraverser& traverser) {
        ...
        do {
            ...
            RESTORE(CLASSIFIER_TAG, traverser.traverseSubLevel(boost::bind(&CDataClassifier::acceptRestoreTraverser, &m_Classifier, _1)))
            RESTORE(SAMPLE_STATS_TAG, traverser.traverseSubLevel(boost::bind(&TSampleQueue::acceptRestoreTraverser, &m_SampleStats, _1)))
            RESTORE(BUCKET_STATS_TAG,
                    traverser.traverseSubLevel(
                        boost::bind<bool>(TStatBucketQueueSerializer(TMetricPartialStatistic(m_Dimension)), boost::ref(m_BucketStats), _1)))
    ...
    SMetricFeatureData featureData(core_t::TTime time, core_t::TTime /*bucketLength*/, double effectiveSampleCount) const {
        ...
        if (count > 0.0) {
            ...
            if (bucketValue.size() > 0) {
                ...
                for (std::size_t i = 0u; i < m_InfluencerBucketStats.size(); ++i) {
                    const TStoredStringPtrStatUMap& influencerStats = m_InfluencerBucketStats[i].get(time);
                    influenceValues[i].reserve(influencerStats.size());
                    for (const auto& stat : influencerStats) {
                        influenceValues[i].emplace_back(boost::cref(*stat.first),
    ...
    //! label \p value.
    inline void add(core_t::TTime time, const TDouble1Vec& value, unsigned int sampleCount, const TStoredStringPtrVec& influences) {
    ...
    void add(core_t::TTime time,
        ...
        for (std::size_t i = 0u; i < n; ++i) {
            ...
            auto j = stats.emplace(influences[i], CMetricStatisticWrappers::template make<STATISTIC>(m_Dimension)).first;
...
private:
    ...
    struct SStatSerializer {
        void operator()(const TMetricPartialStatistic& stat, core::CStatePersistInserter& inserter) const { stat.persist(inserter); }

        bool operator()(TMetricPartialStatistic& stat, core::CStateRestoreTraverser& traverser) const { return stat.restore(traverser); }
    };
    using TStatBucketQueueSerializer = typename TStatBucketQueue::template CSerializer<SStatSerializer>;
    ...
    public:
        ...
        bool operator()(TStoredStringPtrStatUMap& map, core::CStateRestoreTraverser& traverser) const {
            std::string key;
            do {
                const std::string& name = traverser.name();
