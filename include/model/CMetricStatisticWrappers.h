...
template <typename STATISTIC> struct SMake {
    static STATISTIC dispatch(std::size_t /*dimension*/) { return STATISTIC(); }
};
//! \brief Makes a multivariate metric statistic.
template <typename STATISTIC> struct SMake<CMetricMultivariateStatistic<STATISTIC>> {
...
struct MODEL_EXPORT CMetricStatisticWrappers {
    ...
    //! Make a statistic.
    template <typename STATISTIC> static STATISTIC make(std::size_t dimension) {
    ...
    template <typename STATISTIC> static TDouble1Vec influencerValue(const STATISTIC &stat) {
        return value(stat);
    }
    ...
    static void persist(const TMedianAccumulator &stat,
        ...
        inserter.insertLevel(tag,
                             boost::bind(&TMedianAccumulator::acceptPersistInserter, &stat, _1));
    }
    ...
    static bool restore(core::CStateRestoreTraverser &traverser,
                        CMetricMultivariateStatistic<STATISTIC> &stat) {
        return traverser.traverseSubLevel(
            boost::bind(&CMetricMultivariateStatistic<STATISTIC>::restore, &stat, _1));
    }
};
}
}
