...
$(TARGET): $(OBJS) $(TESTED_OBJS) $(RESOURCE_FILE)
	$(CXX) $(LINK_OUT_FLAG)$@ $(PDB_FLAGS) $(OBJS) $(TESTED_OBJS) $(RESOURCE_FILE) $(LDFLAGS) $(LIBS)

build: $(TARGET)

clean:
	$(RM) $(OBJS_DIR)/*$(OBJECT_FILE_EXT) $(OBJS_DIR)/*.d* $(OBJS_DIR)/*.plist $(OBJS_DIR)/*.xml $(OBJS_DIR)/*.res $(APP_CLEAN) core core.* $(TARGET) $(basename $(TARGET)).pdb $(basename $(TARGET)).map $(basename $(TARGET)).exp junit_results.xml cppunit_results.xml boost_test_results.xml
	$(RMDIR) results
	$(RMDIR) data
