...
2. model_leak.pt - A model that leaks heap addresses via assertion errors.
   Not asserted on separately by test_sandbox2_attack_defense.py: the heap
...
Requires --skipModelValidation when fed to pytorch_inference: these models
use aten::as_strided with an out-of-bounds offset, which CModelGraphValidator
rejects at load time by design (see CModelGraphValidatorTest and
test_pytorch_inference_evil_models.py for that, separate, layer of
coverage).
"""
