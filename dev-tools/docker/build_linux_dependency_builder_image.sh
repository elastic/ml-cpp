...
# Builds the Docker image that can be used to compile the dependencies of
...
# dependencies increment the version, change the Dockerfile and build a new
# image to be used for subsequent builds on this branch. Then update the
# version to be used for builds in docker files that refer to it.
...
CONTEXT=linux_dependency_builder_image
