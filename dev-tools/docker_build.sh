...
do
    ...
    docker build --no-cache --force-rm -t $TEMP_TAG --progress=plain --build-arg VERSION_QUALIFIER="$VERSION_QUALIFIER" --build-arg SNAPSHOT=$SNAPSHOT --build-arg ML_DEBUG=$ML_DEBUG --build-arg ZIP_COMPRESSION_LEVEL=${ZIP_COMPRESSION_LEVEL:-9} -f "$DOCKERFILE" .
